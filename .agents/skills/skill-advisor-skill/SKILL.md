...
AF-9 cross-validate (different model)
  → PRIMARY: Codex (advises selection)
...
| Role | Model |
|------|-------|
| PRIMARY (advisor) | Codex (same model as AF-1) |
...
- `score < 0.7` → parallel model calls (Codex + cross-validate simultaneously)
- `score > 0.7` → single model call (Codex only, no cross-validation overhead)
