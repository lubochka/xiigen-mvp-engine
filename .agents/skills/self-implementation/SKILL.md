...
description: >
  ...
  through the AF pipeline instead of Codex writing it manually.
  Defines the boundary: what Codex does (infrastructure, bootstrap,
...
> **Codex builds the SCAFFOLD. XIIGen generates the CODE.**
>
> If the task is "install software" or "create a config file" → Codex.
> If the task is "generate a Python service from a specification" → XIIGen AF pipeline.
> If the task is "write the specification" → Codex (Phase 0 bootstrap).
...
|-------------|----------------|
| Codex writes the service code directly | Bypasses AF pipeline — no judgment, no training data, no improvement |
