...
CRITICAL — change reaches all consumers across the whole repo
  Triggers: public interface signature change, DI token contract change,
            ...
            DNA rule change, AGENTS.md / CLAUDE.md fundamental change
...
|-----------------|----------------------|
| `.agents/skills/`, `.claude/skills/` (skill files) | LOW |
...
| `AGENTS.md` / `CLAUDE.md` (governs all sessions) | HIGH |
...
Changing which DNA rules apply or how they are enforced:
  ...
  → update AGENTS.md / CLAUDE.md and dna-compliance-guard simultaneously.
...
- `change-propagation` (SK-440) — once reach is known, propagate the change to every
  derived view (BOTH skill catalogs, indexes, interfaces doc).
- service-boundary-design — the 3-consumer "promote to a class behind an interface"
  line is exactly where a change becomes CRITICAL here.
