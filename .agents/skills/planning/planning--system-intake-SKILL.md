...
## G08 universal addition from llm_mvp_core — TS-native extraction, typed package, RAG sidecar source-type

The four properties (structure / intent / constraints / quality) and "code is truth,
spec is intent" are the universal core. This block makes the extraction concretely
TS-native for mvp and adds the source-type the original protocol omits.

### A. Code-First extraction — exact mvp grep protocol (replaces C#-centric examples)

```
Entry points  (structure): NestJS controllers / route decorators + service methods
  grep -rn "@\(Controller\|Get\|Post\|Put\|Delete\|Patch\)(" server/src
Service boundaries:        @Injectable providers and what each module owns
  grep -rln "@Injectable()" server/src ; ls server/src/**/*.module.ts
Constraints (intent):      guard clauses + typed Result + thrown HttpException
  grep -rn "throw new \(HttpException\|.*Exception\)\|return .*Result\.\(fail\|err\)" server/src
Event contracts:           queues / emitters / CloudEvents
  grep -rn "emit(\|@OnEvent(\|CloudEvent\|\.publish(" server/src
Quality signals:           TODO/FIXME/HACK + swallowed errors + missing spec coverage
  grep -rn "TODO\|FIXME\|HACK" server/src ; find server/src -name "*.spec.ts" | wc -l
```

The constraint to assert on is the typed `DataProcessResult<T>` / `Result<T>` discriminator,
NOT the HTTP status code.

### B. The domain context package is a TS type, not a C# record

When you emit the package as code (for the convergence input), declare it as a TypeScript
`interface`/`type`, not a C# `record`:

```ts
interface DomainContextPackage {
  sourceType: 'code' | 'design' | 'documentation' | 'rag-sidecar' | 'mixed';
  structure:   { topLevelComponents: string[]; boundaries: string[]; entryPoints: string[] };
  intent:      { summary: string; businessRules: string[]; invariants: string[] };
  constraints: { explicit: string[]; implicit: string[]; unknowns: string[] };
  quality:     { gaps: string[]; knownFailures: string[]; brittleAreas: string[] };
}
```

### C. New source-type — FastAPI RAG sidecar (Python)

mvp has a Python retrieval sidecar. Intaking a capability that touches retrieval requires
extracting from it as its own source-type:

```
structure:   APIRouter routes + request/response Pydantic models (the Node↔sidecar contract)
             grep -rn "APIRouter\|class .*(BaseModel)" rag/
intent:      what retrieval the sidecar performs for the Node side (the consumer requirement)
constraints: payload shape the NestJS RAG fabric sends + expects back; embedding/index config
quality:     pytest coverage present? uvicorn health route present? error handling on miss?
             find rag -name "test_*.py" | wc -l
```

If the request/response contract between the Node RAG fabric and the sidecar cannot be
read, emit `CONTEXT_INSUFFICIENT` (DOWNSTREAM_CONTRACT) — do not guess the wire shape.

### Note-only (NOT ported — stays in G12, R5)

The shared embedding/DPO model internals are owned by `llm_mvp_core` and consumed here via
`.xiigen` manifests; intake records the *contract* to that model, not its training internals.
