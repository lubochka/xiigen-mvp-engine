...
## G08 universal addition from llm_mvp_core — generic subproject unit, Jest gates, mvp record destination

The universal core — FOUNDATION classification (downstream-dependents > 2), wave ordering,
absolute-vs-delta gate model, sequential-vs-parallel (≥3 prior completions), and a written
wave-assignment record — is here, framed around FLOWs. This block generalizes the unit and
pins the gate commands for mvp work that is NOT a flow.

### A. "Subproject" unit for mvp

The wave unit is not only `FLOW-NN`. It is any of: a NestJS module, an engine node-handler,
a React feature, or a RAG sidecar route. The FOUNDATION test is unchanged:

```
FOUNDATION (== Wave -1) : more than 2 downstream units depend on this unit's output.
  e.g. a shared NestJS provider/contract that 3+ modules import → FOUNDATION.
  e.g. a React feature that only one route uses → Wave 0+, not FOUNDATION.
```

### B. Gate commands for mvp (absolute vs delta)

```
Absolute gate : cd server && npx jest 2>&1 | grep "Tests:" | tail-1  → Failed: 0
                (client) npx jest / npx playwright test               → 0 failed
Delta gate    : Jest test COUNT at phase end >= count at phase start, and it does NOT pass
                if tests were deleted to remove failures. Restricted to Wave 2+ parallel.
```

Use absolute for FOUNDATION, Wave 0, Wave 1, and Wave 2+ sequential; delta only for Wave 2+
parallel where the baseline is stable.

### C. Record destination (mvp)

Record the wave assignment in the mvp decision log — `.claude/skills/DECISIONS-LOCKED.md`
or the plan-adjacent `ARCHITECTURE-DECISIONS.json` — with the downstream-dependent count, the
reasoning, and the gate type. mvp does not use a `docs/SUBPROJECT-MAP.md` for this.

### Note-only (NOT ported — stays in G12, R5)

Cost-trajectory math tied to shared-model token savings is owned by `llm_mvp_core`; mvp wave
assignment records ordering/gate decisions, not the shared model's training-cost curve.
