...
## UNIVERSAL STANDARD ADDENDUM — UML-Simulation → Test-Case rows + Part A↔B mapping (ported from llm_mvp_core, uml-simulation-to-testcase)

> Added by the Universal-Skills refresh (UpdateUniversalSkills). Source standard:
> `llm_mvp_core/docs/skills/uml-simulation-to-testcase-SKILL.md`. The step/gap
> simulation tables above are kept. mvp's simulation protocol did NOT convert each
> UML connection/branch into a concrete test-case row with a `test_layer` and an
> explicit Part A↔Part B mapping — this addendum adds that conversion. TS adaptation:
> `test_layer` maps to Jest (unit/integration) and Playwright (e2e_host/qa_automation),
> plus negative; `responsible class/method` are TS (`@Injectable` services, hooks).

### Step 1 — Every §0 UML arrow becomes a per-connection test-case row

```
| connection (from→to) | data_sent | example_input | expected_output | failure_case | responsible class/method | test_case_id | test_layer |
|----------------------|-----------|---------------|-----------------|--------------|--------------------------|--------------|------------|
| RegisterController→RegisterService | RegisterDto | {email:"a@b.co",pwd:"x"} | DataProcessResult.success(userId) | duplicate email → .failure("EMAIL_TAKEN") | RegisterService.register | TC-REG-01 | jest:integration |
| RegisterService→IUserStore | user doc | {email, tenantId} | stored doc id | store down → .failure | UserStore.create | TC-REG-02 | jest:unit |
| useRegistration→/register | POST body | form values | 201 + redirect | 409 shows inline error | useRegistration hook | TC-REG-E2E-01 | playwright:e2e_host |
```

`test_layer` vocabulary (TS-adapted):
```
jest:unit            — pure logic, no I/O
jest:integration     — real NestJS DI (@nestjs/testing), in-memory providers
playwright:e2e_host  — real client UI path
playwright:qa_automation — human labels / accessible names / error clarity (UX layer)
negative             — invalid input / boundary / failure branch (any layer)
```

### Step 2 — Every §1 flowchart branch becomes a per-branch test-case row

```
| branch condition | example_input | expected_output (DataProcessResult) | test_case_id | test_layer |
|------------------|---------------|-------------------------------------|--------------|------------|
| email already registered | existing email | .failure("EMAIL_TAKEN") | TC-REG-03 | jest:integration / negative |
| valid new email | fresh email | .success(userId) + verification queued | TC-REG-04 | jest:integration |
| store unavailable | any | .failure("STORE_DOWN"), no partial write | TC-REG-05 | jest:unit / negative |
```

### Step 3 — Part A ↔ Part B mapping (bidirectional, no orphans)

```
Every Part A branch/connection row  → a Part B work order + the test_case_id above.
Every Part B branch/work order      → appears as a Part A row.
Only small technical details may differ in Part B (exact Jest/Playwright filter,
gate number, CI step, UI/UX detail, error handling, perf threshold).
A whole branch present on one side and missing on the other = FAIL.
```

**FAIL if:** any §0 arrow or §1 branch has no test-case row; any test-case row has a
stub value (`TBD`, `...`) instead of a concrete input/expected; or any Part A branch
has no Part B mapping (or vice-versa).
