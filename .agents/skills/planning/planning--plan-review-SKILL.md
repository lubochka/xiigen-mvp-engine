...
## UNIVERSAL STANDARD ADDENDUM — Gate 0, FC-0, FC-13, FC-14, Dishonest-Claim Rejection (ported from llm_mvp_core)

> Added by the Universal-Skills refresh (UpdateUniversalSkills). Source standard:
> `llm_mvp_core/docs/skills/plan-review-SKILL.md`. These gates were ABSENT in the
> mvp plan-review library (FC-1..FC-31 covered structure/counts/naming/stack-coupling,
> but not "reviewer reads the plan", per-connection simulation review, or dishonest-claim
> rejection). Everything below is additive — the FC-1..FC-31 battery above stays binding.
> TS adaptation: this mvp project's domain return type is `DataProcessResult<T>`
> (`server/src/kernel/data-process-result.ts`, `.success`/`.failure`), NOT core's
> `OperationResult<T>`; tests are Jest (server) + Playwright (client); the fabric is
> NestJS DI (`@Injectable` / `providers: [...]`).

### Gate 0 — Reviewer reads the plan itself (writer report = evidence only)

A plan review is INVALID if the reviewer only read the author's summary/report.
The reviewer must open and read the actual plan document AND every file/line it
cites before any verdict.

```
Gate 0 admission record (must be filled before FC-1 runs):
  reviewer_read_plan_document      = true            # opened PLAN.md / SESSION-*.md, not the summary
  reviewer_read_cited_refs         = true            # opened every server/src/...:line the plan quotes
  writer_report_is_evidence_only   = true            # the author's "done" claim is a claim, not proof
  reviewer_id_or_subagent_id       = <id>            # a real reviewer identity, not "parent self-review"
```

If `reviewer_read_plan_document=false`, STOP — the review cannot proceed on a
report alone. Parent/self review of one's own plan is not an independent review.

### FC-0 — Requirement Traceability (authority → plan section → test/gate → artifact)

Before structural checks, every explicit requirement from the active authority
(current operator instruction, approved work order, accepted reviewer gate) must
map to a concrete plan section, a test/gate, and a produced artifact.

**Detection (human + grep-assisted):**
```bash
# Build the ledger by hand from the authority text, then verify each row resolves:
#   requirement -> plan §  -> Gate B (jest/playwright filter) -> artifact path
# An unmapped, deletion-only, or contradicted requirement BLOCKS execution.
grep -niE "requirement|must|shall|deliver" PLAN.md | head -40
```

**Replacement-mapping rule (ported):** removing an old mechanism is NOT coverage
for a required replacement. If the authority requires a trainable/learned unit,
the plan must name the unit, its trained state, checkpoint/manifest, export/import
path, locator path, fresh-load probe, continue-training probe, and ablation/leakage
gate. A deterministic helper (regex, BM25, dictionary, direct lookup) in a
production route is not coverage for a trainable requirement.

**FAIL if:** any binding requirement has no plan-section + test/gate + artifact row,
or any plan sentence contradicts a binding requirement.

### FC-13 — Part A Completeness Contract (plain-language §0–§5 readable plan)

A plan is not admissible unless its user-readable **Part A** contains §0–§5 at full
depth AND Part A↔Part B match at branch level. (Detail of each section lives in the
`implementation-doc-*` skills; FC-13 is the review gate that they are present.)

```
□ §0 Class diagram — Mermaid classDiagram importable to draw.io; EVERY class has a
     human-language `note for ClassName` (what it does + status: COMPLETE/PARTIAL/
     STUB/SKELETON); legend table; ALL relationship arrows.
□ §1 Algorithms in plain language (the operator's working language) — per algorithm: exact CLASS + implementing
     METHOD/function + PARAMETERS + actual STEP-BY-STEP. A functional spec
     ("what it should do") + a class name alone is NOT sufficient.
□ §2 Algorithm descriptions — human analogy, init/training data with example
     values, step-by-step inference, "What it does NOT do", method-signature block with
     real example values (TS signatures: `Promise<DataProcessResult<T>>`).
□ §3 Code references — WHY/HOW/WHAT per cited block; precise `path:line` reflecting
     code AS ACTUALLY IMPLEMENTED (`.ts`).
□ §4 Status table — one row per component (Component|File|Status|What actually
     works now); "Partially implemented" without naming working vs stub
     methods is banned.
□ §5 Hierarchy — only when an ILearningMachine-style level hierarchy is touched
     (core_specific; for mvp this is usually N/A — record N/A explicitly).
```

**Forbidden substitutions (each blocks admission, not a deferrable repair row):**
a functional spec in place of an algorithm; a class name in place of the
implementing function + parameters; a `path:line` citation in place of a §3
explanation; a §0 diagram without `note for` on every class; an absent §2 or §3.
A plan that names Part A but writes §1 as a bare functional spec instead of plain
prose in the operator's working language also FAILS. (This is the "V20 failure pattern" the standard guards.)

### FC-14 — Per-connection & per-branch simulation review

The reviewer verifies that §0 carries a **per-connection simulation table** and §1
carries a **per-branch simulation** — and that each row is concrete (real example
input + expected output), not a placeholder.

```
Per-connection (one row per arrow in the §0 classDiagram):
  | from→to | data_sent | example_input | expected_output | failure_case | responsible class/method |
Per-branch (one row per decision branch in the §1 flowchart):
  | branch condition | example_input | expected_output (DataProcessResult.success/failure) |
```

**FAIL if:** any arrow has no simulation row, any §1 branch has no simulation row,
or any row is a stub (`example`, `TBD`, `...`) instead of a concrete value.

**Part A ↔ Part B parity:** every algorithm branch/decision in Part A maps to a
Part B work order, and every Part B branch appears in Part A. Only small technical
details may differ in Part B (exact Jest/Playwright filters, gate numbers, CI steps,
UI/UX detail, error handling, perf thresholds). A whole missing branch in either
direction is a FAIL.

### Dishonest Claim Rejection Gate (ledger rows ≠ cycles; parent/self ≠ sub-agent)

Counting is not evidence. A claim of "N review cycles" / "N passes" requires N
INDEPENDENT evidence packets, each naming: reviewer/sub-agent id, artifact
before/after refs, loaded skills/gates, findings (or a justified no-fix), fix refs
when needed, and recheck evidence.

```
FORBIDDEN as a "cycle":
  - a checklist row, ledger row, or generated table row called a "cycle"
  - parent/self review counted as an independent sub-agent review
  - "N reviews" without N independent evidence records
  - "done"/"ready"/"verified" when evidence is narrower, stale, inferred, or uninspected
...
The Review-Fix ledger is an INDEX only; N ledger rows after one real review is a
failed audit.

### Plan-Not-Ready = repair loop (not a stop, not execution)

If this review returns NEEDS_FIX, the plan is not admitted to execution and the
correct next step is plan repair, then re-review — NOT a final stop and NOT
"executor may proceed anyway". "Do not stop" / old `next_action` / an AI-authored
work order do not authorize execution while Part A (FC-13) or simulation (FC-14)
is incomplete. Repair continues until the packet is a human-readable plan in the operator's working language
with §0–§5 and concrete simulations.

### Gate B adaptation for this mvp project (Codex/Claude Code handoff)

```
Gate B (per placement) = the exact passing condition as a command/assertion:
  server tests:  cd server && npx jest <filter>   → failures === 0
  client e2e:    Playwright spec                   → green
  compile:       cd server && npx tsc --noEmit     → 0 new errors
Reviewer also checks: NestJS DI boundaries (constructor injection / `providers`),
and that public Application/service methods return `DataProcessResult<T>` (.success/
.failure) — never throw for business logic, never core-`OperationResult<T>`.
```

### Updated Gate A — add the universal rows

```
✅ Gate 0: Reviewer read the plan + cited refs (writer report = evidence only)
✅ FC-0:  Requirement traceability (authority → § → Gate B → artifact); replacement-mapping
✅ FC-13: Part A §0–§5 present at full depth, plain-language, Part A↔B branch parity
✅ FC-14: Per-connection + per-branch simulation rows present and concrete
✅ Dishonest-Claim Rejection: N cycles = N independent packets; ledger rows ≠ cycles
✅ Plan-Not-Ready handled as repair loop, not stop, not execution
```
