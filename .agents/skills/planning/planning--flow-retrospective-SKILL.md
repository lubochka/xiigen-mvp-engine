...
## UNIVERSAL CORE ALIGNMENT — G10 phase-retrospective (added 2026-06-29)

This section reconciles SK-464 with the universal **phase-retrospective** gate so
the four universal dimensions are all present. SK-464 already covers two of them:
universal **D1 Cycle-Count Accuracy** = SK-464 Dimension 1 (MAE > 1.0 →
recalibrate) and universal **D2 Pattern Transfer Rate** = SK-464 Dimension 2.
The two universal dimensions below were not yet expressed in SK-464 terms; they
are added here without changing anything above. The XIIGen mvp stack is **pure
TypeScript (NestJS / React / Vite)** — there is **no `dotnet`** — so Gate-B is
Jest (`server` + `client`) plus Playwright (`client/e2e`, `e2e/`).

### D3 (universal) — Behavioral Test Coverage

Every behaviour category the batch *claimed* to deliver must have **at least one
green test** that exercises that category. A claimed category with zero passing
tests is a coverage hole, not a pass.

```bash
# For each claimed category, prove ≥1 green test exists (Jest and/or Playwright).
# Map categories → Playwright/Jest test names; a category with no green test fails D3.
#   e.g. category "empty-state CTA" -> e2e/<flow>-empty-state.spec.ts must be green
#        category "role-denied"     -> a Jest case asserting the denied render must pass
```

D3 threshold: `claimed_categories_with_green_test == claimed_categories_total`.
Any shortfall reopens the phase for the missing category.

### D4 (universal) — Gate-B Adherence (BINARY)

Gate-B adherence is **binary**, not a percentage. If the batch had 8 Gate-B
conditions and 7 are met, the result is `NOT_ADHERED` and the phase is
**reopened** — "7 of 8" is a fail, not "87.5% done".

```bash
# Gate-B set for mvp (NO dotnet — pure TS):
#   - Jest server suite green        (npm --prefix server test  / jest)
#   - Jest client suite green        (npm --prefix client test  / jest)
#   - Playwright run green           (client/playwright.config.ts, e2e/playwright.config.ts)
#   - Populated-state PNGs captured  (?mock=… per FC-18 UX-06b)
# Binary rule:
ALL_GATE_B_MET=true   # set false if ANY required Gate-B condition is unmet
echo "D4 verdict: $([ "$ALL_GATE_B_MET" = true ] && echo ADHERED || echo 'NOT_ADHERED — reopen phase')"
```

D4 threshold: ALL required Gate-B conditions met → `ADHERED`; otherwise
`NOT_ADHERED` and the phase reopens. There is no partial credit.

### Four-dimension roll-up (record alongside the JSON above)

```json
{
  "universalRetro": {
    "D1_cycleCountAccuracy": { "mae": 0.67, "action": "acceptable" },
    "D2_patternTransferRate": { "count": 7, "expected": 7, "ok": true },
    "D3_behavioralTestCoverage": { "claimedCategories": 5, "withGreenTest": 5, "ok": true },
    "D4_gateBAdherence": { "required": 4, "met": 4, "verdict": "ADHERED" },
    "nextBatchClearToProceed": true
  }
}
```

`nextBatchClearToProceed` is `true` only when D1 is acceptable (MAE ≤ 1.0), D2 ≥
expected, D3 full coverage, and D4 = `ADHERED`. Any failed dimension makes it
`false` and the next batch must not begin — same hard-gate discipline as the
Wave 2 gate above.

> Scope note (R5/R6): the original universal **D2** measured PROVEN promotions in
> a `DomainModelRegistry`; that registry and any model-promotion machinery stay
> in `llm_mvp_core` (G12). On mvp, D2 is the general pattern-transfer discipline
> (SK-464 Dimension 2) — no model is trained or promoted here.
