...
## G08 universal addition from llm_mvp_core — this IS `parallel-execution-coordinator` (name reconcile)

In the core universal catalogue this skill is named **`parallel-execution-coordinator`**. The
mvp file name (`parallel-wave-coordinator`, SK-434) is a local alias — when a plan or another
skill cites `parallel-execution-coordinator`, it means THIS file. Do not create a second
skill under the core name; cite this one. The three behavior switches and three startup
questions above already match the core standard; this block only pins the canonical wording.

### The three switches (canonical universal names)

```
1. Pre-allocated artifact ranges  (NOT live-query)  → Question 2 above.
2. Delta gate                     (NOT absolute baseline) → "Gate model" above.
3. CAS lifecycle write            (expectedCurrentStatus, NOT blind write) → "Lifecycle CAS write".
```

### The three session-start questions (canonical)

```
Q1: parallel?            → parallel_wave in STATE.json null vs N.
Q2: ranges?              → pre-allocated ranges present? if not → STOP (no live query).
Q3: prerequisite Gate B? → wave_prerequisite flow/unit ACTIVE? if not → STOP.
```

### mvp delta-gate command (the universal switch #2, made concrete)

```
delta gate : npx jest --testPathPattern=test/flow-{XX} --silent
             assert {expected_delta} NEW tests passing for this unit; do NOT check the
             absolute total here (post-merge CI verifies the total). For a non-flow unit,
             scope the pattern to that module/feature's test path.
```

### Note-only (NOT ported — stays in G12, R5)

Pre-allocation of shared-model artifact ids (DPO/checkpoint numbering) is a `llm_mvp_core`
concern; mvp pre-allocation covers product artifact ranges (T/F/SK/CF) only.
