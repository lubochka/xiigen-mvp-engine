...
### FC-13: Cross-Phase Consistency
**NEW v1.1**

Claims in one phase contradict or duplicate claims in another phase. This class
is not caught by any of FC-1 through FC-12 because those check each phase in
isolation. FC-13 requires a cross-phase pass.

**The three cross-phase failure patterns:**

**Pattern A — Duplicate work:** Phase N adds RAG retrieval to component X.
Phase M also adds RAG retrieval to component X. One will be a no-op or they
will conflict at runtime.

**Pattern B — Contradictory framing:** Phase 0 says component X "needs to be
wired." Phase 0 Step 2 Case A says "if X is not imported, add it." Phase 3
says "wire X before context package assembly." If X is already wired and
working (as the live run shows), the framing in all three places is wrong,
and the plan will produce unnecessary re-implementation work.

**Pattern C — Wrong prerequisite chain:** Phase N says "requires Phase M
complete." Phase M's gate does not actually verify what Phase N needs. The
dependency is stated but the handoff is not real.
...
# Step 1: Extract all phase deliverables
phases = re.split(r'## PHASE \d+', content)

# Step 2: For each deliverable item, check if it appears in more than one phase
deliverables = {}
for i, phase in enumerate(phases):
    items = re.findall(r'^\*\*Item \d+.*?\*\*:(.+?)$', phase, re.MULTILINE)
    for item in items:
        key = item.strip()[:60]
        if key not in deliverables:
            deliverables[key] = []
        deliverables[key].append(i)

for key, phases_found in deliverables.items():
    if len(phases_found) > 1:
        print(f"FC-13 CANDIDATE: '{key}' appears in phases {phases_found}")
```

**Manual cross-reference pass (required — automation finds candidates, human confirms):**

Read Phase 0 Step 3 and Phase 3 Item 3 side by side.
Ask: "does Phase 3 Item 3 assume Phase 0 Step 3 did NOT do this?"
If both describe the same change to the same component → flag as duplicate.

**Gate passes when:**
- No deliverable item describes the same change to the same component in two phases
- Every prerequisite dependency has a verifiable handoff (not just a status check)
- Phase N+1 assumptions are satisfied by Phase N's gate, not just by Phase N's intent

## The Review Protocol (11 steps)
...
**Step 11** — Cross-Phase Consistency (FC-13) ← NEW v1.1
...
✅ FC-13: Cross-phase consistency — no duplicate work, no contradictory framing
```
...
| Phase N and Phase M both describe same change to same component | 13 | One phase is a no-op or both phases conflict at runtime |
