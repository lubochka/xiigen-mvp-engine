...
## 9 vs 10 — Reconciling With the llm_mvp_core DNA (G04 universal addition)

`llm_mvp_core` states **10** DNA rules; mvp is the ORIGIN with **9** TS DNA
patterns above. The difference is not a missing rule — it is a mapping:

| core Rule | mvp status | note |
|-----------|-----------|------|
| 1 `OperationResult<T>` everywhere | = mvp DNA-3 | mvp uses `DataProcessResult<T>` (same idiom; never `OperationResult<T>`/`Result<T>` in mvp) |
| 2 Fabric-First | = mvp DNA-4 + fabric interfaces | inject deps via DI ports, never `new ConcreteProvider()` |
| 3 every line commented | style rule | applied in review, not a grep gate here |
| 4 zero failures hard gate | = `dev-safety` SESSION GATE | lives in dev-safety, not a DNA grep |
| 5 read before touching | = `code-examination` | understand-before-touching |
| 10 **no external ML frameworks** | **note-only for mvp** | trainable/ML logic stays in `llm_mvp_core` (R5/R6); mvp consumes models via manifest/locator. Do not add PyTorch/TensorFlow/ML.NET/TorchSharp to mvp. |

So the count gap is reconciled, not a hole: mvp's 9 grep-detectable DNA patterns
+ the cross-cutting core rules (commented above) = the same discipline.

### `OperationResult<T>` namespace note (does NOT apply to mvp code)

The core namespace-mix rule (`AgentInfrastructure` vs `AgentAlgorithms` vs
`SemanticCompiler29` `OperationResult<T>`) is a **core-only** concern. mvp returns
`DataProcessResult<T>` and has no such namespaces — do not import or assert
`OperationResult<T>` in mvp. (BaseService single-ctor `BaseService(ISessionStorage)`
is likewise a core pattern; mvp's equivalent is `extends MicroserviceBase`, DNA-4.)

### Layer-5 link (trainable-vs-stub)

When a modified `.ts` claims a **trainable** capability, the DNA grep gate is not
enough. Hand off to `generated-code-review` **Layer 5** (numeric held-out metrics,
or explicit honest-untrained-scaffold) before the file may be committed as a
trained feature. A DNA-clean file that fakes a trained metric still fails review.
...
dna-compliance-guard
  ...
  → generated-code-review (Layer 5) → trainable-vs-stub when a trained claim is made
