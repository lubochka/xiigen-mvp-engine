...
## RULE 5 — MULTI-GENERATE CONTEXT (when A-4 is active) — NOTE-ONLY IN THIS MVP

> **MVP scope note (note-only):** Rule 5 is tied to the core multi-generate topology
> (DPO arbiter, `ai-pipeline-topology` Pattern F) and the `xiigen-training-data` store,
> which by R5/R6 stay in `llm_mvp_core`. This MVP consumes models through a manifest and
> runs no DPO multi-generate loop, so Rule 5 is reference-only here — do not stand up an
> Elasticsearch training-data store or a multi-generate handler in the MVP to satisfy it.
> Rules 0–4 and 6 are the active patch-authoring contract for the MVP.
...
## MVP PATCH-HISTORY SEAM

The MVP already keeps a de-facto patch history in scattered `PATCH--*.md` files and the
`BEHAVIORAL-CORRECTIONS-REGISTRY`. Treat those as the existing `PatchHistoryEntry` log, and
apply this one formalized contract to every new correction so patches do not regress each
other: one bottleneck per patch (Rule 0), a NAMED_CHECK WRONG/CORRECT pair (Rule 1), the
0.80–0.90 target zone (Rule 2), and the Rule 3 regression check against existing NAMED CHECK
sections. A `REGRESSED` outcome is kept as valuable data, not deleted.
