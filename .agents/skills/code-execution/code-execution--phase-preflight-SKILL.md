...
## UNIVERSAL STANDARD ADDENDUM — Explicit PENDING→verified Assumption status (ported from llm_mvp_core, phase-preflight)

> Added by the Universal-Skills refresh (UpdateUniversalSkills). Source standard:
> `llm_mvp_core/docs/skills/phase-preflight-SKILL.md`. The runtime preflight protocol
> and Patterns 1–3 above are kept. mvp lacked an EXPLICIT per-assumption status
> lifecycle where every assumption about the codebase starts PENDING and must be
> driven to verified (or BLOCKING) before the first line of code. TS adaptation:
> verify commands read NestJS (`@Injectable`, `providers: [...]`, constructor
> injection), React components/hooks, and FastAPI routes — `.ts` not `.cs`; method
> shapes are `Promise<DataProcessResult<T>>`.

### Assumption Registry — every row carries a status, default PENDING

No assumption about the codebase is "known" until a verify command proves it. Each
STEP 0 assumption row starts at `PENDING` and is driven to `verified` or `BLOCKING`.
Writing code while any consumed assumption is still `PENDING` is the failure this
status lifecycle prevents.

```markdown
## ASSUMPTION REGISTRY (STEP 0) — status starts PENDING

| # | Assumption (about the codebase) | Verify command | Expected | Status | Fallback |
|---|----------------------------------|----------------|----------|--------|----------|
| A1 | RegisterService.register(dto) exists with this signature | grep -n "register(" server/src/.../register.service.ts | 1 hit, (dto: RegisterDto) | PENDING → verified | none = BLOCKING |
| A2 | RegisterService is provided in its module | grep -n "providers:" server/src/.../registration.module.ts | RegisterService listed | PENDING → verified | add to providers |
| A3 | register() returns Promise<DataProcessResult<...>> | grep -n "DataProcessResult" server/src/.../register.service.ts | 1+ hits | PENDING → verified | none = BLOCKING |
| A4 | useRegistration() hook calls the API path | grep -n "useRegistration" client/src/... | hook exists | PENDING → verified | wire hook |

Status legend:
  PENDING   — not yet verified (default; no code may consume it)
  verified  — verify command produced the expected output
  BLOCKING  — verify failed AND no fallback → STOP, escalate (do not write code)
```

**Rule:** at the end of STEP 0, every row is `verified` or has an applied fallback.
Any remaining `PENDING` or `BLOCKING` row that a planned change depends on stops the
session and goes to ISSUE INVENTORY + the escalation format above.

### Fix Patterns map to verified assumptions

Pattern 1 (handler injection), Pattern 2 (TS field access camelCase vs snake_case),
and Pattern 3 (module registration) above each correspond to the assumption that is
"almost always wrong" — that assumption MUST be a `PENDING` row in the registry and
must reach `verified` before the matching fix is written.
