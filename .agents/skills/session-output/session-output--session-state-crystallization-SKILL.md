...
## Universal Bits (UUS G07) — ≤40 hard cap, delta-only, Gate D, TS verify formulas
...
### The snapshot is a hard ≤40-line cap and writes only what CHANGED

The universal rule: the snapshot is **≤40 lines (hard cap, not a guideline)** and contains **only what changed since the last snapshot**. Anything derivable from a verification command is cut, not copied. A snapshot longer than the cap is a HANDOFF-PROMPT replacement and is rejected — re-write with only the delta. (The existing `ARCHITECTURE-INFRASTRUCTURE` snapshot in this repo is currently over the cap and must be trimmed to conform.)

### Required fields (universal)

Every snapshot carries these fields, each as a delta: **EXECUTION POSITION**, **BLOCKING PREREQUISITES** (each with a verbatim, runnable verify command + expected result — never paraphrased), **ACTIVE BUGS**, **ARTIFACT BOUNDARIES**, **SUBPROJECT STATUS** (for mvp this is RAG-pipeline / NestJS-module / client status, not per-FLOW), and **DECISIONS MADE THIS SESSION**.

### Gate D — goal advancement (mandatory in the snapshot)

The snapshot must show the goal moved, not merely that the session ran. Add one line:

```markdown
Goal advanced:  [what is true now that was not true at session start] | Verify: [binary command] → Expected: [result]
```

If no goal-state difference can be named with a binary check, the session has not advanced and the snapshot must say so explicitly (it is not "complete").

### Verify formulas adapted to the mvp stack (Jest / wc -l, not dotnet / PowerShell)

```bash
# Test count for ARTIFACT BOUNDARIES — Jest, not `dotnet test --no-build`:
cd server && npx jest 2>&1 | grep "Test Suites:\|Tests:" | tail -2
cd client && npx jest 2>&1 | grep "Tests:" | tail -1

# ≤40-line cap check — wc -l (node/posix), not PowerShell Get-Content:
wc -l SESSION-STATE-SNAPSHOT--$(date +%Y-%m-%d).md   # Expected: ≤ 40
```

The snapshot-example and verify commands inherited from core are hardcoded to `.NET` (`dotnet test`, `git log --oneline`); for mvp rewrite each field's formula under Jest/Playwright and confirm the ≤40 cap holds in the actual snapshot.
