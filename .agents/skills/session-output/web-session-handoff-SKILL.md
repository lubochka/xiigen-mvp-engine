...
## Universal Bits (UUS G07) — self-contained brief, required sections, handoff hierarchy
...
### The brief is a self-contained bridge between actors

The universal rule: `SESSION-BRIEF-{phase}.md` is a **self-contained** bridge between actors — it must not reference any file the reader has to open to understand it. The reader may have no repo access. A brief that says "see EXECUTION-LOG-B.json" fails this rule; inline the facts instead.

### Required sections (universal)

- **PROJECT STATE** — always present, always current.
- **WHAT PHASE BUILT** — prose (3–5 sentences), business/architectural meaning, **not** a file list.
- **DISCOVERIES** — facts established this session, each with a `verified_by` command.
- **REJECTED CLAIMS** — wrong claims caught, each with what was actually true + how it was disproved.
- **OPEN ITEMS** — **omitted entirely** when nothing needs deciding (never "(none)").
- **WHAT NEXT WILL BUILD** + **Gate** — always present.

### Handoff hierarchy — which handoff skill applies when (reconcile, do not duplicate)

The mvp tree has several competing handoff skills (`web-session-handoff`, `session-output--investigation-handoff`, plus the `PATCH--web-session-handoff-discoveries` add-on). The universal discipline is a single clear hierarchy, not three independent authorities:

- **general / phase handoff** → this skill (`web-session-handoff`) is the default cross-session bridge.
- **investigation handoff** → `session-output--investigation-handoff-SKILL.md` for analysis/merge/diagnosis sessions (discoveries + rejected_claims focus).
- Profile add-ons (e.g. the discoveries patch) extend, they do not replace; the base rule "the brief is self-contained" always wins — a brief must not rely on an external discoveries patch to be understood.

### Gate line adapted to the mvp stack (Jest, not dotnet)

```
Tests:  N passing  (run: cd server && npx jest 2>&1 | grep "Tests:" | tail -1)
Gate:   cd server && npx jest → N passed, 0 failed   (and client / npx playwright test when relevant)
```

"Tests: N passing" is the Jest/Playwright count, not a `dotnet test` count.
