...
description: >
  ...
  and how to classify every document for export/import. This is what Codex
...
| Phase | Autonomy | What Codex Does | What Engine Does |
|-------|----------|----------------------|-----------------|
...
## Phase B Stage 2: Concept-based named checks (available after SESSION-P-3)

With `loop.handler` operational (SESSION-P-3), named checks can use AI sub-calls
in a loop pattern instead of provider-keyed regex (Z-3 Stage 1):

```
loop {
  body: ['ai-generate-check-evaluation']
  condition: "verdict !== UNCERTAIN"
  maxIterations: 2
}
```

This replaces the provider-keyed regex approach with:
- `ai-generate` sub-call: "Does this code implement [concept]?"
- Loop: if UNCERTAIN → re-evaluate with more context
- Termination: PASS or FAIL after max 2 iterations

When to migrate from Stage 1 (regex) to Stage 2 (concept checks):
- When a new provider is added and regex variants become hard to maintain
- When a check produces false positives on valid code from unexpected runtimes
- Wave 3+ when multiple stacks are generating simultaneously
