...
## Universal Bits (UUS G07) — three files, Authority Ledger, human-readable board, no machine-log-to-user
...
### Three-file rule + SESSION-0 Authority Requirements Ledger (universal, generic part only)

The portable core: **REFERENCE (for humans) / STATE.json (for navigation) / EXECUTION (for the executor) — never merged into one file.** The SESSION-0 plan-review gate is mandatory before SESSION-1, and it must extract an **Authority Requirements Ledger** (every explicit requirement from the active authority mapped to plan section + gate + artifact) before any execution. This generic FC machinery (format-checks + ledger + human-readable status + Concrete Repair Blueprint) ports across projects. The mvp EXECUTION Gate-B is `cd server && npx jest -t "..."` / `npx playwright test`; STATE is the project STATE-JSON.

### NOT portable: host-extension FC subset (FC-36 / FC-37 / FC-44)

The Full-Extension-Definition / Final-Execution-Review FC gates for **Claude / Cursor / Codex host extensions are core-product-specific** and are **not** universal. Do not import FC-36 / FC-37 / FC-44 (or the live-host extension-readiness gates) into mvp. Only the generic format/ledger/human-status part of the FC block ports.

### Human-Readable Execution Board (lead with meaning, machine labels after)

Every human-facing status must lead with a plain-language board (in the operator's working language) — and only then any machine label:

```text
Total <N> tasks; Done <n> (names); Now <current task + who is doing it>;
Remaining <n> (names); Result type <code | artifact | review | policy | evidence-only>;
Next action <next bounded step, or NONE>.
```

### No-Machine-Log-To-User

A bare machine log, JSON dump, packet id, validator name, or STATE code is **not** an acceptable human status by itself. Such labels may appear only **after** the plain-language board with meaning + counts. A status that is only `07.06 DONE` / `gate_passed=true` / `QSG-28` fails this gate. (mvp adaptation: status/error-code strings come from `DataProcessResult<T>` + Jest output, never from the core `OperationResult<T>` vocabulary.)
