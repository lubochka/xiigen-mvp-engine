/**
 * ai-gemini.integration.spec.ts — SESSION-3
 *
 * AI Provider integration tests — Gemini tier.
 *
 * Two-tier strategy (always runs, never skips):
 *   Tier 1 (default): MockAiProvider — no key required, zero API calls
 *   Tier 2 (upgrade): GeminiProvider with fetch-based clientFactory
 *                     — activated when GEMINI_API_KEY present in secrets
 *
 * Token budget: maxTokens: 200 for all Tier-2 calls.
 * Model: gemini-2.0-flash (or AI_TEST_MODEL_GEMINI from session env).
 */
...
const TENANT = 'gemini-test-tenant';
...
// ── Gemini fetch-based client factory (Tier 2) ────────
// Implements IGeminiClient using the Gemini REST API.
        ...
        generate_content: async (params: {
          ...
          if (params.config) {
            ...
            if (params.config.max_output_tokens !== undefined) {
              genConfig['maxOutputTokens'] = params.config.max_output_tokens;
            }
            ...
            if (params.config.system_instruction) {
              body['system_instruction'] = {
                parts: [{ text: params.config.system_instruction }],
              };
            }
          }
...
const MODEL = secrets.aiTestModelGemini; // gemini-2.0-flash

let activeProvider: IAiProvider;
let activeTier: 'mock' | 'gemini';
...
beforeAll(() => {
  ...
  if (AVAIL.GEMINI) {
    // Tier 2: real GeminiProvider via fetch factory
    const keyResolver = new TenantKeyResolver(cls);
    keyResolver.setSystemKey('google', secrets.geminiApiKey);
    activeProvider = new GeminiProvider(cls, keyResolver, makeFetchGeminiClient, {
      defaultModel: MODEL,
    });
    activeTier = 'gemini';
    console.log(`Tier 2: GeminiProvider active (model: ${MODEL}, maxTokens: ${MAX_TOKENS})`);
  } else {
    // Tier 1: MockAiProvider
    const mock = new MockAiProvider(cls, {
      modelId: `mock-${MODEL}`,
      defaultResponse: '{"status":"ok","message":"mock gemini response"}',
    ...
    mockRef = mock;
    activeTier = 'mock';
    console.log('Tier 1: MockAiProvider active (no GEMINI_API_KEY)');
  }
});
...
describe(`AI Provider — Gemini [active tier: ${AVAIL.GEMINI ? 'Tier2/real' : 'Tier1/mock'}]`, () => {
  ...
  it('tenantId is in scope during generation (CLS-bound context)', async () => {
    ...
    if (activeTier === 'mock' && mockRef) {
      ...
      // Real provider: TenantContext was in scope (no NO_TENANT error)
...
describe('Gemini — IAiProvider Contract Shape', () => {
