...
import { SocialNotificationSenderService } from './social-notification-sender.service';

describe('SocialNotificationSenderService (T94)', () => {
  let callOrder: string[];
  let mockDb: { storeDocument: jest.Mock };
  let mockQueue: { enqueue: jest.Mock };
  let service: SocialNotificationSenderService;
  ...
  beforeEach(() => {
    ...
    service = new SocialNotificationSenderService(mockDb as any, mockQueue as any);
  });
  ...
  it('1. Happy path — storeDocument + enqueue, notificationId returned', async () => {
    const result = await service.execute(baseInput);
    expect(result.isSuccess).toBe(true);
    expect(result.data?.notificationId).toMatch(/^sno-/);
  ...
  it('3. social.notification.sent payload contains notificationId, feedEntryId, userId', async () => {
    await service.execute(baseInput);
    const [event, payload] = mockQueue.enqueue.mock.calls[0];
    expect(event).toBe('social.notification.sent');
    expect(payload.notificationId).toMatch(/^sno-/);
    expect(payload.feedEntryId).toBe('sfe-1');
  ...
  it('5. Validation: missing feedEntryId → VALIDATION_FAILURE', async () => {
    const result = await service.execute({ ...baseInput, feedEntryId: '' });
  ...
  it('8. DNA-3: unexpected throw → SOCIAL_NOTIFICATION_SENDER_ERROR', async () => {
    mockDb.storeDocument.mockImplementationOnce(() => {
      throw new Error('unexpected');
    });
    const result = await service.execute(baseInput);
    expect(result.isSuccess).toBe(false);
    expect(result.errorCode).toBe('SOCIAL_NOTIFICATION_SENDER_ERROR');
  });
});
