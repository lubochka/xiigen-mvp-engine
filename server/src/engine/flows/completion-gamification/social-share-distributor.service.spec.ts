...
import { SocialShareDistributorService } from './social-share-distributor.service';

describe('SocialShareDistributorService (T91)', () => {
  let callOrder: string[];
  let mockDb: { storeDocument: jest.Mock };
  let mockQueue: { enqueue: jest.Mock };
  let service: SocialShareDistributorService;
  ...
  beforeEach(() => {
    ...
    service = new SocialShareDistributorService(mockDb as any, mockQueue as any);
  });
  ...
  it('1. Happy path — storeDocument + enqueue, distributionId returned', async () => {
    const result = await service.execute(baseInput);
    expect(result.isSuccess).toBe(true);
    expect(result.data?.distributionId).toMatch(/^ssd-/);
  ...
  it('3. social.share.distributed payload contains distributionId, shareIntentId, userId', async () => {
    await service.execute(baseInput);
    const [event, payload] = mockQueue.enqueue.mock.calls[0];
    expect(event).toBe('social.share.distributed');
    expect(payload.distributionId).toMatch(/^ssd-/);
    expect(payload.shareIntentId).toBe('ssi-1');
  ...
  it('5. Validation: missing shareIntentId → VALIDATION_FAILURE', async () => {
  ...
  it('8. DNA-3: unexpected throw → SOCIAL_SHARE_DISTRIBUTOR_ERROR', async () => {
    mockDb.storeDocument.mockImplementationOnce(() => {
      throw new Error('unexpected');
    });
    const result = await service.execute(baseInput);
    expect(result.isSuccess).toBe(false);
    expect(result.errorCode).toBe('SOCIAL_SHARE_DISTRIBUTOR_ERROR');
  });
});
