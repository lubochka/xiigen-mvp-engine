 ...
 */
@Injectable()
export class EmailVerificationService extends MicroserviceBase {
  constructor(
    ...
    super({
      descriptor: new ServiceDescriptor({
        serviceId: 'email-verification.service',
        serviceName: 'EmailVerificationService',
        flowId: 'FLOW-01',
      }),
    });
  }
  ...
  async initiateVerification(
    ...
    try {
      if (!input.memberId || !input.email) {
        return DataProcessResult.failure('VALIDATION_FAILURE', 'memberId and email are required');
      }
      return this.issueNewToken(input.memberId, input.email);
    } catch (err) {
  ...
  async verifyToken(input: VerifyTokenInput): Promise<DataProcessResult<{ verified: boolean }>> {
    try {
      ...
      const found = await this.database.searchDocuments(VERIFICATION_TOKENS_INDEX, {
      ...
      await this.database.storeDocument(
      ...
      await this.queueService.enqueue('VerificationCompleted', {
  ...
  async resendVerification(
    ...
    try {
      ...
      const recentAttempts = await this.database.searchDocuments(RESEND_ATTEMPTS_INDEX, {
        tenant_id: tenantId,
        member_id: input.memberId,
  ...
  private async issueNewToken(
    ...
    await this.queueService.enqueue('VerificationEmailRequested', {
      memberId,
      email,
    ...
    const scheduled = await this.schedulerService.scheduleDelayed(
      'token-expiry',
      tokenExpiryMs,
  ...
  private async supersedePreviousTokens(memberId: string): Promise<void> {
    const activeTokens = await this.database.searchDocuments(VERIFICATION_TOKENS_INDEX, {
      member_id: memberId,
      status: 'active',
    });
