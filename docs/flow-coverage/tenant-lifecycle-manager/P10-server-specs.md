# FLOW-30 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** Tenant Lifecycle Manager (`tenant-lifecycle-manager`)
**Classification:** ENGINE_INTERNAL
**P9 rows total:** 5
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-1011 … CF-1014
**Next available CF after this flow:** CF-1015

## Edge case specs

## EC-1: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1011`

### HTTP contract

```
method: POST
path: /api/dynamic/tenant-lifecycle-manager
...
### BFA rule
- `CF-1011` — enforcement at `server/src/engine-contracts/tenant-lifecycle-manager-bfa-rules.ts`
  (if file does not exist, create alongside `tenant-lifecycle-manager-contracts.ts`).
...
- Existence: `grep -n "CF-1011" server/src/engine-contracts/tenant-lifecycle-manager-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest tenant-lifecycle-manager --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-2: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1012`
...
### BFA rule
- `CF-1012` — enforcement at `server/src/engine-contracts/tenant-lifecycle-manager-bfa-rules.ts`
  (if file does not exist, create alongside `tenant-lifecycle-manager-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1012" server/src/engine-contracts/tenant-lifecycle-manager-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest tenant-lifecycle-manager --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-4: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-1013`
...
### BFA rule
- `CF-1013` — enforcement at `server/src/engine-contracts/tenant-lifecycle-manager-bfa-rules.ts`
  (if file does not exist, create alongside `tenant-lifecycle-manager-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1013" server/src/engine-contracts/tenant-lifecycle-manager-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest tenant-lifecycle-manager --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-5: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1014`
...
### BFA rule
- `CF-1014` — enforcement at `server/src/engine-contracts/tenant-lifecycle-manager-bfa-rules.ts`
  (if file does not exist, create alongside `tenant-lifecycle-manager-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1014" server/src/engine-contracts/tenant-lifecycle-manager-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest tenant-lifecycle-manager --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-1011\|CF-1014' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/tenant-lifecycle-manager/P9-edge-cases.md`
