# FLOW-19 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** Durable Sagas & Compliance (`durable-sagas-compliance`)
**Classification:** ADMIN_FACING
**P9 rows total:** 9
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-961 … CF-964
**Next available CF after this flow:** CF-965

## Edge case specs

## EC-5: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-961`

### HTTP contract

```
method: POST
path: /api/dynamic/durable-sagas-compliance
...
### BFA rule
- `CF-961` — enforcement at `server/src/engine-contracts/durable-sagas-compliance-bfa-rules.ts`
  (if file does not exist, create alongside `durable-sagas-compliance-contracts.ts`).
...
- Existence: `grep -n "CF-961" server/src/engine-contracts/durable-sagas-compliance-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest durable-sagas-compliance --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-6: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-962`
...
### BFA rule
- `CF-962` — enforcement at `server/src/engine-contracts/durable-sagas-compliance-bfa-rules.ts`
  (if file does not exist, create alongside `durable-sagas-compliance-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-962" server/src/engine-contracts/durable-sagas-compliance-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest durable-sagas-compliance --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-8: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-963`
...
### BFA rule
- `CF-963` — enforcement at `server/src/engine-contracts/durable-sagas-compliance-bfa-rules.ts`
  (if file does not exist, create alongside `durable-sagas-compliance-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-963" server/src/engine-contracts/durable-sagas-compliance-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest durable-sagas-compliance --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-9: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-964`
...
### BFA rule
- `CF-964` — enforcement at `server/src/engine-contracts/durable-sagas-compliance-bfa-rules.ts`
  (if file does not exist, create alongside `durable-sagas-compliance-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-964" server/src/engine-contracts/durable-sagas-compliance-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest durable-sagas-compliance --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-961\|CF-964' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/durable-sagas-compliance/P9-edge-cases.md`
