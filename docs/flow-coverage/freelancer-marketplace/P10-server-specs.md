# FLOW-17 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** Freelancer Marketplace (`freelancer-marketplace`)
**Classification:** TENANT_FACING
**P9 rows total:** 9
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-953 … CF-956
**Next available CF after this flow:** CF-957

## Edge case specs

## EC-5: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-953`

### HTTP contract

```
method: POST
path: /api/dynamic/freelancer-marketplace
...
### BFA rule
- `CF-953` — enforcement at `server/src/engine-contracts/freelancer-marketplace-bfa-rules.ts`
  (if file does not exist, create alongside `freelancer-marketplace-contracts.ts`).
...
- Existence: `grep -n "CF-953" server/src/engine-contracts/freelancer-marketplace-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest freelancer-marketplace --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-6: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-954`
...
### BFA rule
- `CF-954` — enforcement at `server/src/engine-contracts/freelancer-marketplace-bfa-rules.ts`
  (if file does not exist, create alongside `freelancer-marketplace-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-954" server/src/engine-contracts/freelancer-marketplace-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest freelancer-marketplace --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-8: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-955`
...
### BFA rule
- `CF-955` — enforcement at `server/src/engine-contracts/freelancer-marketplace-bfa-rules.ts`
  (if file does not exist, create alongside `freelancer-marketplace-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-955" server/src/engine-contracts/freelancer-marketplace-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest freelancer-marketplace --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-9: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-956`
...
### BFA rule
- `CF-956` — enforcement at `server/src/engine-contracts/freelancer-marketplace-bfa-rules.ts`
  (if file does not exist, create alongside `freelancer-marketplace-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-956" server/src/engine-contracts/freelancer-marketplace-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest freelancer-marketplace --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-953\|CF-956' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/freelancer-marketplace/P9-edge-cases.md`
