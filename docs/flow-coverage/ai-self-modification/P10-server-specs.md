# FLOW-44 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** AI Self-Modification (`ai-self-modification`)
**Classification:** ENGINE_INTERNAL
**P9 rows total:** 5
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-1074 … CF-1077
**Next available CF after this flow:** CF-1078

## Edge case specs

## EC-1: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1074`

### HTTP contract

```
method: POST
path: /api/dynamic/ai-self-modification
...
### BFA rule
- `CF-1074` — enforcement at `server/src/engine-contracts/ai-self-modification-bfa-rules.ts`
  (if file does not exist, create alongside `ai-self-modification-contracts.ts`).
...
- Existence: `grep -n "CF-1074" server/src/engine-contracts/ai-self-modification-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest ai-self-modification --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-2: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1075`
...
### BFA rule
- `CF-1075` — enforcement at `server/src/engine-contracts/ai-self-modification-bfa-rules.ts`
  (if file does not exist, create alongside `ai-self-modification-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1075" server/src/engine-contracts/ai-self-modification-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest ai-self-modification --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-4: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-1076`
...
### BFA rule
- `CF-1076` — enforcement at `server/src/engine-contracts/ai-self-modification-bfa-rules.ts`
  (if file does not exist, create alongside `ai-self-modification-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1076" server/src/engine-contracts/ai-self-modification-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest ai-self-modification --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-5: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1077`
...
### BFA rule
- `CF-1077` — enforcement at `server/src/engine-contracts/ai-self-modification-bfa-rules.ts`
  (if file does not exist, create alongside `ai-self-modification-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1077" server/src/engine-contracts/ai-self-modification-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest ai-self-modification --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-1074\|CF-1077' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/ai-self-modification/P9-edge-cases.md`
