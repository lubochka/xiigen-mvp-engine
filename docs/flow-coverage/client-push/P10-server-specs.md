# FLOW-40 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** Client Push (`client-push`)
**Classification:** ENGINE_INTERNAL
**P9 rows total:** 7
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-1062 … CF-1065
**Next available CF after this flow:** CF-1066

## Edge case specs

## EC-3: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1062`

### HTTP contract

```
method: POST
path: /api/dynamic/client-push
...
### BFA rule
- `CF-1062` — enforcement at `server/src/engine-contracts/client-push-bfa-rules.ts`
  (if file does not exist, create alongside `client-push-contracts.ts`).
...
- Existence: `grep -n "CF-1062" server/src/engine-contracts/client-push-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest client-push --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-4: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1063`
...
### BFA rule
- `CF-1063` — enforcement at `server/src/engine-contracts/client-push-bfa-rules.ts`
  (if file does not exist, create alongside `client-push-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1063" server/src/engine-contracts/client-push-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest client-push --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-6: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-1064`
...
### BFA rule
- `CF-1064` — enforcement at `server/src/engine-contracts/client-push-bfa-rules.ts`
  (if file does not exist, create alongside `client-push-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1064" server/src/engine-contracts/client-push-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest client-push --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-7: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1065`
...
### BFA rule
- `CF-1065` — enforcement at `server/src/engine-contracts/client-push-bfa-rules.ts`
  (if file does not exist, create alongside `client-push-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1065" server/src/engine-contracts/client-push-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest client-push --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-1062\|CF-1065' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/client-push/P9-edge-cases.md`
