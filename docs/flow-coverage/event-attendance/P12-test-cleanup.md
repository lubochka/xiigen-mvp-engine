# FLOW-04 Test Coverage Cleanup — Phase 12
...
|--------|------|------:|------:|----------:|------:|----------:|------------:|
| client/e2e | `client/e2e/event-attendance-registration.spec.ts` | 137 | 0 | 0 | 0 | 0 | 0 |
| client/e2e | `client/e2e/event-attendance.spec.ts` | 236 | 0 | 0 | 0 | 0 | 0 |
| client/e2e | `client/e2e/event-management-event-attendance-teaching-pipeline.spec.ts` | 330 | 19 | 0 | 0 | 0 | 0 |
| e2e/tests (full-stack) | `e2e/tests/flow04-event-attendance.spec.ts` | 403 | 0 | 0 | 0 | 0 | 0 |
| server/test | `server/test/e2e/event-attendance/event-attendance.e2e.spec.ts` | 726 | 0 | 0 | 0 | 0 | 0 |

## Arbiters

- **Stub free:** `.todo`=0, `.skip` (unconditional)=19, `xit`/`xtest`=0. Conditional skips (0) accepted (server-readiness gate).
- **Duplicate:** ❌ — candidates:
  - `e2e/tests/flow04-event-attendance.spec.ts` — full-stack integration spec. Full-stack and client-mock serve different purposes; merge decision requires review.
- **No false greens:** `expect(true).toBe(true)` = 0.
- **Test gate:** `npx jest event-attendance --no-coverage` → failures === 0 required. Server jest baseline ≥ 10,617.
