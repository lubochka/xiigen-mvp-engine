# FLOW-28 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** Blog/CMS Modules (`blog-cms-modules`)
**Classification:** TENANT_FACING
**P9 rows total:** 8
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-1003 … CF-1006
**Next available CF after this flow:** CF-1007

## Edge case specs

## EC-4: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1003`

### HTTP contract

```
method: POST
path: /api/dynamic/blog-cms-modules
...
### BFA rule
- `CF-1003` — enforcement at `server/src/engine-contracts/blog-cms-modules-bfa-rules.ts`
  (if file does not exist, create alongside `blog-cms-modules-contracts.ts`).
...
- Existence: `grep -n "CF-1003" server/src/engine-contracts/blog-cms-modules-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest blog-cms-modules --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-5: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1004`
...
### BFA rule
- `CF-1004` — enforcement at `server/src/engine-contracts/blog-cms-modules-bfa-rules.ts`
  (if file does not exist, create alongside `blog-cms-modules-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1004" server/src/engine-contracts/blog-cms-modules-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest blog-cms-modules --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-7: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-1005`
...
### BFA rule
- `CF-1005` — enforcement at `server/src/engine-contracts/blog-cms-modules-bfa-rules.ts`
  (if file does not exist, create alongside `blog-cms-modules-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1005" server/src/engine-contracts/blog-cms-modules-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest blog-cms-modules --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-8: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1006`
...
### BFA rule
- `CF-1006` — enforcement at `server/src/engine-contracts/blog-cms-modules-bfa-rules.ts`
  (if file does not exist, create alongside `blog-cms-modules-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1006" server/src/engine-contracts/blog-cms-modules-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest blog-cms-modules --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-1003\|CF-1006' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/blog-cms-modules/P9-edge-cases.md`
