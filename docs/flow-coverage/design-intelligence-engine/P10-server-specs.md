# FLOW-31 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** Design Intelligence Engine (`design-intelligence-engine`)
**Classification:** ENGINE_INTERNAL
**P9 rows total:** 5
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-1015 … CF-1018
**Next available CF after this flow:** CF-1019

## Edge case specs

## EC-1: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1015`

### HTTP contract

```
method: POST
path: /api/dynamic/design-intelligence-engine
...
### BFA rule
- `CF-1015` — enforcement at `server/src/engine-contracts/design-intelligence-engine-bfa-rules.ts`
  (if file does not exist, create alongside `design-intelligence-engine-contracts.ts`).
...
- Existence: `grep -n "CF-1015" server/src/engine-contracts/design-intelligence-engine-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest design-intelligence-engine --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-2: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1016`
...
### BFA rule
- `CF-1016` — enforcement at `server/src/engine-contracts/design-intelligence-engine-bfa-rules.ts`
  (if file does not exist, create alongside `design-intelligence-engine-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1016" server/src/engine-contracts/design-intelligence-engine-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest design-intelligence-engine --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-4: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-1017`
...
### BFA rule
- `CF-1017` — enforcement at `server/src/engine-contracts/design-intelligence-engine-bfa-rules.ts`
  (if file does not exist, create alongside `design-intelligence-engine-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1017" server/src/engine-contracts/design-intelligence-engine-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest design-intelligence-engine --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-5: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1018`
...
### BFA rule
- `CF-1018` — enforcement at `server/src/engine-contracts/design-intelligence-engine-bfa-rules.ts`
  (if file does not exist, create alongside `design-intelligence-engine-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1018" server/src/engine-contracts/design-intelligence-engine-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest design-intelligence-engine --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-1015\|CF-1018' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/design-intelligence-engine/P9-edge-cases.md`
