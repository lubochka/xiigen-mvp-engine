# FLOW-38 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** RAG Quality Feedback (`rag-quality-feedback`)
**Classification:** ENGINE_INTERNAL
**P9 rows total:** 5
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-1054 … CF-1057
**Next available CF after this flow:** CF-1058

## Edge case specs

## EC-1: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1054`

### HTTP contract

```
method: POST
path: /api/dynamic/rag-quality-feedback
...
### BFA rule
- `CF-1054` — enforcement at `server/src/engine-contracts/rag-quality-feedback-bfa-rules.ts`
  (if file does not exist, create alongside `rag-quality-feedback-contracts.ts`).
...
- Existence: `grep -n "CF-1054" server/src/engine-contracts/rag-quality-feedback-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest rag-quality-feedback --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-2: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1055`
...
### BFA rule
- `CF-1055` — enforcement at `server/src/engine-contracts/rag-quality-feedback-bfa-rules.ts`
  (if file does not exist, create alongside `rag-quality-feedback-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1055" server/src/engine-contracts/rag-quality-feedback-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest rag-quality-feedback --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-4: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-1056`
...
### BFA rule
- `CF-1056` — enforcement at `server/src/engine-contracts/rag-quality-feedback-bfa-rules.ts`
  (if file does not exist, create alongside `rag-quality-feedback-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1056" server/src/engine-contracts/rag-quality-feedback-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest rag-quality-feedback --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-5: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1057`
...
### BFA rule
- `CF-1057` — enforcement at `server/src/engine-contracts/rag-quality-feedback-bfa-rules.ts`
  (if file does not exist, create alongside `rag-quality-feedback-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1057" server/src/engine-contracts/rag-quality-feedback-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest rag-quality-feedback --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-1054\|CF-1057' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/rag-quality-feedback/P9-edge-cases.md`
