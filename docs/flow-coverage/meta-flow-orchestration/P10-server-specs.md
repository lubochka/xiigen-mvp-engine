# FLOW-43 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** Meta Flow Orchestration (`meta-flow-orchestration`)
**Classification:** ENGINE_INTERNAL
**P9 rows total:** 5
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-1070 … CF-1073
**Next available CF after this flow:** CF-1074

## Edge case specs

## EC-1: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1070`

### HTTP contract

```
method: POST
path: /api/dynamic/meta-flow-orchestration
...
### BFA rule
- `CF-1070` — enforcement at `server/src/engine-contracts/meta-flow-orchestration-bfa-rules.ts`
  (if file does not exist, create alongside `meta-flow-orchestration-contracts.ts`).
...
- Existence: `grep -n "CF-1070" server/src/engine-contracts/meta-flow-orchestration-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest meta-flow-orchestration --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-2: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1071`
...
### BFA rule
- `CF-1071` — enforcement at `server/src/engine-contracts/meta-flow-orchestration-bfa-rules.ts`
  (if file does not exist, create alongside `meta-flow-orchestration-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1071" server/src/engine-contracts/meta-flow-orchestration-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest meta-flow-orchestration --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-4: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-1072`
...
### BFA rule
- `CF-1072` — enforcement at `server/src/engine-contracts/meta-flow-orchestration-bfa-rules.ts`
  (if file does not exist, create alongside `meta-flow-orchestration-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1072" server/src/engine-contracts/meta-flow-orchestration-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest meta-flow-orchestration --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-5: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1073`
...
### BFA rule
- `CF-1073` — enforcement at `server/src/engine-contracts/meta-flow-orchestration-bfa-rules.ts`
  (if file does not exist, create alongside `meta-flow-orchestration-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1073" server/src/engine-contracts/meta-flow-orchestration-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest meta-flow-orchestration --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-1070\|CF-1073' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/meta-flow-orchestration/P9-edge-cases.md`
