# FLOW-29 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** Adaptive RAG / Deep Research (`adaptive-rag-deep-research`)
**Classification:** ENGINE_INTERNAL
**P9 rows total:** 5
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-1007 … CF-1010
**Next available CF after this flow:** CF-1011

## Edge case specs

## EC-1: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1007`

### HTTP contract

```
method: POST
path: /api/dynamic/adaptive-rag-deep-research
...
### BFA rule
- `CF-1007` — enforcement at `server/src/engine-contracts/adaptive-rag-deep-research-bfa-rules.ts`
  (if file does not exist, create alongside `adaptive-rag-deep-research-contracts.ts`).
...
- Existence: `grep -n "CF-1007" server/src/engine-contracts/adaptive-rag-deep-research-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest adaptive-rag-deep-research --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-2: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1008`
...
### BFA rule
- `CF-1008` — enforcement at `server/src/engine-contracts/adaptive-rag-deep-research-bfa-rules.ts`
  (if file does not exist, create alongside `adaptive-rag-deep-research-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1008" server/src/engine-contracts/adaptive-rag-deep-research-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest adaptive-rag-deep-research --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-4: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-1009`
...
### BFA rule
- `CF-1009` — enforcement at `server/src/engine-contracts/adaptive-rag-deep-research-bfa-rules.ts`
  (if file does not exist, create alongside `adaptive-rag-deep-research-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1009" server/src/engine-contracts/adaptive-rag-deep-research-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest adaptive-rag-deep-research --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-5: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1010`
...
### BFA rule
- `CF-1010` — enforcement at `server/src/engine-contracts/adaptive-rag-deep-research-bfa-rules.ts`
  (if file does not exist, create alongside `adaptive-rag-deep-research-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1010" server/src/engine-contracts/adaptive-rag-deep-research-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest adaptive-rag-deep-research --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-1007\|CF-1010' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/adaptive-rag-deep-research/P9-edge-cases.md`
