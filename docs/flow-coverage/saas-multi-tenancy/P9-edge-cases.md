# FLOW-15 Edge Cases — Phase 9 Deliverable
...
|--:|-----------|----------|------|------------------|---------|
| 1 | CF-15-1 rule violation: T605 atomic bootstrap (SETNX + synchronous FREEDOM seed + TenantProvisioningFailed) | HIGH | SERVER_REQUIRED | Rule enforced by BFA; violation produces BUILD_FAILURE at ship time. | `CF-15-1` |
| 2 | CF-15-2 rule violation: T606 MACHINE_LOCKED_KEYS compile-time constant + OCC write | HIGH | SERVER_REQUIRED | Rule enforced by BFA; violation produces BUILD_FAILURE at ship time. | `CF-15-2` |
| 3 | CF-15-3 rule violation: T607 Redis MULTI/EXEC atomicity + T608 suspend-not-delete + cascade | HIGH | SERVER_REQUIRED | Rule enforced by BFA; violation produces BUILD_FAILURE at ship time. | `CF-15-3` |
| 4 | CF-15-4 rule violation: scope_isolation arbiter FC-32 compliance across all 4 services | HIGH | SERVER_REQUIRED | Rule enforced by BFA; violation produces BUILD_FAILURE at ship time. | `CF-15-4` |
...
- **Iron rule coverage:** 4 CF rules in `server/src/engine-contracts/saas-multi-tenancy*.ts` → 4 edge case rows in this doc. ✅ PASS.
...
## Inputs
- P1 states from `docs/flow-coverage/saas-multi-tenancy/P1-business-logic-inventory.md` (6 items)
- CF rules from `server/src/engine-contracts/saas-multi-tenancy*.ts` (4 rules)
