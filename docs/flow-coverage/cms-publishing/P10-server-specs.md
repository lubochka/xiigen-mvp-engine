# FLOW-22 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** CMS Publishing (`cms-publishing`)
**Classification:** TENANT_FACING
**P9 rows total:** 11
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-973 … CF-976
**Next available CF after this flow:** CF-977

## Edge case specs

## EC-7: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-973`

### HTTP contract

```
method: POST
path: /api/dynamic/cms-publishing
...
### BFA rule
- `CF-973` — enforcement at `server/src/engine-contracts/cms-publishing-bfa-rules.ts`
  (if file does not exist, create alongside `cms-publishing-contracts.ts`).
...
- Existence: `grep -n "CF-973" server/src/engine-contracts/cms-publishing-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest cms-publishing --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-8: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-974`
...
### BFA rule
- `CF-974` — enforcement at `server/src/engine-contracts/cms-publishing-bfa-rules.ts`
  (if file does not exist, create alongside `cms-publishing-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-974" server/src/engine-contracts/cms-publishing-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest cms-publishing --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-10: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-975`
...
### BFA rule
- `CF-975` — enforcement at `server/src/engine-contracts/cms-publishing-bfa-rules.ts`
  (if file does not exist, create alongside `cms-publishing-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-975" server/src/engine-contracts/cms-publishing-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest cms-publishing --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-11: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-976`
...
### BFA rule
- `CF-976` — enforcement at `server/src/engine-contracts/cms-publishing-bfa-rules.ts`
  (if file does not exist, create alongside `cms-publishing-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-976" server/src/engine-contracts/cms-publishing-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest cms-publishing --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-973\|CF-976' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/cms-publishing/P9-edge-cases.md`
