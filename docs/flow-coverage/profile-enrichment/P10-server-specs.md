# FLOW-02 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** Profile Enrichment (`profile-enrichment`)
**Classification:** TENANT_FACING
**P9 rows total:** 9
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-854 … CF-857
**Next available CF after this flow:** CF-858

## Edge case specs

## EC-5: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-854`

### HTTP contract

```
method: POST
path: /api/dynamic/profile-enrichment
...
### BFA rule
- `CF-854` — enforcement at `server/src/engine-contracts/profile-enrichment-bfa-rules.ts`
  (if file does not exist, create alongside `profile-enrichment-contracts.ts`).
...
- Existence: `grep -n "CF-854" server/src/engine-contracts/profile-enrichment-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest profile-enrichment --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-6: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-855`
...
### BFA rule
- `CF-855` — enforcement at `server/src/engine-contracts/profile-enrichment-bfa-rules.ts`
  (if file does not exist, create alongside `profile-enrichment-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-855" server/src/engine-contracts/profile-enrichment-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest profile-enrichment --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-8: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-856`
...
### BFA rule
- `CF-856` — enforcement at `server/src/engine-contracts/profile-enrichment-bfa-rules.ts`
  (if file does not exist, create alongside `profile-enrichment-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-856" server/src/engine-contracts/profile-enrichment-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest profile-enrichment --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-9: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-857`
...
### BFA rule
- `CF-857` — enforcement at `server/src/engine-contracts/profile-enrichment-bfa-rules.ts`
  (if file does not exist, create alongside `profile-enrichment-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-857" server/src/engine-contracts/profile-enrichment-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest profile-enrichment --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-854\|CF-857' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/profile-enrichment/P9-edge-cases.md`
