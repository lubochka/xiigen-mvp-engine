# FLOW-20 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** Ads Platform (`ads-platform`)
**Classification:** ADMIN_FACING
**P9 rows total:** 9
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-965 … CF-968
**Next available CF after this flow:** CF-969

## Edge case specs

## EC-5: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-965`

### HTTP contract

```
method: POST
path: /api/dynamic/ads-platform
...
### BFA rule
- `CF-965` — enforcement at `server/src/engine-contracts/ads-platform-bfa-rules.ts`
  (if file does not exist, create alongside `ads-platform-contracts.ts`).
...
- Existence: `grep -n "CF-965" server/src/engine-contracts/ads-platform-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest ads-platform --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-6: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-966`
...
### BFA rule
- `CF-966` — enforcement at `server/src/engine-contracts/ads-platform-bfa-rules.ts`
  (if file does not exist, create alongside `ads-platform-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-966" server/src/engine-contracts/ads-platform-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest ads-platform --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-8: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-967`
...
### BFA rule
- `CF-967` — enforcement at `server/src/engine-contracts/ads-platform-bfa-rules.ts`
  (if file does not exist, create alongside `ads-platform-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-967" server/src/engine-contracts/ads-platform-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest ads-platform --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-9: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-968`
...
### BFA rule
- `CF-968` — enforcement at `server/src/engine-contracts/ads-platform-bfa-rules.ts`
  (if file does not exist, create alongside `ads-platform-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-968" server/src/engine-contracts/ads-platform-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest ads-platform --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-965\|CF-968' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/ads-platform/P9-edge-cases.md`
