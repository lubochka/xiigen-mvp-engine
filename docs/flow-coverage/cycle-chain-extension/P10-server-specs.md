# FLOW-45 Server-Side Edge Case Specs — Phase 10 Deliverable

**Flow:** Cycle Chain Extension (`cycle-chain-extension`)
**Classification:** ENGINE_INTERNAL
**P9 rows total:** 5
**SERVER_REQUIRED rows without prior CF:** 4
**CF consumed:** CF-1078 … CF-1081
**Next available CF after this flow:** CF-1082

## Edge case specs

## EC-1: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1078`

### HTTP contract

```
method: POST
path: /api/dynamic/cycle-chain-extension
...
### BFA rule
- `CF-1078` — enforcement at `server/src/engine-contracts/cycle-chain-extension-bfa-rules.ts`
  (if file does not exist, create alongside `cycle-chain-extension-contracts.ts`).
...
- Existence: `grep -n "CF-1078" server/src/engine-contracts/cycle-chain-extension-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest cycle-chain-extension --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-2: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1079`
...
### BFA rule
- `CF-1079` — enforcement at `server/src/engine-contracts/cycle-chain-extension-bfa-rules.ts`
  (if file does not exist, create alongside `cycle-chain-extension-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1079" server/src/engine-contracts/cycle-chain-extension-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest cycle-chain-extension --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-4: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-1080`
...
### BFA rule
- `CF-1080` — enforcement at `server/src/engine-contracts/cycle-chain-extension-bfa-rules.ts`
  (if file does not exist, create alongside `cycle-chain-extension-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1080" server/src/engine-contracts/cycle-chain-extension-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest cycle-chain-extension --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-5: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1081`
...
### BFA rule
- `CF-1081` — enforcement at `server/src/engine-contracts/cycle-chain-extension-bfa-rules.ts`
  (if file does not exist, create alongside `cycle-chain-extension-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1081" server/src/engine-contracts/cycle-chain-extension-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest cycle-chain-extension --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-1078\|CF-1081' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/cycle-chain-extension/P9-edge-cases.md`
