# FLOW-21 Edge Cases — Phase 9 Deliverable
...
|--:|-----------|----------|------|------------------|---------|
| 1 | CF-21-1 rule violation: T629 FormSchemaPublisher — OCC schema publishing with immutable published state | HIGH | SERVER_REQUIRED | Rule enforced by BFA; violation produces BUILD_FAILURE at ship time. | `CF-21-1` |
| 2 | CF-21-2 rule violation: T630 FormSubmissionProcessor — BOLA check + validation against published schema | HIGH | SERVER_REQUIRED | Rule enforced by BFA; violation produces BUILD_FAILURE at ship time. | `CF-21-2` |
| 3 | CF-21-3 rule violation: T631 AutomationDispatcher — SETNX rule locks + conditional branching | HIGH | SERVER_REQUIRED | Rule enforced by BFA; violation produces BUILD_FAILURE at ship time. | `CF-21-3` |
| 4 | CF-21-4 rule violation: T632 SubmissionAnalyticsCollector — append-only analytics with PII exclusion | HIGH | SERVER_REQUIRED | Rule enforced by BFA; violation produces BUILD_FAILURE at ship time. | `CF-21-4` |
...
- **Iron rule coverage:** 4 CF rules in `server/src/engine-contracts/dynamic-forms-workflows*.ts` → 4 edge case rows in this doc. ✅ PASS.
...
## Inputs
- P1 states from `docs/flow-coverage/dynamic-forms-workflows/P1-business-logic-inventory.md` (6 items)
- CF rules from `server/src/engine-contracts/dynamic-forms-workflows*.ts` (4 rules)
