# FLOW-34 Server-Side Edge Case Specs — Phase 10 Deliverable
...
**CF consumed:** CF-1027 … CF-1030
**Next available CF after this flow:** CF-1031

## Edge case specs

## EC-1: Concurrent write on same resource (two clients simultaneously)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1027`

### HTTP contract

```
method: POST
path: /api/dynamic/marketplace-plugin-adapter
...
### BFA rule
- `CF-1027` — enforcement at `server/src/engine-contracts/marketplace-plugin-adapter-bfa-rules.ts`
  (if file does not exist, create alongside `marketplace-plugin-adapter-contracts.ts`).
...
- Existence: `grep -n "CF-1027" server/src/engine-contracts/marketplace-plugin-adapter-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest marketplace-plugin-adapter --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-2: Request retried 3× with same idempotency key

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1028`
...
### BFA rule
- `CF-1028` — enforcement at `server/src/engine-contracts/marketplace-plugin-adapter-bfa-rules.ts`
  (if file does not exist, create alongside `marketplace-plugin-adapter-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1028" server/src/engine-contracts/marketplace-plugin-adapter-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest marketplace-plugin-adapter --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-4: Boundary value: empty / null / zero input to primary field

**Severity:** MEDIUM &nbsp;•&nbsp; **BFA rule:** `CF-1029`
...
### BFA rule
- `CF-1029` — enforcement at `server/src/engine-contracts/marketplace-plugin-adapter-bfa-rules.ts`
  (if file does not exist, create alongside `marketplace-plugin-adapter-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1029" server/src/engine-contracts/marketplace-plugin-adapter-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest marketplace-plugin-adapter --no-coverage` → failures === 0; server jest ≥ 10,617.

## EC-5: Timeout / partial failure from downstream fabric (DB or queue)

**Severity:** HIGH &nbsp;•&nbsp; **BFA rule:** `CF-1030`
...
### BFA rule
- `CF-1030` — enforcement at `server/src/engine-contracts/marketplace-plugin-adapter-bfa-rules.ts`
  (if file does not exist, create alongside `marketplace-plugin-adapter-contracts.ts`).
- Violation severity: BUILD_FAILURE (blocks ship via BFA gate).
...
### Test oracle
- Concurrency/retry: fire 2 calls simultaneously → expected split (201 + error code).
- Existence: `grep -n "CF-1030" server/src/engine-contracts/marketplace-plugin-adapter-bfa-rules.ts` → ≥1 hit.
- Zero regression: `npx jest marketplace-plugin-adapter --no-coverage` → failures === 0; server jest ≥ 10,617.
...
- **CF assignment:** sequential, no collision. Declared range above. Verify with `grep -n 'CF-1027\|CF-1030' server/src/engine-contracts/` → existing hits = 0 before P11.
- **No code:** behavior only. No TypeScript. No class names (until P11).

## Inputs
- P9 rows from `docs/flow-coverage/marketplace-plugin-adapter/P9-edge-cases.md`
