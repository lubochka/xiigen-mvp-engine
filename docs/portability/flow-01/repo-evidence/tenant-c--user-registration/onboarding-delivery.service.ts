...
export class OnboardingDeliveryService {
  ...
  async checkCompletionGate(
    ...
    try {
      ...
      const deliveryRecords = await this.db.searchDocuments(ONBOARDING_DELIVERIES_INDEX, {
      ...
      await this.queue.enqueue(ONBOARDING_COMPLETED_EVENT, cloudEvent);
  ...
  private async storeDelivery(params: {
    ...
    try {
      ...
      const stored = await this.db.storeDocument(ONBOARDING_DELIVERIES_INDEX, doc, deliveryId);
      if (!stored.isSuccess) {
        ...
        await this.db.storeDocument(ONBOARDING_DELIVERIES_INDEX, failedDoc, deliveryId);
        return DataProcessResult.failure(stored.errorCode!, stored.errorMessage!);
      }
