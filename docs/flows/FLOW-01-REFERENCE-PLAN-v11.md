# FLOW-01: USER REGISTRATION & ONBOARDING — REFERENCE PLAN v11
...
```bash
# Submit each task type to AF pipeline
for taskType in T47 T48 T49; do
  ...
  console.log('${taskType}:', result.passed ? '✅' : '❌',
    'score:', result.score, 'promotion:', result.promotionLevel);
  if (!result.passed) throw new Error('${taskType} AF pipeline FAILED');
  "
done
...
Test baseline:
  ...
  After Phase E:   ≥ 4,096 (+30 server tests from AF-generated code)
