...
## REFERENCE PLAN v3 — DO NOT EXECUTE
## Date: 2026-03-22 (updated from v2)
...
|-------------|-------------|
| Phase A gate: `SKILL-REGISTRATION-MANIFEST-v3.md` | Phase A gate: `SKILL-REGISTRATION-MANIFEST-v4.md` |
| Phase A gate: no lint gate | Phase A gate: `npm run lint:naming exits 0` + FC-16/17/18 checks |
| engine-contracts file: `meta-arbitration-contracts.ts` | Canonical file: `meta-arbitration-engine-contracts.ts` (D-NAMING-1 Rule 1) |
| engine/flows branch: `flow/meta-arbitration/*` | Canonical branches: `flow/meta-arbitration-engine/*` (D-NAMING-1 Rule 2) |
| flow-lifecycle doc: no flowName field | flow-lifecycle doc includes `"flowName": "Meta-Arbitration Engine"` (SK-430 Rule 3) |
| EngineContracts T565/T566: no flowId/flowName | EngineContract params include `flowId: 'FLOW-35', flowName: 'Meta-Arbitration Engine'` (SK-430 Rule 3) |
| STATE.json: no flow_name field | STATE.json template includes `"flow_name": "Meta-Arbitration Engine"` (SK-430 Rule 4) |
| Constants: `FLOW35_*` prefix | Constants: `META_ARBITRATION_ENGINE_*` prefix (SK-430 Rule 6) |
| Jira comments: no format requirement | Jira comments follow SK-430 Rule 5 (5-section business context format) |
| SK-430: not registered in Phase A | SK-430 is NOT registered here — registered in FLOW-00.1 Phase D |

All previous v2 content (visibility plan additions, flow-lifecycle index, FlowMatrixRunner,
cross-flow edge stubs, engine health dashboard) is unchanged.
...
## CANONICAL NAMING (D-NAMING-1) — SK-430 Rules 1, 2 & 6

```
engine-contracts/ file:   meta-arbitration-engine-contracts.ts     ← Rule 1
                          meta-arbitration-engine-seed-prompts.ts  ← Rule 1
engine/flows/ directory:  engine/flows/meta-arbitration-engine/    ← Rule 2
Branch prefix:            flow/meta-arbitration-engine/            ← Rule 2

flowId:   'FLOW-35'
flowName: 'Meta-Arbitration Engine'

Constants prefix:  META_ARBITRATION_ENGINE_*                        ← Rule 6
  e.g.:  META_ARBITRATION_ENGINE_QUALITY_GATES
         META_ARBITRATION_ENGINE_IRON_RULES
         META_ARBITRATION_ENGINE_AF_STATIONS
```

**Authoritative table:** DECISIONS-LOCKED.md → D-NAMING-1.
NEVER use `flow35-*`, `meta-arbitration-*` without `-engine`, or `FLOW35_*` prefixes.

⚠️ **Note on existing file:** The file `meta-arbitration-contracts.ts` created in
SESSION-NAMING-REFACTOR (FLOW-00.1 interim) will be renamed to
`meta-arbitration-engine-contracts.ts` in FLOW-00.1 Phase B (canonical alignment pass).
Until then, both names are in use. New contracts always target the canonical name.
...
| Skill | SK-430 | NamingConventionsEnforcer | Registered in FLOW-00.1 Phase D — NOT Phase A |
...
**Branch:** `flow/meta-arbitration-engine/phase-a`
...
- Create EngineContracts: T565 (RoundSummaryProcessor), T566 (MetaDecisionEngine)
    File: `server/src/engine-contracts/meta-arbitration-engine-contracts.ts`  ← D-NAMING-1 Rule 1
    Each contract MUST include (SK-430 Rule 3):
    ```typescript
    taskTypeId: 'T565',
    taskTypeName: 'RoundSummaryProcessor',
    flowId: 'FLOW-35',
    flowName: 'Meta-Arbitration Engine',
    ```
- Seed genesis prompts to AF-3
    File: `server/src/engine-contracts/meta-arbitration-engine-seed-prompts.ts`  ← D-NAMING-1 Rule 1
...
- Create FLOW-35-STATE.json (includes flow_name — SK-430 Rule 4):
    ```json
    {
      "flow_id": "FLOW-35",
...
Document structure (updated v3 — adds flowName per SK-430 Rule 3):
```json
{
  "flowId": "FLOW-01",
  "flowName": "User Registration",                    ← NEW v3 — SK-430 Rule 3
...
Seed 1 — Retroactive (infra flows already complete, seeded as ACTIVE):
  ...
  status: ACTIVE
  flowName: per D-NAMING-1 table (e.g. FLOW-25 → "BFA Conflict Detection")  ← NEW v3
...
Seed 2 — Forward (user flows not yet generated, seeded as NOT_STARTED):
  ...
  status: NOT_STARTED
  flowName: per D-NAMING-1 where known; blank for FLOW-01..24 (populated during generation)
  generationHistory: []
...
□ 2 EngineContracts (T565, T566) in meta-arbitration-engine-contracts.ts         [NEW v3 — D-NAMING-1]
□ T565 EngineContract has flowId: 'FLOW-35', flowName: 'Meta-Arbitration Engine' [NEW v3 — SK-430 Rule 3]
□ T566 EngineContract has flowId: 'FLOW-35', flowName: 'Meta-Arbitration Engine' [NEW v3 — SK-430 Rule 3]
...
□ [NEW — skills package v4] Register SK-434, SK-435, SK-436 in SkillGraphService
  alongside SK-416–425 registration.
  Use registration commands from SKILL-REGISTRATION-MANIFEST-v4.md.             [NEW v3 — was v3]
  ...
  NOTE: SK-430 NamingConventionsEnforcer is NOT registered in Phase A.
        It is registered in FLOW-00.1 Phase D (after FLOW-00 completes).        [NEW v3]
□ Data structure schemas valid (JSON Schema Draft 7)
□ [NEW v2] flow-lifecycle index created with correct mapping
□ [NEW v3] flow-lifecycle document structure includes flowName field              [NEW v3 — SK-430 Rule 3]
□ [NEW v2] Seed 1: 7 infra flows seeded as ACTIVE (with flowName per D-NAMING-1) [NEW v3]
...
□ FLOW-35-STATE.json includes "flow_name": "Meta-Arbitration Engine"            [NEW v3 — SK-430 Rule 4]
□ FC-16: All proposed service files use domain names (meta-arbitration-engine-*, not flow35-*)  [NEW v3]
□ FC-17: EngineContract params include flowId + flowName for T565 and T566      [NEW v3]
□ FC-18: STATE.json template includes flow_name field                           [NEW v3]
□ npm run lint:naming exits 0                                                    [NEW v3 — SK-430 CI gate]
...
**[NEW v3] lint:naming gate for Phase D and Phase E:**
```
□ npm run lint:naming exits 0   (Phase D gate)
□ npm run lint:naming exits 0   (Phase E gate)
```
...
**Branch:** `flow/meta-arbitration-engine/integration-wiring`                    ← [NEW v3 — D-NAMING-1]
...
New in v3:
□ [NEW v3] npm run lint:naming exits 0                                           [SK-430 CI gate]
□ [NEW v3] Phase F Jira comment follows SK-430 Rule 5 (5 sections present)
□ [NEW v3] Branch name is flow/meta-arbitration-engine/integration-wiring        [D-NAMING-1]
...
**[NEW v3] Branch naming for Phases G, H, I:**
```
Phase G: flow/meta-arbitration-engine/end-to-end
Phase H: flow/meta-arbitration-engine/self-modification
Phase I: flow/meta-arbitration-engine/session-output
```

**[NEW v3] Jira comment format for all remaining phases (SK-430 Rule 5):**
All Jira comments written by SK-429 after FLOW-35 phases must follow:
```
## What was built — Phase {X} [Flow: FLOW-35 — Meta-Arbitration Engine]

### Business purpose
[2–3 sentences: what problem this solves, who uses it, when it runs]

### Flow context
- **Flow:** FLOW-35 — Meta-Arbitration Engine
- **Task types implemented:** T565 RoundSummaryProcessor, T566 MetaDecisionEngine
...
FLOW-35 STATE.json template (all phases):
  {
    "flow_id": "FLOW-35",
    "flow_name": "Meta-Arbitration Engine",   ← REQUIRED (SK-430 Rule 4)
    "current_phase": "phase-X",
    ...
  }

Naming decisions (DECISIONS-LOCKED.md → D-NAMING-1):
  Domain name (files/dirs): meta-arbitration-engine
  Human name (flowName):    Meta-Arbitration Engine
  Constants prefix:         META_ARBITRATION_ENGINE_

SK-430 registration: FLOW-00.1 Phase D (NOT this flow — runs after FLOW-00 completes)
...
```
FLOW-35-STATE.json              ← must include "flow_name": "Meta-Arbitration Engine"
SESSION-FLOW-35-A.md    ← enum (2) + 2 contracts (meta-arbitration-engine-contracts.ts)
                           + 8 factories (F1484–F1491) + 14 SK stubs
                           + flow-lifecycle index + two seed operations [AMENDED v2]
                           + flowId/flowName in each contract [AMENDED v3]
