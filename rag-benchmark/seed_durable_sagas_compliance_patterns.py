"""
FLOW-19 Pattern Seeder — seeds FLOW-19 (Durable Sagas & Compliance) design decisions
into Elasticsearch and nano-graphrag.

Reads fixture files from:
  fixtures/rag-patterns/       — ARCH_PATTERN fixtures
  fixtures/design-reasoning/   — DESIGN_REASONING records (DR-19-A through DR-19-F)
...
Usage:
  python seed_durable_sagas_compliance_patterns.py                                  # seed all, default endpoints
  python seed_durable_sagas_compliance_patterns.py --dry-run                        # parse only, no writes
  python seed_durable_sagas_compliance_patterns.py --es-endpoint http://localhost:19200
  python seed_durable_sagas_compliance_patterns.py --graphrag-url http://localhost:8001
  python seed_durable_sagas_compliance_patterns.py --skip-es                        # graphrag only
  python seed_durable_sagas_compliance_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (4):
    saga-persist-before-dispatch-001     — T621 storeDocumentWithOCC versionPin:-1 + OCC_CONFLICT idempotency
    lifo-saga-compensation-001           — T622 LIFO reverse-order compensation + stop-on-first-failure
    compliance-append-only-001           — T623 append-only compliance audit + retention policy at write time
    data-retention-execution-order-001   — T624 dual-gate retention (archive then delete) + SETNX purge lock

  DESIGN_REASONING records (6):
    DR-19-A through DR-19-F (from fixtures/design-reasoning/durable-sagas-compliance-design-decisions.json)
"""
...
FLOW19_FIXTURE_IDS = {
    'saga-persist-before-dispatch-001',
    'lifo-saga-compensation-001',
    'compliance-append-only-001',
    'data-retention-execution-order-001',
}

DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'durable-sagas-compliance-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-19 ARCH_PATTERN fixtures."""
    docs = []
    for fixture_id in FLOW19_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-19 DESIGN_REASONING records (DR-19-A through DR-19-F)."""
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow19-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-19 Durable Sagas & Compliance Pattern Seeder')
    ...
    print(f'  DESIGN_REASONING records (DR-19-A..F): {len(dr_docs)}')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow19-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-19 Durable Sagas & Compliance design patterns into Elasticsearch and nano-graphrag',
    )
