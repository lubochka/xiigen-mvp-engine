"""
FLOW-24 Pattern Seeder — seeds FLOW-24 (AI Safety Moderation) design decisions
into Elasticsearch and nano-graphrag.

Reads fixture files from:
  fixtures/bootstrap/history-seeds/   — DESIGN_REASONING records (DR-24-A through DR-24-F)
  (ARCH_PATTERN fixtures embedded in design corpus)
...
Usage:
  python seed_ai_safety_moderation_patterns.py                                  # seed all, default endpoints
  python seed_ai_safety_moderation_patterns.py --dry-run                        # parse only, no writes
  python seed_ai_safety_moderation_patterns.py --es-endpoint http://localhost:19200
  python seed_ai_safety_moderation_patterns.py --graphrag-url http://localhost:8001
  python seed_ai_safety_moderation_patterns.py --skip-es                        # graphrag only
  python seed_ai_safety_moderation_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (4):
    consent-gate-cf461              — T638 CheckSafetyConsentGate + explicit grant validation + pre-flight
    saga-ordering-cf465             — T639 SagaOrchestrator + 3-step deterministic order + completion signals
    client-score-rejection-dd226    — T640 GradeQuizService + AI authority + score mismatch logging
    timezone-from-profile-dd223     — T641 StreakResetService + user timezone + local midnight

  DESIGN_REASONING records (6):
    DR-24-A through DR-24-F (from server/src/bootstrap/history-seeds/ai-safety-moderation-design-corpus.json)
"""
...
DESIGN_REASONING_FILE = HISTORY_SEEDS_DIR / 'ai-safety-moderation-design-corpus.json'
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-24 DESIGN_REASONING + ARCH_PATTERN records from design corpus."""
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow24-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-24 AI Safety Moderation Pattern Seeder')
    ...
    docs = load_design_reasoning()
    dr_docs = [d for d in docs if d.get('patternType') == 'DESIGN_REASONING']
    rag_docs = [d for d in docs if d.get('patternType') == 'ARCH_PATTERN']
    print(f'  ARCH_PATTERN fixtures: {len(rag_docs)}')
    print(f'  DESIGN_REASONING records (DR-24-A..F): {len(dr_docs)}')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            graphrag_ok = True
            result = graphrag_insert(graphrag_url, docs, workspace='flow24-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-24 AI Safety Moderation design patterns into Elasticsearch and nano-graphrag',
    )
