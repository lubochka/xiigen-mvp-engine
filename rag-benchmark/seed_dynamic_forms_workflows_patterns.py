"""
FLOW-21 Pattern Seeder — seeds FLOW-21 (Dynamic Forms & Workflows) design decisions
into Elasticsearch and nano-graphrag.

Reads fixture files from:
  fixtures/rag-patterns/       — ARCH_PATTERN fixtures
  fixtures/design-reasoning/   — DESIGN_REASONING records (DR-21-A through DR-21-F)
...
Usage:
  python seed_dynamic_forms_workflows_patterns.py                                  # seed all, default endpoints
  python seed_dynamic_forms_workflows_patterns.py --dry-run                        # parse only, no writes
  python seed_dynamic_forms_workflows_patterns.py --es-endpoint http://localhost:19200
  python seed_dynamic_forms_workflows_patterns.py --graphrag-url http://localhost:8001
  python seed_dynamic_forms_workflows_patterns.py --skip-es                        # graphrag only
  python seed_dynamic_forms_workflows_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (4):
    form-schema-publishing-001           — T629 FormSchemaPublisher OCC schema DRAFT→PUBLISHED, immutable published version
    submission-validation-bola-001       — T630 FormSubmissionProcessor BOLA + published schema validation
    rule-automation-setnx-001            — T631 AutomationDispatcher SETNX rule locks + conditional dispatch
    submission-metrics-append-only-001   — T632 SubmissionAnalyticsCollector append-only PII-excluded analytics

  DESIGN_REASONING records (6):
    DR-21-A through DR-21-F (from fixtures/design-reasoning/dynamic-forms-workflows-design-decisions.json)
"""
...
FLOW21_FIXTURE_IDS = {
    'form-schema-publishing-001',
    'submission-validation-bola-001',
    'rule-automation-setnx-001',
    'submission-metrics-append-only-001',
}

DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'dynamic-forms-workflows-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-21 ARCH_PATTERN fixtures."""
    docs = []
    for fixture_id in FLOW21_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-21 DESIGN_REASONING records (DR-21-A through DR-21-F)."""
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow21-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-21 Dynamic Forms & Workflows Pattern Seeder')
    ...
    print(f'  DESIGN_REASONING records (DR-21-A..F): {len(dr_docs)}')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow21-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-21 Dynamic Forms & Workflows design patterns into Elasticsearch and nano-graphrag',
    )
