"""
FLOW-32 Pattern Seeder — seeds FLOW-32 (Sharable Flows Marketplace) design decisions
...
DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'sharable-flows-marketplace-design-decisions.json'
TENANT_ID = 'platform'
...
def es_ensure_index(endpoint: str, index_name: str) -> bool:
    try:
        ...
        if e.code == 404:
            try:
                _http('PUT', f'{endpoint}/{index_name}', {'settings': {'number_of_shards': 1, 'number_of_replicas': 0}})
...
def es_bulk_index(endpoint: str, index_name: str, docs: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    body = ('\n'.join(lines) + '\n').encode('utf-8')
    try:
        req = urllib.request.Request(f'{endpoint}/_bulk', data=body, headers={'Content-Type': 'application/x-ndjson'}, method='POST')
...
def fixture_to_graphrag_content(doc: Dict[str, Any]) -> str:
    ...
    content = f'Pattern: {pattern_id} | Type: {pattern_type}\nTags: {" ".join(tags)} | Keywords: {keywords[:120]}'
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow32-design') -> Dict[str, Any]:
    if not docs:
    ...
    for doc in docs:
        ...
        graphrag_docs.append({'doc_id': pattern_id, 'content': content, 'workspace': workspace, 'metadata': {'patternType': doc.get('patternType', ''), 'flowId': doc.get('flowId', ''), 'curriculumTier': doc.get('curriculumTier', 2)}})
    payload = json.dumps({'documents': graphrag_docs}).encode('utf-8')
    try:
        req = urllib.request.Request(f'{base_url}/insert', data=payload, headers={'Content-Type': 'application/json'}, method='POST')
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-32 Sharable Flows Marketplace Pattern Seeder')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            graphrag_ok = True
            all_docs = dr_docs
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow32-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Seed FLOW-32 Sharable Flows Marketplace design patterns')
