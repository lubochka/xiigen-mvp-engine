"""
FLOW-15 Pattern Seeder — seeds FLOW-15 (SaaS Multi-Tenancy) design decisions
into Elasticsearch and nano-graphrag.

Reads fixture files from:
  fixtures/rag-patterns/       — ARCH_PATTERN fixtures
  fixtures/design-reasoning/   — DESIGN_REASONING records (DR-15-A through DR-15-F)
...
Usage:
  python seed_saas_multi_tenancy_patterns.py                                  # seed all, default endpoints
  python seed_saas_multi_tenancy_patterns.py --dry-run                        # parse only, no writes
  python seed_saas_multi_tenancy_patterns.py --es-endpoint http://localhost:19200
  python seed_saas_multi_tenancy_patterns.py --graphrag-url http://localhost:8001
  python seed_saas_multi_tenancy_patterns.py --skip-es                        # graphrag only
  python seed_saas_multi_tenancy_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (4):
    atomic-tenant-bootstrap-001      — T605 SETNX ORDER 1 + synchronous bulkSeed + DNA-8 storeDocument before enqueue
    immutable-machine-key-guard-001  — T606 MACHINE_LOCKED_KEYS compile-time + ORDER 1 rejection + OCC write-only
    quota-atomic-materialization-001 — T607 Redis MULTI/EXEC atomic quota — never individual SET loop
    suspend-not-delete-001           — T608 suspend=updateDocument + cascadeToSubscriptions + delegated purge tombstoneRef

  DESIGN_REASONING records (6):
    DR-15-A through DR-15-F (from fixtures/design-reasoning/saas-multi-tenancy-design-decisions.json)
"""
...
FLOW15_FIXTURE_IDS = {
    'atomic-tenant-bootstrap-001',
    'immutable-machine-key-guard-001',
    'quota-atomic-materialization-001',
    'suspend-not-delete-001',
}

DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'saas-multi-tenancy-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-15 ARCH_PATTERN fixtures."""
    docs = []
    for fixture_id in FLOW15_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-15 DESIGN_REASONING records (DR-15-A through DR-15-F)."""
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow15-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-15 SaaS Multi-Tenancy Pattern Seeder')
    ...
    print(f'  DESIGN_REASONING records (DR-15-A..F): {len(dr_docs)}')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow15-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-15 SaaS Multi-Tenancy design patterns into Elasticsearch and nano-graphrag',
    )
