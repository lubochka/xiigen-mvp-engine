"""
FLOW-20 Ads Platform Pattern Seeder

Seeds FLOW-20 design decisions and RAG patterns into Elasticsearch and nano-graphrag.
...
Usage:
  python seed_ads_platform_patterns.py                                   # seed all
  python seed_ads_platform_patterns.py --dry-run                         # parse only
  python seed_ads_platform_patterns.py --es-endpoint http://localhost:19200
  python seed_ads_platform_patterns.py --graphrag-url http://localhost:8001
  python seed_ads_platform_patterns.py --skip-es                         # graphrag only
  python seed_ads_platform_patterns.py --skip-graphrag                   # ES only

What this seeds:
  ARCH_PATTERN fixtures (4):
    consent-gate-first-001
    stateless-redis-auction-001
    fraud-before-billing-001
    political-dual-gate-001

  DESIGN_REASONING triples (6):
    DR-20-A through DR-20-F (from fixtures/design-reasoning/ads-platform-design-decisions.json)
"""
...
FLOW20_DR_FILE = 'ads-platform-design-decisions.json'
...
def seed_rag_patterns(
    ...
    seeded = 0
    print('\n── FLOW-20 RAG Patterns ──────────────────────────────────────────')
    for fixture_id in sorted(FLOW20_FIXTURE_IDS):
...
def seed_design_reasoning(
    ...
    seeded = 0
    print('\n── FLOW-20 Design Reasoning Triples ─────────────────────────────')
    dr_path = DESIGN_REASONING_DIR / FLOW20_DR_FILE
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-20 ads platform RAG patterns into Elasticsearch and nano-graphrag.'
    )
    ...
    print('FLOW-20 Ads Platform Pattern Seeder')
