"""
FLOW-12 Pattern Seeder — seeds FLOW-12 (Subscription Billing) design decisions
into Elasticsearch and nano-graphrag.

Reads fixture files from:
  fixtures/rag-patterns/       — ARCH_PATTERN fixtures
  fixtures/design-reasoning/   — DESIGN_REASONING records (DR-12-A through DR-12-F)
...
Usage:
  python seed_subscription_billing_patterns.py                                  # seed all, default endpoints
  python seed_subscription_billing_patterns.py --dry-run                        # parse only, no writes
  python seed_subscription_billing_patterns.py --es-endpoint http://localhost:19200
  python seed_subscription_billing_patterns.py --graphrag-url http://localhost:8001
  python seed_subscription_billing_patterns.py --skip-es                        # graphrag only
  python seed_subscription_billing_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (4):
    integer-cents-guard-001         — T209 price must be integer cents > 0 at ORDER 1
    status-before-lock-001          — T211 getStatus() at ORDER 1 before lock acquisition
    additive-subtractive-mrr-001    — T212 both SubscriptionActivated + SubscriptionCancelled handlers
    additive-subtractive-aggregate-001 — T212 MRR normalization formula (MACHINE constant)

  DESIGN_REASONING records (5):
    DR-12-A through DR-12-E (from fixtures/design-reasoning/subscription-billing-design-decisions.json)
"""
...
FLOW12_FIXTURE_IDS = {
    'integer-cents-guard-001',
    'status-before-lock-001',
    'additive-subtractive-mrr-001',
    'additive-subtractive-aggregate-001',
}

DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'subscription-billing-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-12 ARCH_PATTERN fixtures."""
    docs = []
    for fixture_id in FLOW12_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-12 DESIGN_REASONING records (DR-12-A through DR-12-F)."""
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow12-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-12 Subscription Billing Pattern Seeder')
    ...
    print(f'  DESIGN_REASONING records (DR-12-A..E): {len(dr_docs)}')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow12-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-12 Subscription Billing design patterns into Elasticsearch and nano-graphrag',
    )
