"""
FLOW-17 Pattern Seeder — seeds FLOW-17 (Freelancer Marketplace) design decisions
into Elasticsearch and nano-graphrag.

Reads fixture files from:
  fixtures/rag-patterns/       — ARCH_PATTERN fixtures
  fixtures/design-reasoning/   — DESIGN_REASONING records (DR-17-A through DR-17-F)
...
Usage:
  python seed_freelancer_marketplace_patterns.py                                  # seed all, default endpoints
  python seed_freelancer_marketplace_patterns.py --dry-run                        # parse only, no writes
  python seed_freelancer_marketplace_patterns.py --es-endpoint http://localhost:19200
  python seed_freelancer_marketplace_patterns.py --graphrag-url http://localhost:8001
  python seed_freelancer_marketplace_patterns.py --skip-es                        # graphrag only
  python seed_freelancer_marketplace_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (4):
    proposal-acceptance-lock-001      — T613 BOLA ORDER 1 + SETNX gig-accept-lock + OCC bid status three-guard gate
    milestone-immutability-gate-001   — T614 CONTRACT_IMMUTABLE_FIELDS compile-time + sum validation + OCC write
    delivery-gate-before-release-001  — T615 delivery gate + LIFO compensation [REFUND_MILESTONE, RESTORE_GIG_STATUS] + timer FREEDOM
    single-direction-review-001       — T616 single-direction per engagement + append-only audit + comment excluded

  DESIGN_REASONING records (6):
    DR-17-A through DR-17-F (from fixtures/design-reasoning/freelancer-marketplace-design-decisions.json)
"""
...
FLOW17_FIXTURE_IDS = {
    'proposal-acceptance-lock-001',
    'milestone-immutability-gate-001',
    'delivery-gate-before-release-001',
    'single-direction-review-001',
}

DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'freelancer-marketplace-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-17 ARCH_PATTERN fixtures."""
    docs = []
    for fixture_id in FLOW17_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-17 DESIGN_REASONING records (DR-17-A through DR-17-F)."""
...
def fixture_to_graphrag_content(doc: Dict[str, Any]) -> str:
    ...
    content = (
        ...
        f'IronRules: {"; ".join(iron_rules[:200])} | Code: {code_snippet[:200]}\n'
    ...
    if 'positiveExample' in doc:
        content += f'\nPositiveExample: {doc["positiveExample"]}'
    if 'negativeExample' in doc:
        content += f'\nNegativeExample: {doc["negativeExample"]}'
    if 'discriminatingConstraint' in doc:
        content += f'\nDiscriminatingConstraint: {doc["discriminatingConstraint"]}'
    if 'appliesTo' in doc:
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow17-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-17 Freelancer Marketplace Pattern Seeder')
    ...
    print(f'  ARCH_PATTERN fixtures: {len(rag_docs)}')
    print(f'  DESIGN_REASONING records (DR-17-A..F): {len(dr_docs)}')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow17-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-17 Freelancer Marketplace design patterns into Elasticsearch and nano-graphrag',
    )
