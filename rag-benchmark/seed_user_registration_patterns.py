"""
FLOW-01 Pattern Seeder — seeds FLOW-01 (User Registration) design decisions into Elasticsearch and nano-graphrag.

Reads fixture files from:
  fixtures/rag-patterns/       — ARCH_PATTERN fixtures
  server/src/bootstrap/history-seeds/ — design corpus records
...
Usage:
  python seed_user_registration_patterns.py                                  # seed all, default endpoints
  python seed_user_registration_patterns.py --dry-run                        # parse only, no writes
  python seed_user_registration_patterns.py --es-endpoint http://localhost:19200
  python seed_user_registration_patterns.py --graphrag-url http://localhost:8001
  python seed_user_registration_patterns.py --skip-es                        # graphrag only
  python seed_user_registration_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (6):
    FLOW-01-RAG-01 through FLOW-01-RAG-06

  DESIGN_REASONING records (from server/src/bootstrap/history-seeds/user-registration-design-corpus.json)
"""
...
DESIGN_REASONING_FILE = PROJECT_ROOT / 'server' / 'src' / 'bootstrap' / 'history-seeds' / 'user-registration-design-corpus.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-01 ARCH_PATTERN fixtures."""
    docs = []
    for fixture_id in FLOW01_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-01 DESIGN_REASONING records from history-seeds corpus."""
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow01-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-01 User Registration Pattern Seeder')
    ...
    print(f'  DESIGN_REASONING records: {len(dr_docs)}')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow01-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-01 User Registration design patterns into Elasticsearch and nano-graphrag',
    )
