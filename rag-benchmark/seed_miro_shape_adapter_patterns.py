"""
FLOW-42 Miro App Shape Heuristics Adapter Pattern Seeder

Seeds FLOW-42 design decisions and RAG patterns into Elasticsearch and nano-graphrag.
...
Usage:
  python seed_miro_shape_adapter_patterns.py                                   # seed all
  python seed_miro_shape_adapter_patterns.py --dry-run                         # parse only
  python seed_miro_shape_adapter_patterns.py --es-endpoint http://localhost:19200
  python seed_miro_shape_adapter_patterns.py --graphrag-url http://localhost:8001
  python seed_miro_shape_adapter_patterns.py --skip-es                         # graphrag only
  python seed_miro_shape_adapter_patterns.py --skip-graphrag                   # ES only

What this seeds:
  Design Reasoning + RAG (8):
    DR-42-A through DR-42-D
    SHAPE-HEURISTIC-INFERENCE-001, UNSUPPORTED-ELEMENT-AUDIT-001, etc. (4 RAG patterns)
"""
...
FLOW42_CORPUS_FILE = 'miro-shape-adapter-design-corpus.json'
...
def seed_design_corpus(
    ...
    seeded = 0
    print('\n── FLOW-42 Design Corpus (DR + RAG patterns) ──────────────────')
    corpus_path = HISTORY_SEEDS_DIR / FLOW42_CORPUS_FILE
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-42 Miro Shape Adapter RAG patterns into Elasticsearch and nano-graphrag.'
    )
    ...
    print('FLOW-42 Miro Shape Adapter Pattern Seeder')
