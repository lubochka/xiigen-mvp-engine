"""
FLOW-09 Pattern Seeder — seeds FLOW-09 design decisions into Elasticsearch and nano-graphrag.

Reads fixture files from:
  fixtures/rag-patterns/       — ARCH_PATTERN fixtures
  fixtures/design-reasoning/   — DESIGN_REASONING records (DR-09-A through DR-09-G)
...
Usage:
  python seed_flow09_patterns.py                                  # seed all, default endpoints
  python seed_flow09_patterns.py --dry-run                        # parse only, no writes
  python seed_flow09_patterns.py --es-endpoint http://localhost:19200
  python seed_flow09_patterns.py --graphrag-url http://localhost:8001
  python seed_flow09_patterns.py --skip-es                        # graphrag only
  python seed_flow09_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (5):
    seat-before-payment
    fail-open-fraud-detection
    inline-pure-calculation
    all-or-nothing-group-booking
    platform-only-qr

  DESIGN_REASONING records (7):
    DR-09-A through DR-09-G (from fixtures/design-reasoning/flow09-design-decisions.json)
"""
...
DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'flow09-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-09 ARCH_PATTERN fixtures."""
    docs = []
    for fixture_id in FLOW09_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-09 DESIGN_REASONING records (DR-09-A through DR-09-G)."""
...
def fixture_to_graphrag_content(doc: Dict[str, Any]) -> str:
    ...
    if 'antiPattern' in doc:
        content += f'\nAnti-Pattern (REJECTED): {doc["antiPattern"]}'
    if 'appliesTo' in doc:
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow09-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-09 Pattern Seeder')
    ...
    print(f'  DESIGN_REASONING records (DR-09-A..G): {len(dr_docs)}')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow09-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-09 design patterns into Elasticsearch and nano-graphrag',
    )
