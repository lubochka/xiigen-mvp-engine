"""
FLOW-45 History Bootstrap Pattern Seeder

Seeds FLOW-45 design decisions and RAG patterns into Elasticsearch and nano-graphrag.

Reads fixture files from:
  server/src/bootstrap/history-seeds/ — Design corpus (4 RAG + 4 DR = 8 records)

Usage:
  python seed_history_bootstrap_patterns.py                                   # seed all
  python seed_history_bootstrap_patterns.py --dry-run                         # parse only
"""
...
FLOW45_CORPUS_FILE = 'history-bootstrap-design-corpus.json'
...
def seed_design_corpus(es_url: str, graphrag_url: str, skip_es: bool, skip_graphrag: bool, dry_run: bool) -> int:
    seeded = 0
    print('\n── FLOW-45 History Bootstrap Corpus ───────────────────────────')
    corpus_path = HISTORY_SEEDS_DIR / FLOW45_CORPUS_FILE
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Seed FLOW-45 History Bootstrap RAG patterns.')
    ...
    print('FLOW-45 History Bootstrap Pattern Seeder')
