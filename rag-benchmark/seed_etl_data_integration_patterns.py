"""
FLOW-14 Pattern Seeder — seeds FLOW-14 (ETL & Data Integration) design decisions
into Elasticsearch and nano-graphrag.

Reads fixture files from:
  fixtures/rag-patterns/       — ARCH_PATTERN fixtures
  fixtures/design-reasoning/   — DESIGN_REASONING records (DR-14-A through DR-14-F)
...
Usage:
  python seed_etl_data_integration_patterns.py                                  # seed all, default endpoints
  python seed_etl_data_integration_patterns.py --dry-run                        # parse only, no writes
  python seed_etl_data_integration_patterns.py --es-endpoint http://localhost:19200
  python seed_etl_data_integration_patterns.py --graphrag-url http://localhost:8001
  python seed_etl_data_integration_patterns.py --skip-es                        # graphrag only
  python seed_etl_data_integration_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (4):
    ep4-saga-cycle-001          — T214 EP-4 durable saga: rate_check→poll_page→land_raw→commit_cursor
    timing-safe-hmac-001        — T215 CF-211 HMAC timingSafeEqual — NEVER string ===
    scd2-no-update-001          — T219 DR-62 SCD-2 close_old+open_new — never updateDocument
    pii-gate-order1-001         — T220 DR-63 PII gate at ORDER 1 — blocks mart write if piiFieldsDetected > 0

  DESIGN_REASONING records (6):
    DR-14-A through DR-14-F (from fixtures/design-reasoning/etl-data-integration-design-decisions.json)
"""
...
FLOW14_FIXTURE_IDS = {
    'ep4-saga-cycle-001',
    'timing-safe-hmac-001',
    'scd2-no-update-001',
    'pii-gate-order1-001',
}

DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'etl-data-integration-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-14 ARCH_PATTERN fixtures."""
    docs = []
    for fixture_id in FLOW14_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-14 DESIGN_REASONING records (DR-14-A through DR-14-F)."""
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow14-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-14 ETL & Data Integration Pattern Seeder')
    ...
    print(f'  DESIGN_REASONING records (DR-14-A..F): {len(dr_docs)}')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow14-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-14 ETL & Data Integration design patterns into Elasticsearch and nano-graphrag',
    )
