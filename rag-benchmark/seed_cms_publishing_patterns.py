"""
FLOW-22 Pattern Seeder — seeds FLOW-22 (CMS Publishing) design decisions
into Elasticsearch and nano-graphrag.

Reads fixture files from:
  fixtures/rag-patterns/       — ARCH_PATTERN fixtures
  fixtures/design-reasoning/   — DESIGN_REASONING records (DR-22-A through DR-22-F)
...
Usage:
  python seed_cms_publishing_patterns.py                                  # seed all, default endpoints
  python seed_cms_publishing_patterns.py --dry-run                        # parse only, no writes
  python seed_cms_publishing_patterns.py --es-endpoint http://localhost:19200
  python seed_cms_publishing_patterns.py --graphrag-url http://localhost:8001
  python seed_cms_publishing_patterns.py --skip-es                        # graphrag only
  python seed_cms_publishing_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (4):
    content-version-publishing-001       — T633 ContentVersionPublisher OCC content DRAFT→PUBLISHED, immutable published version
    content-approval-sequential-001      — T634 ContentApprovalWorkflow 4-stage sequential approval with role gates
    content-schedule-dispatch-001        — T635 ContentScheduleDispatcher SETNX schedule locks + CRON dispatch
    content-analytics-append-only-001    — T636 ContentAnalyticsAggregator append-only PII-excluded metrics

  DESIGN_REASONING records (6):
    DR-22-A through DR-22-F (from fixtures/design-reasoning/cms-publishing-design-decisions.json)
"""
...
FLOW22_FIXTURE_IDS = {
    'content-version-publishing-001',
    'content-approval-sequential-001',
    'content-schedule-dispatch-001',
    'content-analytics-append-only-001',
}

DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'cms-publishing-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-22 ARCH_PATTERN fixtures."""
    docs = []
    for fixture_id in FLOW22_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-22 DESIGN_REASONING records (DR-22-A through DR-22-F)."""
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow22-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-22 CMS Publishing Pattern Seeder')
    ...
    print(f'  DESIGN_REASONING records (DR-22-A..F): {len(dr_docs)}')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow22-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-22 CMS Publishing design patterns into Elasticsearch and nano-graphrag',
    )
