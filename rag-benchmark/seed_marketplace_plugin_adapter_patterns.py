"""
FLOW-34 Pattern Seeder — seeds FLOW-34 (Marketplace Plugin Adapter) design decisions
...
DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'marketplace-plugin-adapter-design-decisions.json'
TENANT_ID = 'platform'
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow34-design') -> Dict[str, Any]:
    if not docs:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-34 Marketplace Plugin Adapter Pattern Seeder')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            graphrag_ok = True
            all_docs = dr_docs
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow34-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Seed FLOW-34 Marketplace Plugin Adapter design patterns')
