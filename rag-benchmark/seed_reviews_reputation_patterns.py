"""
FLOW-10 Pattern Seeder — seeds FLOW-10 (Reviews & Reputation) design decisions into Elasticsearch and nano-graphrag.

Reads fixture files from:
  fixtures/rag-patterns/       — ARCH_PATTERN fixtures
  fixtures/design-reasoning/   — DESIGN_REASONING records (DR-10-A through DR-10-J)
...
Usage:
  python seed_reviews_reputation_patterns.py                                  # seed all, default endpoints
  python seed_reviews_reputation_patterns.py --dry-run                        # parse only, no writes
  python seed_reviews_reputation_patterns.py --es-endpoint http://localhost:19200
  python seed_reviews_reputation_patterns.py --graphrag-url http://localhost:8001
  python seed_reviews_reputation_patterns.py --skip-es                        # graphrag only
  python seed_reviews_reputation_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (5):
    eligibility-before-audit-001
    three-path-moderation-001
    additive-subtractive-aggregate-001
    reputation-score-clamping-001
    conditional-revision-gate-001

  DESIGN_REASONING records (10):
    DR-10-A through DR-10-J (from fixtures/design-reasoning/reviews-reputation-design-decisions.json)
"""
...
DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'reviews-reputation-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-10 ARCH_PATTERN fixtures."""
    docs = []
    for fixture_id in FLOW10_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-10 DESIGN_REASONING records (DR-10-A through DR-10-J)."""
    ...
    for doc in records:
        doc['tenantId'] = TENANT_ID
        doc['patternId'] = doc.get('id', doc.get('patternId', 'unknown'))
    return records
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow10-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-10 Reviews & Reputation Pattern Seeder')
    ...
    print(f'  DESIGN_REASONING records (DR-10-A..J): {len(dr_docs)}')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow10-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-10 Reviews & Reputation design patterns into Elasticsearch and nano-graphrag',
    )
