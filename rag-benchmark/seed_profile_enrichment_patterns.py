"""
FLOW-02 Pattern Seeder — seeds FLOW-02 design decisions into Elasticsearch and nano-graphrag.
...
Usage:
  python seed_flow02_patterns.py                                  # seed all, default endpoints
  python seed_flow02_patterns.py --dry-run                        # parse only, no writes
  python seed_flow02_patterns.py --es-endpoint http://localhost:19200
  python seed_flow02_patterns.py --graphrag-url http://localhost:8001
  python seed_flow02_patterns.py --skip-es                        # graphrag only
  python seed_flow02_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (8):
  ...
  PLAN_EXEMPLAR fixture (1):
    plan-flow02-business-onboarding

  DESIGN_REASONING triples (7):
    DR-02-A through DR-02-G (from fixtures/design-reasoning/flow02-design-decisions.json)
"""
...
DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'flow02-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-02 ARCH_PATTERN and PLAN_EXEMPLAR fixtures."""
    docs = []
    for fixture_id in FLOW02_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-02 DESIGN_REASONING triples."""
...
def fixture_to_graphrag_content(doc: Dict[str, Any]) -> str:
    ...
    domain_id    = doc.get('domainId', '')

    parts.append(f'Pattern: {pattern_id}')
    parts.append(f'Type: {pattern_type} | Flow: {flow_id} | Domain: {domain_id}')
    ...
    # PLAN_EXEMPLAR fields
    if 'summary' in doc:
        parts.append(f'\nSummary:\n{doc["summary"]}')
    if 'keyPatterns' in doc:
        parts.append(f'\nKey Patterns:\n' + '\n'.join(f'  - {p}' for p in doc['keyPatterns']))
    if 'crossFlowGate' in doc:
        parts.append(f'\nCross-Flow Gate: {doc["crossFlowGate"]}')
    if 'criticalConstraints' in doc:
        parts.append(f'\nCritical Constraints:\n' + '\n'.join(f'  - {c}' for c in doc['criticalConstraints']))

    # ARCH_PATTERN fields
    if 'codeSnippet' in doc:
        parts.append(f'\nCode Pattern:\n{doc["codeSnippet"]}')
    if 'ironRules' in doc:
        parts.append(f'\nIron Rules (MACHINE constraints):\n' + '\n'.join(f'  - {r}' for r in doc['ironRules']))
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow02-design') -> Dict[str, Any]:
    """
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-02 Pattern Seeder')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            print(f'  Inserting {len(all_docs)} documents into workspace flow02-design ...')
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow02-design')
    ...
    if not es_ok and not graphrag_ok and not skip_es and not skip_graphrag:
        ...
        print()
        print('  Then re-run:')
        print('    python rag-benchmark/seed_flow02_patterns.py')
    print(sep)
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-02 design patterns into Elasticsearch and nano-graphrag',
