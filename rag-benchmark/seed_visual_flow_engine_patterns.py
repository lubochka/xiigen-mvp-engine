"""
FLOW-18 Pattern Seeder — seeds FLOW-18 (Visual Flow Engine) design decisions
into Elasticsearch and nano-graphrag.

Reads fixture files from:
  fixtures/rag-patterns/       — ARCH_PATTERN fixtures
  fixtures/design-reasoning/   — DESIGN_REASONING records (DR-18-A through DR-18-F)
...
Usage:
  python seed_visual_flow_engine_patterns.py                                  # seed all, default endpoints
  python seed_visual_flow_engine_patterns.py --dry-run                        # parse only, no writes
  python seed_visual_flow_engine_patterns.py --es-endpoint http://localhost:19200
  python seed_visual_flow_engine_patterns.py --graphrag-url http://localhost:8001
  python seed_visual_flow_engine_patterns.py --skip-es                        # graphrag only
  python seed_visual_flow_engine_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (4):
    draft-before-publish-gate-001          — T617 BOLA ORDER 1 + FLOW_IMMUTABLE ORDER 2 + one-way DRAFT→PUBLISHED guard
    connection-type-compatibility-001      — T618 DFS WHITE/GRAY/BLACK cycle detection + type compatibility per-edge + OCC
    atomic-node-type-registration-001      — T619 SETNX node-type-reg-lock + atomic dual-write + lock release in catch
    injection-version-lock-001             — T620 version lock + PRE_WRITE audit rollback pointer + append-only three-phase

  DESIGN_REASONING records (6):
    DR-18-A through DR-18-F (from fixtures/design-reasoning/visual-flow-engine-design-decisions.json)
"""
...
FLOW18_FIXTURE_IDS = {
    'draft-before-publish-gate-001',
    'connection-type-compatibility-001',
    'atomic-node-type-registration-001',
    'injection-version-lock-001',
}

DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'visual-flow-engine-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-18 ARCH_PATTERN fixtures."""
    docs = []
    for fixture_id in FLOW18_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-18 DESIGN_REASONING records (DR-18-A through DR-18-F)."""
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow18-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-18 Visual Flow Engine Pattern Seeder')
    ...
    print(f'  DESIGN_REASONING records (DR-18-A..F): {len(dr_docs)}')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow18-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-18 Visual Flow Engine design patterns into Elasticsearch and nano-graphrag',
    )
