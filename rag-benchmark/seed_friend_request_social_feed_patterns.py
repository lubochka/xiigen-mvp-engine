"""
FLOW-07 Pattern Seeder — seeds FLOW-07 design decisions into Elasticsearch and nano-graphrag.
...
Usage:
  python seed_flow07_patterns.py                                  # seed all, default endpoints
  python seed_flow07_patterns.py --dry-run                        # parse only, no writes
  python seed_flow07_patterns.py --es-endpoint http://localhost:19200
  python seed_flow07_patterns.py --graphrag-url http://localhost:8001
  python seed_flow07_patterns.py --skip-es                        # graphrag only
  python seed_flow07_patterns.py --skip-graphrag                  # ES only
...
What this seeds:
  ...
  DESIGN_REASONING triples (6):
    FLOW-07-DR-01 through FLOW-07-DR-06 (from fixtures/design-reasoning/flow07-design-decisions.json)
"""
...
DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'flow07-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-07 ARCH_PATTERN and PLAN_EXEMPLAR fixtures."""
    docs = []
    for fixture_id in FLOW07_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-07 DESIGN_REASONING triples."""
...
def fixture_to_graphrag_content(doc: Dict[str, Any]) -> str:
    ...
    # Rich text format for GraphRAG extraction
    content = (
        f'Pattern: {pattern_id} | Type: {pattern_type} | Archetype: {archetype}\n'
        f'IronRules: {"; ".join(iron_rules[:200])} | Code: {code_snippet[:200]}\n'
        f'Tags: {" ".join(tags)} | Keywords: {keywords[:120]}'
    )

    # Teaching content for DESIGN_REASONING records
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow07-design') -> Dict[str, Any]:
    """
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-07 Pattern Seeder')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            print(f'  Inserting {len(all_docs)} documents into workspace flow07-design ...')
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow07-design')
    ...
    if not es_ok and not graphrag_ok and not skip_es and not skip_graphrag:
        ...
        print()
        print('  Then re-run:')
        print('    python rag-benchmark/seed_flow07_patterns.py')
    print(sep)
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-07 design patterns into Elasticsearch and nano-graphrag',
