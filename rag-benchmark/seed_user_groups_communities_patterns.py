"""
FLOW-06 Pattern Seeder — seeds FLOW-06 design decisions into Elasticsearch and nano-graphrag.
...
Usage:
  python seed_flow06_patterns.py                                  # seed all, default endpoints
  python seed_flow06_patterns.py --dry-run                        # parse only, no writes
  python seed_flow06_patterns.py --es-endpoint http://localhost:19200
  python seed_flow06_patterns.py --graphrag-url http://localhost:8001
  python seed_flow06_patterns.py --skip-es                        # graphrag only
  python seed_flow06_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (4):
    arch--access-control-before-content
    arch--server-side-tier-assignment
    arch--group-join-idempotency
    arch--tier-gated-content-query

  PLAN_EXEMPLAR fixture (1):
    plan-flow06-membership-group-feed

  DESIGN_REASONING triples (7):
    DR-06-A through DR-06-G (from fixtures/design-reasoning/flow06-design-decisions.json)
"""
...
DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'flow06-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-06 ARCH_PATTERN and PLAN_EXEMPLAR fixtures."""
    docs = []
    for fixture_id in FLOW06_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-06 DESIGN_REASONING triples."""
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow06-design') -> Dict[str, Any]:
    """
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-06 Pattern Seeder')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            print(f'  Inserting {len(all_docs)} documents into workspace flow06-design ...')
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow06-design')
    ...
    if not es_ok and not graphrag_ok and not skip_es and not skip_graphrag:
        ...
        print()
        print('  Then re-run:')
        print('    python rag-benchmark/seed_flow06_patterns.py')
    print(sep)
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-06 design patterns into Elasticsearch and nano-graphrag',
