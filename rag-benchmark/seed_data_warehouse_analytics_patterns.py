"""
FLOW-13 Pattern Seeder — seeds FLOW-13 (Data Warehouse & Analytics) design decisions
into Elasticsearch and nano-graphrag.

Reads fixture files from:
  fixtures/rag-patterns/       — ARCH_PATTERN fixtures
  fixtures/design-reasoning/   — DESIGN_REASONING records (DR-13-A through DR-13-E)
...
Usage:
  python seed_data_warehouse_analytics_patterns.py                                  # seed all, default endpoints
  python seed_data_warehouse_analytics_patterns.py --dry-run                        # parse only, no writes
  python seed_data_warehouse_analytics_patterns.py --es-endpoint http://localhost:19200
  python seed_data_warehouse_analytics_patterns.py --graphrag-url http://localhost:8001
  python seed_data_warehouse_analytics_patterns.py --skip-es                        # graphrag only
  python seed_data_warehouse_analytics_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (3):
    three-layer-gate-order-001       — T173 gate order immutable (T187→F422→F423)
    inline-quota-manager-001         — T187 inline, F426 does not exist
    data-retention-execution-order-001 — T186 4-step purge order + tombstoneRef-only

  DESIGN_REASONING records (5):
    DR-13-A through DR-13-E (from fixtures/design-reasoning/data-warehouse-analytics-design-decisions.json)
"""
...
FLOW13_FIXTURE_IDS = {
    'three-layer-gate-order-001',
    'inline-quota-manager-001',
    'data-retention-execution-order-001',
}

DESIGN_REASONING_FILE = DESIGN_REASONING_DIR / 'data-warehouse-analytics-design-decisions.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-13 ARCH_PATTERN fixtures."""
    docs = []
    for fixture_id in FLOW13_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-13 DESIGN_REASONING records (DR-13-A through DR-13-E)."""
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow13-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-13 Data Warehouse & Analytics Pattern Seeder')
    ...
    print(f'  DESIGN_REASONING records (DR-13-A..E): {len(dr_docs)}')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow13-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-13 Data Warehouse & Analytics design patterns into Elasticsearch and nano-graphrag',
    )
