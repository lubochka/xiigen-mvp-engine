"""
FLOW-03 Pattern Seeder — seeds FLOW-03 (Event Management) design decisions into Elasticsearch and nano-graphrag.
...
Usage:
  python seed_event_management_patterns.py                                  # seed all, default endpoints
  python seed_event_management_patterns.py --dry-run                        # parse only, no writes
  python seed_event_management_patterns.py --es-endpoint http://localhost:19200
  python seed_event_management_patterns.py --graphrag-url http://localhost:8001
  python seed_event_management_patterns.py --skip-es                        # graphrag only
  python seed_event_management_patterns.py --skip-graphrag                  # ES only

What this seeds:
  ARCH_PATTERN fixtures (4):
    FLOW-03-RAG-01 through FLOW-03-RAG-04

  DESIGN_REASONING records (from server/src/bootstrap/history-seeds/event-management-design-corpus.json)
"""
...
DESIGN_REASONING_FILE = PROJECT_ROOT / 'server' / 'src' / 'bootstrap' / 'history-seeds' / 'event-management-design-corpus.json'
...
def load_rag_fixtures() -> List[Dict[str, Any]]:
    """Load FLOW-03 ARCH_PATTERN fixtures."""
    docs = []
    for fixture_id in FLOW03_FIXTURE_IDS:
...
def load_design_reasoning() -> List[Dict[str, Any]]:
    """Load FLOW-03 DESIGN_REASONING records from history-seeds corpus."""
...
def graphrag_insert(base_url: str, docs: List[Dict[str, Any]], workspace: str = 'flow03-design') -> Dict[str, Any]:
...
def run(args: argparse.Namespace) -> int:
    ...
    sep = '-' * 60
    print(sep)
    print('XIIGen FLOW-03 Event Management Pattern Seeder')
    ...
    if not skip_graphrag:
        ...
        if not graphrag_check(graphrag_url):
            ...
            result = graphrag_insert(graphrag_url, all_docs, workspace='flow03-design')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Seed FLOW-03 Event Management design patterns into Elasticsearch and nano-graphrag',
    )
