/**
 * FLOW-42 — RAG Quality Graph Real-CRUD Round-Trip E2E
 *
 * Exercises /api/dynamic/xiigen-rag-quality-graph end-to-end.
...
const INDEX = 'xiigen-rag-quality-graph';

test.describe('FLOW-42 — RAG Quality Graph real CRUD', () => {
  ...
  test('C-02: create → list → delete round-trip', async ({ page, request }) => {
    ...
    await expect(page.getByTestId(`rag-quality-graph-row-${docId}`)).toBeVisible();
  ...
  test('C-03: UI create form submits + refreshes list', async ({ page, request }) => {
    ...
    await page.getByTestId('rag-quality-graph-create-button').click();
    await expect(page.getByTestId('rag-quality-graph-form')).toBeVisible();

    const name = 'ui-form-record';
    await page.getByTestId('rag-quality-graph-form-name').fill(name);
    await page.getByTestId('rag-quality-graph-form-status').fill('active');
    await page.getByTestId('rag-quality-graph-form-notes').fill('created via UI form');
    const postResponse = page.waitForResponse(
      (r) => r.url().includes(`/api/dynamic/${INDEX}`) && r.request().method() === 'POST',
    );
    await page.getByTestId('rag-quality-graph-form-submit').click();
    const resp = await postResponse;
    expect(resp.ok()).toBeTruthy();

    await expect(page.getByTestId('rag-quality-graph-form')).toBeHidden();
  ...
  test('C-04: list or empty-state renders on load', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('page-rag-quality-graph')).toBeVisible();
    const listVisible = await page.getByTestId('rag-quality-graph-list').isVisible().catch(() => false);
    const emptyVisible = await page.getByTestId('rag-quality-graph-empty').isVisible().catch(() => false);
