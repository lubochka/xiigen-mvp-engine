...
test.describe('FLOW-00 Tenant C installed Tenant B package visual evidence', () => {
  test.beforeAll(() => {
    ...
    for (const cell of CELLS) {
      test(`${role} ${cell.id}`, async ({ page }) => {
        ...
        await expect(page.getByTestId('ba-tenant-adaptation')).toContainText(
          'Tessera installed branch launch policy',
        );
        await expect(page.getByTestId('ba-tenant-adaptation-summary')).toContainText(
          'Confirm branch readiness',
        );
        await expect(page.getByTestId('ba-tenant-adaptation-summary')).toContainText(
          'Northwind branch ops',
        );
        await expect(page.getByTestId('ba-tenant-adaptation-summary')).toContainText(
          'about 11 seconds',
        );
        await expect(page.getByTestId('ba-tenant-adaptation-summary')).toContainText(
          'Installed from Northwind launch bundles',
        );
