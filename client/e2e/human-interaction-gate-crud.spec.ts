/**
 * FLOW-27 — Human Interaction Gate Real-CRUD Round-Trip E2E
 *
 * Exercises /api/dynamic/xiigen-human-interaction-gate end-to-end.
...
const SNAP = (name: string) =>
  path.join(__dirname, '..', '..', 'docs', 'e2e-snapshots', 'human-interaction-gate', name);

const API = 'http://localhost:3000/api';
const INDEX = 'xiigen-human-interaction-gate';

test.describe('FLOW-27 — Human Interaction Gate real CRUD', () => {
  ...
  test('C-02: create → list → delete round-trip', async ({ page, request }) => {
    ...
    await page.goto('/admin/human-interaction-gate');
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('page-human-interaction-gate')).toBeVisible();
    await expect(page.getByTestId(`human-interaction-gate-row-${docId}`)).toBeVisible();
  ...
  test('C-03: UI create form submits + refreshes list', async ({ page, request }) => {
    ...
    await page.getByTestId('human-interaction-gate-create-button').click();
    await expect(page.getByTestId('human-interaction-gate-form')).toBeVisible();

    const name = 'ui-form-record';
    await page.getByTestId('human-interaction-gate-form-name').fill(name);
    await page.getByTestId('human-interaction-gate-form-status').fill('active');
    await page.getByTestId('human-interaction-gate-form-notes').fill('created via UI form');
    const postResponse = page.waitForResponse(
      (r) => r.url().includes(`/api/dynamic/${INDEX}`) && r.request().method() === 'POST',
    );
    await page.getByTestId('human-interaction-gate-form-submit').click();
    const resp = await postResponse;
    expect(resp.ok()).toBeTruthy();

    await expect(page.getByTestId('human-interaction-gate-form')).toBeHidden();
  ...
  test('C-04: list or empty-state renders on load', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('page-human-interaction-gate')).toBeVisible();
    const listVisible = await page.getByTestId('human-interaction-gate-list').isVisible().catch(() => false);
    const emptyVisible = await page.getByTestId('human-interaction-gate-empty').isVisible().catch(() => false);
