/**
 * schema-registry-dag-topology-qa.spec.ts — Phase 6 Topology Visual QA (REAL TOPOLOGY)
 *
 * TVQ-01..TVQ-08 for FLOW-11 — REPLACED 2026-04-17 (Track B batch).
 *
 * Source of truth: contracts/topologies/schema-registry-dag.topology.json
 * (7 nodes, 9 edges incl. 0 terminal `to:null` markers).
 *
...
const SLUG            = 'schema-registry-dag';
...
test.describe(`${FLOW_ID} - Topology Visual QA (real topology)`, () => {
  ...
  test('TVQ-01: topology graph renders without error', async ({ page }) => {
    try { await page.goto(TOPOLOGY_URL, { timeout: 30_000 }); }
  ...
  test(`TVQ-06: SUSPENDED state on first node shows badge + resume button`, async ({ page }) => {
    ...
    try { await page.goto(`${TOPOLOGY_URL}?runId=tvq-test-run`, { timeout: 30_000 }); }
    catch { test.skip(true, 'Dev server not running'); }
    await page.waitForLoadState('networkidle');
  ...
  test('TVQ-10: DagVisualizationPage renders nodes (not Mermaid text)', async ({ page }) => {
    const dagUrl = `${BASE_URL}/dag-visualization?role=platform-admin`;
    try { await page.goto(dagUrl, { timeout: 30_000 }); }
    catch { test.skip(true, 'Dev server not running'); }
    await page.waitForLoadState('networkidle');
...
//   NODE_COUNT       = 7
//   EDGE_COUNT_RAW   = 9     (0 terminal-to-null filtered)
//   Source           = contracts/topologies/schema-registry-dag.topology.json
void EDGE_COUNT;
void TERMINAL_COUNT;
