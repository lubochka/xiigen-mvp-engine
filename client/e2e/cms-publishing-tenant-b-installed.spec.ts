...
async function assertRoleBranch(page: Page, roleCell: (typeof ROLES)[number]) {
  ...
  if (roleCell.role === 'anonymous') {
    await expect(page.getByTestId('cms-reader-subscribe')).toBeVisible();
    await expect(page.getByTestId('cms-article-read-more')).toBeVisible();
  }

  if (roleCell.role === 'public-marketplace-visitor') {
    await expect(page.getByTestId('cms-public-mkt-banner')).toBeVisible();
    await expect(page.getByTestId('cms-reader-subscribe')).toBeVisible();
  }

  if (roleCell.role === 'tenant-user') {
    await expect(page.getByTestId('cms-user-actions')).toBeVisible();
    await expect(page.getByTestId('cms-user-related')).toBeVisible();
  }

  if (roleCell.role === 'freelancer') {
    await expect(page.getByTestId('cms-fl-composer')).toBeVisible();
    await expect(page.getByTestId('cms-fl-submit-review')).toBeVisible();
  }

  if (roleCell.role === 'tenant-admin') {
    await expect(page.getByTestId('cms-admin-kpis')).toBeVisible();
    await expect(page.getByTestId('cms-admin-queue')).toBeVisible();
  }

  if (roleCell.role === 'platform-admin') {
    await expect(page.getByTestId('cms-platform-banner')).toBeVisible();
    await expect(page.getByTestId('cms-platform-published')).toBeVisible();
  }

  if (roleCell.role === 'platform-support') {
    await expect(page.getByTestId('cms-support-search')).toBeVisible();
    await expect(page.getByTestId('cms-support-result')).toBeVisible();
  }
}

test.describe('FLOW-22 Tenant B installed Tenant A CMS package visual evidence', () => {
