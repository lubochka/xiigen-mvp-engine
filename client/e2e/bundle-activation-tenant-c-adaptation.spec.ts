...
test.describe('FLOW-00 Tenant C adapted visual evidence', () => {
  test.beforeAll(() => {
    ...
    for (const cell of CELLS) {
      test(`${role} ${cell.id}`, async ({ page }) => {
        ...
        await expect(page.getByTestId('ba-tenant-adaptation')).toContainText(
          'Tessera community launch policy',
        );
        await expect(page.getByTestId('ba-tenant-adaptation-summary')).toContainText(
          'Confirm circle readiness',
        );
        await expect(page.getByTestId('ba-tenant-adaptation-summary')).toContainText(
          'Tessera community ops',
        );
        await expect(page.getByTestId('ba-tenant-adaptation-summary')).toContainText(
          'about 12 seconds',
        );
        await expect(page.getByTestId('ba-tenant-adaptation-summary')).toContainText(
          'Adapted from Northwind launch bundles',
        );
