/**
 * FLOW-34 — Marketplace Plugin Adapter Real-CRUD Round-Trip E2E
 *
 * Exercises /api/dynamic/xiigen-marketplace-plugin-adapter end-to-end.
...
const SNAP = (name: string) =>
  path.join(__dirname, '..', '..', 'docs', 'e2e-snapshots', 'marketplace-plugin-adapter', name);

const API = 'http://localhost:3000/api';
const INDEX = 'xiigen-marketplace-plugin-adapter';

test.describe('FLOW-34 — Marketplace Plugin Adapter real CRUD', () => {
  ...
  test('C-02: create → list → delete round-trip', async ({ page, request }) => {
    ...
    await page.goto('/admin/marketplace-plugin-adapter');
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('page-marketplace-plugin-adapter')).toBeVisible();
    await expect(page.getByTestId(`marketplace-plugin-adapter-row-${docId}`)).toBeVisible();
  ...
  test('C-03: UI create form submits + refreshes list', async ({ page, request }) => {
    ...
    await page.getByTestId('marketplace-plugin-adapter-create-button').click();
    await expect(page.getByTestId('marketplace-plugin-adapter-form')).toBeVisible();

    const name = 'ui-form-record';
    await page.getByTestId('marketplace-plugin-adapter-form-name').fill(name);
    await page.getByTestId('marketplace-plugin-adapter-form-status').fill('active');
    await page.getByTestId('marketplace-plugin-adapter-form-notes').fill('created via UI form');
    const postResponse = page.waitForResponse(
      (r) => r.url().includes(`/api/dynamic/${INDEX}`) && r.request().method() === 'POST',
    );
    await page.getByTestId('marketplace-plugin-adapter-form-submit').click();
    const resp = await postResponse;
    expect(resp.ok()).toBeTruthy();

    await expect(page.getByTestId('marketplace-plugin-adapter-form')).toBeHidden();
  ...
  test('C-04: list or empty-state renders on load', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('page-marketplace-plugin-adapter')).toBeVisible();
    const listVisible = await page.getByTestId('marketplace-plugin-adapter-list').isVisible().catch(() => false);
    const emptyVisible = await page.getByTestId('marketplace-plugin-adapter-empty').isVisible().catch(() => false);
