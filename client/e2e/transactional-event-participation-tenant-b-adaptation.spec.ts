...
test.describe('FLOW-09 Tenant B adapted visual evidence', () => {
  ...
  test('marketplace listing shows Tenant B adapted package', async ({ page }) => {
    ...
    await page.goto(
      '/marketplace?role=tenant-admin&hideChrome=1&showcase=transactional-event-participation-v1.0.2',
      { waitUntil: 'domcontentloaded' },
    );
    await expect(page.getByTestId('marketplace-page')).toBeVisible();
    const card = page.getByTestId(
      'marketplace-card-pkg-transactional-event-participation-northwind-exchange-ticketing',
    );
    await expect(card).toBeVisible();
    await expect(card).toContainText('Northwind Exchange Ticketing');
    await expect(card).toContainText('4-minute seat holds');
    await expect(card).toContainText('72-hour refunds');
    await page.screenshot({
      path: path.join(MARKETPLACE_ROOT, 'transactional-event-participation-v1.0.2-listing.png'),
      fullPage: true,
    });
  });
});
