/**
 * FLOW-17 Tenant B adapted package cascade evidence.
 *
 * Verifies the Northwind curated freelancer marketplace package renders the
 * expected role branches after Tenant B adapts and publishes Tenant A's package.
 */
...
function urlFor(role: (typeof ROLES)[number]): string {
  ...
  query.set('adaptedPackage', 'northwind-curated');
  query.set('packageVersion', '1.0.2');
  query.set('hideChrome', '1');
  return `/gigs/post?${query.toString()}`;
}

test.describe('FLOW-17 Tenant B adapted freelancer marketplace package', () => {
  ...
  for (const role of ROLES) {
    test(`${role} adapted package branch renders`, async ({ page }, testInfo) => {
