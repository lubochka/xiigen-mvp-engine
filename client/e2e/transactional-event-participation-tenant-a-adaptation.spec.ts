...
test.describe('FLOW-09 Tenant A adapted visual evidence', () => {
  ...
  test('marketplace listing shows Tenant A adapted package', async ({ page }) => {
    ...
    await page.goto('/marketplace?role=tenant-admin&hideChrome=1&showcase=transactional-event-participation-v1.0.1', {
      waitUntil: 'domcontentloaded',
    });
    await expect(page.getByTestId('marketplace-page')).toBeVisible();
    const card = page.getByTestId(
      'marketplace-card-pkg-transactional-event-participation-acme-fast-release-ticketing',
    );
    await expect(card).toBeVisible();
    await expect(card).toContainText('Acme Fast-Release Ticketing');
    await expect(card).toContainText('3-minute seat holds');
    await expect(card).toContainText('48-hour refunds');
    await page.screenshot({
      path: path.join(MARKETPLACE_ROOT, 'transactional-event-participation-v1.0.1-listing.png'),
      fullPage: true,
    });
  });
});
