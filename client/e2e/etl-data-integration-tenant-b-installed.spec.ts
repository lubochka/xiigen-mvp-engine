...
test.describe('FLOW-14 Tenant B installed Tenant A data integration visual evidence', () => {
    ...
    for (const cell of CELLS) {
      test(`${roleCase.role} ${cell.id}`, async ({ page }) => {
        ...
        await expect(page.getByTestId('etl-tenant-adaptation')).toContainText(
          'Northwind installed Acme enterprise data integration',
        );
        await expect(page.getByTestId('etl-tenant-adaptation-summary')).toContainText(
          '5,000-row backfill slices',
        );
        await expect(page.getByTestId('etl-tenant-adaptation-summary')).toContainText(
          'shorter reverse-sync lock windows',
        );
