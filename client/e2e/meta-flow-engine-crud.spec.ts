/**
 * FLOW-26 — Meta Flow Engine Real-CRUD Round-Trip E2E
 *
 * Exercises /api/dynamic/xiigen-meta-flow-engine end-to-end.
...
const INDEX = 'xiigen-meta-flow-engine';

test.describe('FLOW-26 — Meta Flow Engine real CRUD', () => {
  ...
  test('C-02: create → list → delete round-trip', async ({ page, request }) => {
    ...
    await expect(page.getByTestId(`meta-flow-engine-row-${docId}`)).toBeVisible();
  ...
  test('C-03: UI create form submits + refreshes list', async ({ page, request }) => {
    ...
    await page.getByTestId('meta-flow-engine-create-button').click();
    await expect(page.getByTestId('meta-flow-engine-form')).toBeVisible();

    const name = 'ui-form-record';
    await page.getByTestId('meta-flow-engine-form-name').fill(name);
    await page.getByTestId('meta-flow-engine-form-status').fill('active');
    await page.getByTestId('meta-flow-engine-form-notes').fill('created via UI form');
    const postResponse = page.waitForResponse(
      (r) => r.url().includes(`/api/dynamic/${INDEX}`) && r.request().method() === 'POST',
    );
    await page.getByTestId('meta-flow-engine-form-submit').click();
    const resp = await postResponse;
    expect(resp.ok()).toBeTruthy();

    await expect(page.getByTestId('meta-flow-engine-form')).toBeHidden();
  ...
  test('C-04: list or empty-state renders on load', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('page-meta-flow-engine')).toBeVisible();
    const listVisible = await page.getByTestId('meta-flow-engine-list').isVisible().catch(() => false);
    const emptyVisible = await page.getByTestId('meta-flow-engine-empty').isVisible().catch(() => false);
