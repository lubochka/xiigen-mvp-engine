/**
 * FLOW-45 — Cycle Chain Extension Real-CRUD Round-Trip E2E
 *
 * Exercises /api/dynamic/xiigen-cycle-chain-extension end-to-end.
...
const INDEX = 'xiigen-cycle-chain-extension';

test.describe('FLOW-45 — Cycle Chain Extension real CRUD', () => {
  ...
  test('C-02: create → list → delete round-trip', async ({ page, request }) => {
    ...
    await expect(page.getByTestId(`cycle-chain-extension-row-${docId}`)).toBeVisible();
  ...
  test('C-03: UI create form submits + refreshes list', async ({ page, request }) => {
    ...
    await page.getByTestId('cycle-chain-extension-create-button').click();
    await expect(page.getByTestId('cycle-chain-extension-form')).toBeVisible();

    const name = 'ui-form-record';
    await page.getByTestId('cycle-chain-extension-form-name').fill(name);
    await page.getByTestId('cycle-chain-extension-form-status').fill('active');
    await page.getByTestId('cycle-chain-extension-form-notes').fill('created via UI form');
    const postResponse = page.waitForResponse(
      (r) => r.url().includes(`/api/dynamic/${INDEX}`) && r.request().method() === 'POST',
    );
    await page.getByTestId('cycle-chain-extension-form-submit').click();
    const resp = await postResponse;
    expect(resp.ok()).toBeTruthy();

    await expect(page.getByTestId('cycle-chain-extension-form')).toBeHidden();
  ...
  test('C-04: list or empty-state renders on load', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('page-cycle-chain-extension')).toBeVisible();
    const listVisible = await page.getByTestId('cycle-chain-extension-list').isVisible().catch(() => false);
    const emptyVisible = await page.getByTestId('cycle-chain-extension-empty').isVisible().catch(() => false);
