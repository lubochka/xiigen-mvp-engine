/**
 * FLOW-25 — BFA Cross-Flow Governance Real-CRUD Round-Trip E2E
 *
 * Exercises /api/dynamic/xiigen-bfa-cross-flow-governance end-to-end.
...
const INDEX = 'xiigen-bfa-cross-flow-governance';

test.describe('FLOW-25 — BFA Cross-Flow Governance real CRUD', () => {
  ...
  test('C-02: create → list → delete round-trip', async ({ page, request }) => {
    ...
    await expect(page.getByTestId(`bfa-cross-flow-governance-row-${docId}`)).toBeVisible();
  ...
  test('C-03: UI create form submits + refreshes list', async ({ page, request }) => {
    ...
    await page.getByTestId('bfa-cross-flow-governance-create-button').click();
    await expect(page.getByTestId('bfa-cross-flow-governance-form')).toBeVisible();

    const name = 'ui-form-record';
    await page.getByTestId('bfa-cross-flow-governance-form-name').fill(name);
    await page.getByTestId('bfa-cross-flow-governance-form-status').fill('active');
    await page.getByTestId('bfa-cross-flow-governance-form-notes').fill('created via UI form');
    const postResponse = page.waitForResponse(
      (r) => r.url().includes(`/api/dynamic/${INDEX}`) && r.request().method() === 'POST',
    );
    await page.getByTestId('bfa-cross-flow-governance-form-submit').click();
    const resp = await postResponse;
    expect(resp.ok()).toBeTruthy();

    await expect(page.getByTestId('bfa-cross-flow-governance-form')).toBeHidden();
  ...
  test('C-04: list or empty-state renders on load', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('page-bfa-cross-flow-governance')).toBeVisible();
    const listVisible = await page.getByTestId('bfa-cross-flow-governance-list').isVisible().catch(() => false);
    const emptyVisible = await page.getByTestId('bfa-cross-flow-governance-empty').isVisible().catch(() => false);
