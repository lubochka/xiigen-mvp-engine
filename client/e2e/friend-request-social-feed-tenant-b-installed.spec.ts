...
test.describe('FLOW-07 Tenant B installed Tenant A adaptation visual evidence', () => {
  for (const role of ROLES) {
    for (const cell of CELLS) {
      test(`${role} ${cell.id}`, async ({ page }) => {
        ...
        await expect(page.getByTestId('feed-tenant-adaptation-summary')).toContainText('Acme quiet community feed');
