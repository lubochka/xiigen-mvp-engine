...
test.describe('FLOW-10 Tenant B adapted visual evidence', () => {
    ...
    for (const cell of CELLS) {
      test(`${role} ${cell.id}`, async ({ page }) => {
        ...
        await expect(page.getByTestId('review-tenant-adaptation-summary')).toContainText(
          '88% auto-approve threshold',
        );
        await expect(page.getByTestId('review-tenant-adaptation-summary')).toContainText(
          'longer-tail reputation weighting',
        );
        if (cell.lang === 'he') {
  ...
  test('marketplace listing shows Tenant B adapted package', async ({ page }) => {
    ...
    await page.goto('/marketplace?role=tenant-admin&hideChrome=1&showcase=reviews-reputation-v1.0.2', {
      waitUntil: 'domcontentloaded',
    });
    await expect(page.getByTestId('marketplace-page')).toBeVisible();
    const card = page.getByTestId('marketplace-card-pkg-reviews-reputation-northwind-trust-signal-reviews');
    await expect(card).toBeVisible();
    await expect(card).toContainText('Northwind Trust-Signal Reviews');
    await expect(card).toContainText('88% auto-approve threshold');
    await expect(card).toContainText('longer-tail reputation weighting');

    fs.mkdirSync(MARKETPLACE_ROOT, { recursive: true });
    await page.screenshot({
      path: path.join(MARKETPLACE_ROOT, 'reviews-reputation-v1.0.2-listing.png'),
      fullPage: true,
    });
  });
});
