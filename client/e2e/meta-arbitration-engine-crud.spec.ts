/**
 * FLOW-35 — Meta Arbitration Engine Real-CRUD Round-Trip E2E
 *
 * Exercises /api/dynamic/xiigen-meta-arbitration-engine end-to-end.
...
const SNAP = (name: string) =>
  path.join(__dirname, '..', '..', 'docs', 'e2e-snapshots', 'meta-arbitration-engine', name);

const API = 'http://localhost:3000/api';
const INDEX = 'xiigen-meta-arbitration-engine';

test.describe('FLOW-35 — Meta Arbitration Engine real CRUD', () => {
  ...
  test('C-02: create → list → delete round-trip', async ({ page, request }) => {
    ...
    await page.goto('/admin/meta-arbitration-engine');
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('page-meta-arbitration-engine')).toBeVisible();
    await expect(page.getByTestId(`meta-arbitration-engine-row-${docId}`)).toBeVisible();
  ...
  test('C-03: UI create form submits + refreshes list', async ({ page, request }) => {
    ...
    await page.getByTestId('meta-arbitration-engine-create-button').click();
    await expect(page.getByTestId('meta-arbitration-engine-form')).toBeVisible();

    const name = 'ui-form-record';
    await page.getByTestId('meta-arbitration-engine-form-name').fill(name);
    await page.getByTestId('meta-arbitration-engine-form-status').fill('active');
    await page.getByTestId('meta-arbitration-engine-form-notes').fill('created via UI form');
    const postResponse = page.waitForResponse(
      (r) => r.url().includes(`/api/dynamic/${INDEX}`) && r.request().method() === 'POST',
    );
    await page.getByTestId('meta-arbitration-engine-form-submit').click();
    const resp = await postResponse;
    expect(resp.ok()).toBeTruthy();

    await expect(page.getByTestId('meta-arbitration-engine-form')).toBeHidden();
  ...
  test('C-04: list or empty-state renders on load', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('page-meta-arbitration-engine')).toBeVisible();
    const listVisible = await page.getByTestId('meta-arbitration-engine-list').isVisible().catch(() => false);
    const emptyVisible = await page.getByTestId('meta-arbitration-engine-empty').isVisible().catch(() => false);
