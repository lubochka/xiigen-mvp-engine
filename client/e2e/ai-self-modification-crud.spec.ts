/**
 * FLOW-44 — AI Self-Modification Real-CRUD Round-Trip E2E
 *
 * Exercises /api/dynamic/xiigen-ai-self-modification end-to-end.
...
const INDEX = 'xiigen-ai-self-modification';

test.describe('FLOW-44 — AI Self-Modification real CRUD', () => {
  ...
  test('C-02: create → list → delete round-trip', async ({ page, request }) => {
    ...
    await expect(page.getByTestId(`ai-self-modification-row-${docId}`)).toBeVisible();
  ...
  test('C-03: UI create form submits + refreshes list', async ({ page, request }) => {
    ...
    await page.getByTestId('ai-self-modification-create-button').click();
    await expect(page.getByTestId('ai-self-modification-form')).toBeVisible();

    const name = 'ui-form-record';
    await page.getByTestId('ai-self-modification-form-name').fill(name);
    await page.getByTestId('ai-self-modification-form-status').fill('active');
    await page.getByTestId('ai-self-modification-form-notes').fill('created via UI form');
    const postResponse = page.waitForResponse(
      (r) => r.url().includes(`/api/dynamic/${INDEX}`) && r.request().method() === 'POST',
    );
    await page.getByTestId('ai-self-modification-form-submit').click();
    const resp = await postResponse;
    expect(resp.ok()).toBeTruthy();

    await expect(page.getByTestId('ai-self-modification-form')).toBeHidden();
  ...
  test('C-04: list or empty-state renders on load', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('page-ai-self-modification')).toBeVisible();
    const listVisible = await page.getByTestId('ai-self-modification-list').isVisible().catch(() => false);
    const emptyVisible = await page.getByTestId('ai-self-modification-empty').isVisible().catch(() => false);
