...
test.describe('FLOW-07 Tenant A adapted visual evidence', () => {
  ...
  test('marketplace listing shows Tenant A adapted package', async ({ page }) => {
    ...
    await page.goto('/marketplace?role=tenant-admin&hideChrome=1&showcase=friend-request-social-feed-v1.0.1', {
      waitUntil: 'domcontentloaded',
    });
    await expect(page.getByTestId('marketplace-page')).toBeVisible();
    const card = page.getByTestId('marketplace-card-pkg-friend-request-social-feed-acme-quiet-community-feed');
    await expect(card).toBeVisible();
    await expect(card).toContainText('Acme Quiet Community Feed');
    await expect(card).toContainText('0.35');
    await expect(card).toContainText('12 updates');
    await page.screenshot({
      path: path.join(MARKETPLACE_ROOT, 'friend-request-social-feed-v1.0.1-listing.png'),
      fullPage: true,
    });
  });
});
