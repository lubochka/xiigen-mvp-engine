/**
 * FLOW-16 Tenant C installed-from-B cascade evidence.
 *
 * Verifies Tessera's install of the Northwind marketplace payments package
 * keeps the role-specific checkout surfaces available without changing
 * payment-sensitive machine rules.
 */
...
test.describe('FLOW-16 Tenant C installed from Tenant B adapted package', () => {
  ...
  for (const state of STATES) {
    test(`${state.name} renders`, async ({ page }) => {
      await page.goto(
        `/checkout?role=${encodeURIComponent(state.role)}&tenant=tessera-collective&installedFrom=northwind&cascade=tessera-installed-from-b`,
      );
