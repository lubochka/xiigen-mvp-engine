/**
 * FLOW-21 — Dynamic Forms & Workflows Real-CRUD Round-Trip E2E
 *
 * Exercises /api/dynamic/xiigen-dynamic-forms-workflows end-to-end.
...
const INDEX = 'xiigen-dynamic-forms-workflows';

test.describe('FLOW-21 — Dynamic Forms & Workflows real CRUD', () => {
  ...
  test('C-02: create → list → delete round-trip', async ({ page, request }) => {
    ...
    await expect(page.getByTestId(`dynamic-forms-workflows-row-${docId}`)).toBeVisible();
  ...
  test('C-03: UI create form submits + refreshes list', async ({ page, request }) => {
    ...
    await page.getByTestId('dynamic-forms-workflows-create-button').click();
    await expect(page.getByTestId('dynamic-forms-workflows-form')).toBeVisible();

    const name = 'ui-form-record';
    await page.getByTestId('dynamic-forms-workflows-form-name').fill(name);
    await page.getByTestId('dynamic-forms-workflows-form-status').fill('active');
    await page.getByTestId('dynamic-forms-workflows-form-notes').fill('created via UI form');
    const postResponse = page.waitForResponse(
      (r) => r.url().includes(`/api/dynamic/${INDEX}`) && r.request().method() === 'POST',
    );
    await page.getByTestId('dynamic-forms-workflows-form-submit').click();
    const resp = await postResponse;
    expect(resp.ok()).toBeTruthy();

    await expect(page.getByTestId('dynamic-forms-workflows-form')).toBeHidden();
  ...
  test('C-04: list or empty-state renders on load', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await expect(page.getByTestId('page-dynamic-forms-workflows')).toBeVisible();
    const listVisible = await page.getByTestId('dynamic-forms-workflows-list').isVisible().catch(() => false);
    const emptyVisible = await page.getByTestId('dynamic-forms-workflows-empty').isVisible().catch(() => false);
