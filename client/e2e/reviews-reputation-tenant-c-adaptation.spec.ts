...
test.describe('FLOW-10 Tenant C adapted visual evidence', () => {
  for (const role of ROLES) {
    for (const cell of CELLS) {
      test(`${role} ${cell.id}`, async ({ page }) => {
        ...
        await expect(page.getByTestId('review-tenant-adaptation-summary')).toContainText(
          'community-weighted reputation decay',
        );
        if (cell.lang === 'he') {
  ...
  test('marketplace listing shows Tenant C adapted package', async ({ page }) => {
    ...
    await page.goto('/marketplace?role=tenant-admin&hideChrome=1&showcase=reviews-reputation-v1.0.3', {
      waitUntil: 'domcontentloaded',
    });
    await expect(page.getByTestId('marketplace-page')).toBeVisible();
    const card = page.getByTestId('marketplace-card-pkg-reviews-reputation-tessera-community-trust-reviews');
    await expect(card).toBeVisible();
    await expect(card).toContainText('Tessera Community-Trust Reviews');
    await expect(card).toContainText('90% auto-approve threshold');
    await expect(card).toContainText('community-weighted reputation decay');

    fs.mkdirSync(MARKETPLACE_ROOT, { recursive: true });
    await page.screenshot({
      path: path.join(MARKETPLACE_ROOT, 'reviews-reputation-v1.0.3-listing.png'),
      fullPage: true,
    });
  });
});
