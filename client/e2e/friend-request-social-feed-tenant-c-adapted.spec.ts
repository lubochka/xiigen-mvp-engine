...
const CASCADE_SEGMENT = 'tenant-c-v1.0.0';
...
test.describe('FLOW-07 Tenant C adapted visual evidence', () => {
  for (const role of ROLES) {
    for (const cell of CELLS) {
      test(`${role} ${cell.id}`, async ({ page }) => {
        ...
        await expect(page.getByTestId('feed-tenant-adaptation-summary')).toContainText(
          '0.65 recency',
        );
        await expect(page.getByTestId('feed-tenant-adaptation-summary')).toContainText(
          '0.25 relationship',
  ...
  test('marketplace listing shows Tenant C adapted package', async ({ page }) => {
    ...
    await page.goto(
      '/marketplace?role=tenant-admin&hideChrome=1&showcase=friend-request-social-feed-tenant-c-v1.0.0',
      {
        waitUntil: 'domcontentloaded',
      },
    );
    await expect(page.getByTestId('marketplace-page')).toBeVisible();
    const card = page.getByTestId('marketplace-card-pkg-friend-request-social-feed-tessera-community-bridge-feed');
    await expect(card).toBeVisible();
    await expect(card).toContainText('Tessera Community Bridge Feed');
    await expect(card).toContainText('push and email');
    await expect(card).toContainText('Northwind freshness');
    await page.screenshot({
      path: path.join(MARKETPLACE_ROOT, 'friend-request-social-feed-tenant-c-v1.0.0-listing.png'),
      fullPage: true,
    });
  });
});
