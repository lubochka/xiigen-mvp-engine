/**
 * FLOW-17 Tenant C installed-from-B cascade evidence.
 *
 * Verifies the Northwind curated freelancer marketplace package renders the
 * expected role branches for Tessera after the fork engine rewrites tenant
 * ownership during Tenant C installation.
 */
...
function urlFor(role: (typeof ROLES)[number]): string {
  ...
  query.set('installedFrom', 'northwind');
  query.set('packageVersion', '1.0.2');
  query.set('hideChrome', '1');
  return `/gigs/post?${query.toString()}`;
}

test.describe('FLOW-17 Tenant C installed from Tenant B package', () => {
  ...
  for (const role of ROLES) {
    test(`${role} installed-from-B package branch renders`, async ({ page }, testInfo) => {
