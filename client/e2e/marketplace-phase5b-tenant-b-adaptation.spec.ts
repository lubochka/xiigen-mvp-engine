...
const OUT_DIR = path.join(MARKETPLACE_DIR, 'tenant-b-v1.0.2');
...
test.describe('Flow 08 Tenant B adapted marketplace package', () => {
  test.beforeAll(() => {
    fs.mkdirSync(OUT_DIR, { recursive: true });
  });

  test('marketplace listing shows the adapted Northwind package', async ({ page }) => {
    await page.setViewportSize({ width: 1280, height: 800 });
    await page.goto('/marketplace?showcase=marketplace-v1.0.2&role=tenant-admin');
    ...
    await expect(card).toContainText('sponsor-ready partner positioning');
    await expect(card).toContainText('v1.0.2');

    await card.screenshot({
      path: path.join(MARKETPLACE_DIR, 'marketplace-v1.0.2-listing.png'),
    });
  });
