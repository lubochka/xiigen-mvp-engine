...
test.describe('FLOW-14 Tenant B adapted data integration visual evidence', () => {
    ...
    for (const cell of CELLS) {
      test(`${roleCase.role} ${cell.id}`, async ({ page }) => {
        ...
        await expect(page.getByTestId('etl-tenant-adaptation')).toContainText(
          'Northwind exchange data integration package',
        );
        await expect(page.getByTestId('etl-tenant-adaptation-summary')).toContainText(
          'exchange-partner approval windows',
        );
        await expect(page.getByTestId('etl-tenant-adaptation-summary')).toContainText(
          'reverse-sync batches',
        );
        await expect(page.getByText('Northwind v1.0.2')).toBeVisible();
