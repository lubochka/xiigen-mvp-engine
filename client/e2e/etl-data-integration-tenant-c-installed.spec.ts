...
test.describe('FLOW-14 Tenant C installed Tenant B data integration visual evidence', () => {
    ...
    for (const cell of CELLS) {
      test(`${roleCase.role} ${cell.id}`, async ({ page }) => {
        ...
        await expect(page.getByTestId('etl-tenant-adaptation')).toContainText(
          'Tessera installed Northwind exchange data integration',
        );
        await expect(page.getByTestId('etl-tenant-adaptation-summary')).toContainText(
          'inherited Northwind exchange policy',
        );
        await expect(page.getByTestId('etl-tenant-adaptation-summary')).toContainText(
          'community data programs',
        );
        await expect(page.getByText('Northwind v1.0.2 installed')).toBeVisible();
