...
export type FramerComponentType = 'frame' | 'text' | 'svg' | 'image' | 'component';
export type SharedUIRole = 'FRAME' | 'TEXT' | 'SVG' | 'IMAGE' | 'COMPONENT';
...
export interface FramerComponent {
  id: string;
  name: string;
  type: FramerComponentType;
  width: number;
  height: number;
  backgroundColor?: string | undefined;
}

export interface SharedUIElement {
  type: SharedUIRole;
  name: string;
  width: number;
  height: number;
...
export interface SharedUIStyle {
  backgroundColor: string;
  aspectRatio: string;
  uiRole: SharedUIRole;
}

export interface FramerComponentWritePayload {
  type: 'COMPONENT_RENAME';
  name: string;
  componentType: FramerComponentType;
  backgroundColor: string;
}

export interface FramerComponentNameOutput {
  element: SharedUIElement;
  style: SharedUIStyle;
  generatedName: string;
}

export interface FramerComponentWriteResult {
  written: number;
  failed: number;
  payloads: FramerComponentWritePayload[];
}
