...
Prevents silent truncation at context limit. Claude Code monitors context usage and triggers a clean session-end protocol before the window fills, rather than hitting the wall mid-task.

## The Problem

Without monitoring, Claude Code at 28% remaining context will continue a complex task, produce a truncated Section A, drop Section B entirely, and stop mid-sentence. STATE.json is not saved. The next session has no clean handoff point.
...
Claude Code does not have direct access to context percentage. Proxy indicators:
