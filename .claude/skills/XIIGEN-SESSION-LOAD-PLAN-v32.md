# XIIGEN Session Load Plan v32
## Date: 2026-04-21
## Branch: claude/vigorous-margulis
## Status: Current — supersedes v31

## What changed in v32:
##   NEW: LOAD ORDER 0-PRE — Project Knowledge Inventory (before SK-529)
##     Mandatory for all ARCHITECT sessions; any domain-relevant session
##     Lists governance docs + domain-specific skills to read before Tier-0
##   NEW: ARCHITECT SESSION COMMUNICATION CONTRACT
##     Output to Luba is prose. STATE/RECON/Q-artifacts are internal. Always.
##   CFI-11 CLOSED 2026-04-21 (via this fix-plan Phases 1-7)
##   CFI-13 OPEN (NEW): Claude.ai project custom instruction update required
##     Human action: copy XIIGEN-SESSION-START-PROMPT-v5.0.md into project settings
##     Blocking: YES — all ARCHITECT sessions until done
##   CFI-14 CLOSED 2026-04-21 (FLOW-DESIGN-SKILL-INDEX updated to v4.0+)
##   Document Registry: ARCHITECT-GUIDE bumped to v1.9 (Q-MINUS-1, THINKING mode,
##     Sections 2b/2c/8, P-A8, Mistakes 24-28); SK-529 bumped to v2.1.0 (T0-0)
##   SK-529 version in skill table updated to v2.1.0
##
## What changed in v31:
...
```json
{
  "productGoal": {
    ...
    "currentGapsRef": "CARRY-FORWARD-ISSUES.md CFI-01..CFI-14"
  }
}
```
...
| `XIIGEN-SESSION-LOAD-PLAN` | **v32 (this file)** | Rule registry, STATE schema, artifact boundaries, test counts |
| `XIIGEN-DESIGN-ARCHITECT-SESSION-GUIDE` | **v1.9** | Q-MINUS-1 + Q0 DEFERRED + THINKING mode + Sections 2b/2c/8 + P-A8 + Mistakes 24–28; Q0-Q10 + Mistakes 1-28 |
...
| `FLOW-PREP-LIBRARY` | **v6.1-FINAL NEW** | 50 guidance files (GUIDE-B01..B50) for generating complete List B documentation for any new flow. B-50 (ROLE-SCREEN-MATRIX) must be generated before B-46/B-47/B-48. Covers state files, design simulation, implementation plan, gap translation, session outputs, visualization, and role-enriched Phase 7 files. `prompt-to-claude.md` is the operational entry point. Location: `docs/flow-prep-library/` |
...
|-------|------------|---------|---------|
| SK-529 Reconnaissance Gate | 0 | v2.1.0 | Evidence before synthesis; T0-0 project skills; XIIGen Tier-0 search list |
...
- **Branch:** `claude/vigorous-margulis`
- **Pending work:** per-flow Tracks A-E execution; Track A topology creation; CFI-13 human action (start prompt update)
...
- **CFI-11** — **CLOSED 2026-04-21:** HOW-TO-USE-SKILLS.md updated to v4.5.0; FLOW-DESIGN-SKILL-INDEX updated to v4.0+ with Layers 6/6x/7/8/9/10; ARCHITECT session infrastructure created across Phases 1–7 of ARCHITECT-SESSION-INFRASTRUCTURE-FIX-PLAN v9. See SESSION-STATE-SNAPSHOT-ARCHITECT-INFRASTRUCTURE.md.
...
- **CFI-13** — **OPEN (NEW v32):** Claude.ai project custom instruction must be manually updated to v5.0. File: `XIIGEN-SESSION-START-PROMPT-v5.0.md`. Human action: copy file content into Claude.ai project settings. Prerequisite: verify all Phase 1–8 output files are in project knowledge before activating (the start prompt references files that must exist). **Blocking: YES** — all ARCHITECT sessions until done.
- **CFI-14** — **CLOSED 2026-04-21 (v32):** FLOW-DESIGN-SKILL-INDEX updated to include Layers 6/6x/7/8/9/10 skills (SK-520 through SK-542). Navigator no longer stops at SK-519.
...
0    SK-529 Reconnaissance Gate v2.1.0   (MANDATORY, ALL sessions — Tier-0 for ARCHITECT)
...
- **v32** — LOAD ORDER 0-PRE (project knowledge inventory before SK-529); ARCHITECT SESSION COMMUNICATION CONTRACT; CFI-11 CLOSED; CFI-13 OPEN (start prompt update — human action required); CFI-14 CLOSED; ARCHITECT-GUIDE → v1.9; SK-529 → v2.1.0; SESSION-START-PROMPT → v5.0; SESSION-SETUP-LIBRARY SESSION TYPE 7; RESPONSE-CONSTRUCTION-PROTOCOL skills-before-codebase + SESSION-RESTART additions
- **v31** — SK-540 v1.0.0 + SK-541 v1.0.0 + SK-542 v1.0.0 (Layer 10); planning--business-flows-registry.md; SK-539 → v1.1.0; FC-18 → v1.1.0; CODE-REVIEW → v1.8 (Gate 0g business-intent sub-check); DESIGN-REVIEW → v1.5 (Signal 13 grammar correctness); AUTHORING-GUIDE → v1.16 (Q8 + Rule 35 + SCREEN INTENT SERVED); ARCHITECT-GUIDE → v1.8 (Q10 + Mistake 23); ORIENTATION-MAP → v1.3 (Q-24/Q-25/Q-26 + UI/UX authority hierarchy); HOW-TO → v4.5.0; CFI-05 refined (Page rewrite, not routing sweep; FLOW-45 CLOSED); CFI-10 CLOSED; CFI-11 OPEN (8 items including flow-prep-library + GUIDE-B21 Rule 35 amendment); CFI-12 OPEN; 6 docs/screen-examination/ companion docs + 38 examination records registered; FLOW-PREP-LIBRARY v6.1-FINAL registered (50 guidance files, prompt-to-claude.md); SK-543 next; FC-19 next
...
- **DESIGN-ARCHITECT-SESSION-GUIDE v1.9** — Q-MINUS-1 + Q0 DEFERRED + THINKING mode + Sections 2b/2c/8 + P-A8 + Mistakes 1-28
