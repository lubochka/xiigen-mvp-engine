...
- **§4.6 expanded**: correction severity classification added — three classes (LOCAL / TRAJECTORY / SESSION-RESTART) with named detection signals for each. TRAJECTORY is the new intermediate class between point correction and whole-session restart. Detection signals for SESSION-RESTART extended with two new corpus-grounded signals. §4.4 / §4.6 override relationship clarified per class.
- **Mistake 31 added**: treating a TRAJECTORY correction as LOCAL — acknowledging the correction and continuing in the same direction with one additional action. Signature, why it fails, corpus examples, fix.
- **§9 Versioning** updated.
- **§10 Observability** updated: correction severity classification check added to STOP checklist.
- **Mistakes Catalog count** updated to 1–31.

## What changed in v2.1

- **§2b enriched**: perspective artifacts added as mandatory pre-synthesis artifacts. Each perspective now requires a named internal artifact block (IMPLEMENTATION ARTIFACT, PRODUCT INTENT ARTIFACT, PRINCIPLES ARTIFACT) with source citations, evidence-layer tags (from SK-529 v2.4.0), and a one-sentence finding. The synthesis gate upgraded from a prose reminder to a verifiable checklist: all three artifacts must exist AND each must cite only its own evidence layer before synthesis may begin.
- **Mistake 30 added**: collapsing perspectives mid-paragraph — writing Implementation, Product Intent, and Principles findings in a single paragraph. Signature, why it fails, and fix.
- **§9 Versioning** updated.
- **§10 Observability** updated: perspective artifact check added to STOP checklist.
- **Mistakes Catalog count** updated to 1–30.

## What changed in v2.0

- **Q-MINUS-2 added** (before Q-MINUS-1): goal-type classification — REMEDIATION / GREENFIELD / HYBRID. Determines reading order before any skill is loaded. REMEDIATION reads design intent first; GREENFIELD reads code first; HYBRID declares which parts are which. Detection signals and reading-order consequences for each type.
- **Mistake 29 added**: wrong reading order for goal type — starting with code when the goal is REMEDIATION, or starting with design intent when the goal is GREENFIELD. Fix: classify at Q-MINUS-2 before touching any file.
- **§9 Versioning** updated.
- **§10 Observability** updated: Q-MINUS-2 artifact added to STOP checklist.
- **SK-529 version reference** in Q2 updated to v2.3.0.
- **Mistakes Catalog count** updated to 1–29.
...
## 2. Session Start Checklist — Q-MINUS-2 through Q10

Every architect session answers these questions before producing substantive output. The answers are internal artifacts; what the user sees is the response that rests on them.

### Q-MINUS-2 — What type of architectural task is this? *(NEW v2.0)*

Classify the user's request before loading any skill or reading any file. Goal type determines reading order. Reading in the wrong order for the goal type produces the wrong evidence base before any synthesis begins.

Classify into exactly one of three types:

**REMEDIATION**
Signal phrases: "where did it go wrong", "find the gap", "what needs to be fixed", "why doesn't X work", "what was supposed to happen", "where did we go in the wrong direction."
Reading order: design intent FIRST, then implementation.
  First files: DECISIONS-LOCKED.md, DD-xxx records in docs/sessions/FLOW-XX/,
               STEP-1-INVARIANTS, RECONCILIATION-STATE.md
  Last files: source code, service files, bootstrapper
Logic: in REMEDIATION the design is the standard; the implementation is measured against it. Reading the code first produces archaeology — findings about what is, not findings about what diverged from intent. The divergence is only visible when you hold both the design and the implementation at the same time, in that order.

**GREENFIELD**
Signal phrases: "design X", "how should X work", "build X", "plan X", "what architecture supports X", no prior design mentioned or referenced.
Reading order: existing code and state FIRST, then intent documents.
  First files: flow current states, coupling maps, service inventories,
               INFRASTRUCTURE-FLOWS-STATE-v6.json
  Last files: design decisions, product specs
Logic: in GREENFIELD the existing code establishes constraints; the new design extends from what exists. Reading design documents first produces a plan that ignores existing implementation reality.

**HYBRID**
Signal phrases: "extend X to support Y", "X exists but needs to do Z", task has both "fix what's broken" and "add new capability" components, or the scope explicitly covers both existing and new behavior.
Declaration required: state which parts of the task are REMEDIATION and which are GREENFIELD, in writing, before reading begins.
Reading order: REMEDIATION parts follow REMEDIATION order; GREENFIELD parts follow GREENFIELD order. Do not merge the reading sequences — the REMEDIATION reads complete before GREENFIELD reads begin, or vice versa depending on which is the prerequisite.

**Declaration format** (internal artifact, not shown to Luba):
```
Q-MINUS-2: [REMEDIATION | GREENFIELD | HYBRID]
Signal: "[exact phrase from user's message that determined this]"
Reading order consequence: [one sentence — what gets read first and why]
[If HYBRID: which parts are REMEDIATION and which are GREENFIELD, one sentence each]
```

**Why this question precedes Q-MINUS-1:** Q-MINUS-1 asks which skills to load. The answer depends on goal type — REMEDIATION sessions load design-intent skills (DECISIONS-LOCKED, DD-xxx lookup) before codebase-survey skills; GREENFIELD sessions do the reverse. Classifying the goal type first makes Q-MINUS-1 deterministic.

**The failure this question prevents:** Both sessions in the module-separation transcript corpus classified a REMEDIATION goal ("where did we go wrong") as GREENFIELD and spent multiple rounds reading code before reading design intent. Each round produced a coherent-looking analysis of what the code contained — which was not what was asked. The question was "where does the implementation diverge from the design?" That question requires reading the design before the implementation, not after. Q-MINUS-2 installs that classification as the first step, before any file is touched.
...
### Q2 — Skills loaded
For ARCHITECT: SK-529 v2.3.0, SK-535, SK-536, SK-538 v1.2.0, SK-530, RESPONSE-CONSTRUCTION-PROTOCOL v1.0, SK-528 if the work touches a pipeline stage. List each with version and one sentence of what it governs.
...
**The synthesis gate:** *(enriched v2.1 — artifact requirement added)*
...
Each perspective must produce a named internal artifact block before synthesis begins.
These blocks are internal — not shown to Luba unless she asks. What reaches Luba
is the synthesis that cites them.

```
IMPLEMENTATION ARTIFACT
  Sources cited: [list of file:line references from tool reads — IMPLEMENTATION
                 or TEST layer only, per SK-529 v2.4.0 evidence-layer tags]
  Finding: [one sentence — what the code actually does in this domain,
            stated without reference to what it should do]
  SK-530 score: [must reach ARCHITECT threshold of 11 points before this
                artifact closes — 5+ file:line + 3+ integer counts + 3+ verbatim excerpts]

PRODUCT INTENT ARTIFACT
  Sources cited: [list of authoritative decision records — DESIGN_DOC layer only:
                 DECISIONS-LOCKED.md entries, DD-xxx records, STEP-1-INVARIANTS,
                 locked decisions in session files]
  Finding: [one sentence — what the system was designed to do in this domain,
            stated without reference to what the code currently does]
  If no decision record exists for this question: state explicitly —
  "No decision record found at [searched paths]. Intent cannot be determined
  from authoritative sources. DESIGN_DOC evidence is absent for this domain."

PRINCIPLES ARTIFACT
  Sources cited: [list of principles consulted — DESIGN_DOC layer: DECISIONS-LOCKED,
                 DNA rules 1–9, iron rules, P1–P22]
  Finding: [one sentence — whether the hypothesis violates any principle,
            stated without domain context or implementation details]
  If no principle is relevant: state "No applicable principle found in
  DECISIONS-LOCKED or DNA rules for this hypothesis."
```

**Synthesis gate checklist** (all must pass before synthesis paragraph is written):
```
□ IMPLEMENTATION ARTIFACT exists with at least one file:line citation
□ IMPLEMENTATION ARTIFACT cites only IMPLEMENTATION or TEST layer excerpts
□ IMPLEMENTATION ARTIFACT SK-530 score ≥ 11
□ PRODUCT INTENT ARTIFACT exists (or absence of decision record declared explicitly)
□ PRODUCT INTENT ARTIFACT cites only DESIGN_DOC layer excerpts
□ PRODUCT INTENT ARTIFACT finding contains no implementation observations
□ PRINCIPLES ARTIFACT exists (or no-applicable-principle stated explicitly)
□ PRINCIPLES ARTIFACT finding contains no file names or implementation details
□ All three artifacts are separate blocks — no combined "Implementation/Intent" paragraph
```

A synthesis paragraph may draw from all three artifacts. But it may only be written
after all three artifacts exist as separate blocks with layer-tagged citations.
The separation is enforced in writing order: write IMPLEMENTATION ARTIFACT completely,
stop, write PRODUCT INTENT ARTIFACT completely, stop, write PRINCIPLES ARTIFACT
completely, stop, then write synthesis.
...
### 4.6 Correction severity classification *(expanded v2.2)*

Before responding to any correction, classify its severity. The classification determines the response shape. Applying the wrong response shape to a correction is itself a failure — it produces a response that addresses the correction's surface while leaving the underlying problem intact.

**Three classes:**

---

**LOCAL** — this specific claim or fact was wrong; everything else holds.

Detection signals:
- User corrects a specific fact: "that file doesn't exist at that path"
- User corrects a specific count: "it's 7, not 4"
- User corrects a specific quote or attribution: "I said X, not Y"
- User confirms the direction but fixes a detail: "yes, but the service name is..."

Response: adjust the specific claim. Continue the current trajectory. Do not restart. §4.4 applies.

---

**TRAJECTORY** — the session's direction is wrong, not one claim. The evidence gathered may still be valid; the synthesis frame built on that evidence is not.

Detection signals:
- "you went too fast" / "you're jumping to conclusions" (direction of synthesis, not a specific fact)
- "you're still [doing the same thing]" — the correction names a recurring behavior, not a one-time error
- "you have the information but you're not using it correctly" — evidence is present but synthesis frame misapplies it
- Correction arrives after the session has produced a plan, synthesis, or structured output — the whole output is in the wrong frame
- User restates the original question verbatim, unchanged — the session answered a different question
- "seems a little bit overconfident" when the issue is not a single claim but the basis for all claims

Response: stop the current synthesis trajectory. Restate what the session's goal actually is at the level the user described. Do not restart reconnaissance — the reads are valid. Restart synthesis from the correct frame. Do not add one read and continue producing in the prior frame.

§4.4 ("don't restart from scratch") applies to LOCAL. **§4.4 does NOT apply to TRAJECTORY.** The synthesis restarts. The file reads do not.

---

**SESSION-RESTART** — the session's fundamental assumption was wrong. Prior trajectory ends completely.

Detection signals (original §4.6 signals, preserved):
...
Detection signals (NEW v2.2 — corpus-grounded):
- "if you had examined X, you would have found..." → the session's evidence scope was wrong at the premise level, not just in synthesis frame
- "the question is [fundamentally different than what you answered]" → the goal-type classification (Q-MINUS-2) was wrong; the session answered a GREENFIELD question when the goal was REMEDIATION or vice versa
- A TRAJECTORY correction arrives for the second time with no change in the session's behavior → the session is incorporating the correction linguistically but not applying it structurally; it has exceeded TRAJECTORY tolerance

Response (original §4.6 response text, preserved):
...
§4.4 does NOT apply to SESSION-RESTART. §4.6 overrides §4.4 for SESSION-RESTART.

---

**Classification declaration format** (internal, Step 3 of RESPONSE-CONSTRUCTION-PROTOCOL):

```
Correction: "[verbatim user words]"
Severity class: [LOCAL | TRAJECTORY | SESSION-RESTART]
Evidence for class: [one sentence — which detection signal applies]
Response shape: [adjust claim and continue | restart synthesis frame | restart session]
```

If severity is ambiguous between TRAJECTORY and SESSION-RESTART: classify as SESSION-RESTART. The cost of an unnecessary restart (one round) is lower than the cost of applying TRAJECTORY response to a SESSION-RESTART correction (the same failure recurs).
...
### Mistake 29 — Wrong reading order for goal type *(NEW v2.0)*

**Trigger:** A REMEDIATION session (user asks "where did it go wrong", "what was supposed to happen", "find the gap") starts by reading source code and service files before reading design intent documents.

**Signature:** First tool calls in the session open `*.service.ts`, `*.controller.ts`, or flow directory listings before opening `DECISIONS-LOCKED.md`, `DD-xxx` records, `STEP-1-INVARIANTS`, or `RECONCILIATION-STATE.md`. The session then produces findings about what the code contains — which is not the question. The question was where the code diverges from the design.

**Example from corpus:** Module-separation session 1. User asked "where did we go in the wrong direction and what needs to be fixed." Session read FLOW-34, FLOW-41, FLOW-47 source files and concluded "the direction forward is not design — it is remediation of six documented gaps." This was an implementation-first reading of a design-first question. The session had the facts about what the code contained. It did not have the standard against which to measure the divergence. That standard was in the design documents it had not read. Four correction cycles followed before the session read design intent first.

**Why it happens:** GREENFIELD sessions — "design X", "build X" — correctly start with code surveys. REMEDIATION questions use similar grammatical structure: "how does X work" sounds like GREENFIELD; "where did X go wrong" is REMEDIATION but uses the same opening framing. Sessions default to GREENFIELD reading order because that is the more common task. Without Q-MINUS-2 classification, the default applies to REMEDIATION tasks.

**Fix:** Run Q-MINUS-2 before any file is opened. Classify as REMEDIATION, GREENFIELD, or HYBRID. For REMEDIATION: first files are DECISIONS-LOCKED.md, DD-xxx design decision records, STEP-1-INVARIANTS, RECONCILIATION-STATE.md. Source code reads come after the design standard is established. The divergence between design and implementation is only visible when both are held simultaneously — and the design must be the anchor, not the implementation.

### Mistake 30 — Collapsing perspectives mid-paragraph *(NEW v2.1)*

**Trigger:** A synthesis paragraph that draws on Implementation, Product Intent, and Principles evidence simultaneously — "The code does X (Implementation), which is inconsistent with the design intent Y (Product Intent), which also violates Principle Z (Principles)" — all three in one paragraph, written as a single flowing analysis.

**Signature:** The paragraph begins with an implementation observation, pivots to a design intent comparison, then makes a principles judgment. Each sentence is individually correct. Together they are a single-voice analysis labeled as three-perspective analysis. The author cannot hold three separate evidence filters simultaneously — one perspective absorbs the others, usually the most recently read one.

**Why it happens:** The three-perspective structure feels complete when described: "I considered implementation, intent, and principles." The artifact requirement in §2b makes the failure visible: if no IMPLEMENTATION ARTIFACT block exists as a separate named section before the synthesis paragraph, the perspective was not separated — it was narrated. Narration is not separation.

**Why it fails:** The synthesis paragraph produced by collapsing perspectives is shaped by whichever perspective was read most recently. In session 1 of the module-separation corpus, the most recently read evidence was code — so the synthesis concluded "these are implementation gaps that need remediation." The product intent perspective (what was the design supposed to do) was folded into the same paragraph and came out as a supporting clause, not a separate finding. If product intent had been examined first and separately, the conclusion would have been different: the code diverged from the design, not the other way around.

**Fix:** Write IMPLEMENTATION ARTIFACT completely. Stop. Write PRODUCT INTENT ARTIFACT completely. Stop. Write PRINCIPLES ARTIFACT completely. Stop. Then write the synthesis paragraph that references all three by name. The separation is in the writing order. If you cannot write each artifact block separately without referencing the other perspectives, you have not separated the perspectives — you have interleaved them.

**Detection at STOP:** §10 Observability checklist includes: "All three perspective artifact blocks present as separate named sections with layer-tagged citations." If any artifact block is absent, the synthesis paragraph cannot have been written correctly. The STOP does not fire until all three blocks exist.

### Mistake 31 — Treating a TRAJECTORY correction as LOCAL *(NEW v2.2)*

**Trigger:** User says "you're still going too fast" / "you're concluding before examining all flows" / "you answered the wrong question." Session responds "you're right, here is what I missed: [one additional read]" and then continues in the same direction with slightly more evidence.

**Signature:** The correction is acknowledged in one sentence. The next paragraph continues the prior synthesis frame with one adjustment. The output shape is: prior synthesis body + correction acknowledgment + one fix + continuation. The correction becomes a paragraph, not a direction change.

**Why it is TRAJECTORY, not LOCAL:** LOCAL corrections name a specific wrong claim. TRAJECTORY corrections name a wrong direction. "You're still going too fast" does not mean "your count of 14 topology files is wrong" — it means the session's entire synthesis posture is wrong. Acknowledging it and adding one read does not change the synthesis posture. It changes one data point inside the same wrong posture.

**Corpus evidence:** Module-separation session 1. Correction: "You run again with conclusion again without examining all the flows." Session response: acknowledged, read FLOW-41..44 additionally, then produced a new five-phase plan from the extended evidence. The posture — synthesize from partial evidence and produce conclusions — was unchanged. The same correction arrived again two rounds later. The TRAJECTORY correction was treated as LOCAL four consecutive times before the session finally restarted the synthesis frame. Each LOCAL treatment produced a round of work that had to be discarded.

**Why it happens:** LOCAL is the cheaper response. It preserves the prior work and adds one correction. TRAJECTORY requires discarding the synthesis frame and restarting from the evidence. The temptation is to treat every correction as LOCAL because LOCAL preserves more and costs less. The cost of this choice is paid in subsequent correction cycles — the same TRAJECTORY correction arrives repeatedly because the direction was never actually corrected.

**Detection:** "You're still X" is the canonical TRAJECTORY signal. "Still" indicates the correction was already given and the behavior continued. A correction beginning with "still" cannot be LOCAL — LOCAL corrections address new failures, not repeated ones.

**Fix:** Classify as TRAJECTORY before responding. Stop the synthesis frame — not one claim in it, the frame itself. Restate: "The synthesis direction was wrong. The reads are valid. The correct frame is: [restate the user's actual question at the level they described it]." Then rebuild synthesis from the existing evidence under the correct frame. Do not add more reads before reframing — adding evidence to a wrong frame produces wrong synthesis with more evidence.

---

## 7. When you notice you're doing one of these

1. **Scan** — read draft against Mistakes 1–31. Note any hit.
...
- v2.0 — Q-MINUS-2 added (goal-type classification — REMEDIATION / GREENFIELD / HYBRID;
    reading-order consequences per type; detection signals; declaration format; rationale
    from module-separation session corpus); checklist section header updated to
    "Q-MINUS-2 through Q10"; Q2 SK-529 version reference updated to v2.3.0;
    Mistake 29 added (wrong reading order for goal type — REMEDIATION session reads
    code before design intent; corpus example cited; fix: Q-MINUS-2 classification
    before first file read); §7 Mistakes Catalog scan updated to 1–29; §10 Observability
    updated with Q-MINUS-2 artifact check
- v2.1 — §2b synthesis gate enriched: three perspective artifact blocks (IMPLEMENTATION
    ARTIFACT, PRODUCT INTENT ARTIFACT, PRINCIPLES ARTIFACT) added as mandatory named
    internal artifacts, each with source citations, evidence-layer tags (SK-529 v2.4.0),
    one-sentence finding, and prohibited-source constraints; SK-530 threshold check added
    to IMPLEMENTATION ARTIFACT; nine-item synthesis gate checklist added (all must pass
    before synthesis paragraph is written); writing-order discipline stated (write each
    artifact completely before starting the next); Mistake 30 added (collapsing
    perspectives mid-paragraph — signature, why it fails with module-separation corpus
    example, fix, detection at STOP); §7 Mistakes Catalog scan updated to 1–30;
    §10 Observability updated with perspective artifact check
- v2.2 — §4.6 expanded: correction severity classification replaces the two-class
    (point / whole-session) model with a three-class model (LOCAL / TRAJECTORY /
    SESSION-RESTART); TRAJECTORY class added as the intermediate — session direction
    wrong, synthesis restarts, file reads valid; detection signals defined for each
    class; two new SESSION-RESTART detection signals added (evidence scope wrong at
    premise level; goal-type classification was wrong; TRAJECTORY correction arrives
    twice with no behavioral change); classification declaration format added for
    Step 3 of RESPONSE-CONSTRUCTION-PROTOCOL; ambiguity tie-break rule (prefer
    SESSION-RESTART over TRAJECTORY when uncertain); §4.4 override relationship
    clarified per class; Mistake 31 added (treating TRAJECTORY as LOCAL — signature,
    why it is TRAJECTORY, corpus evidence from module-separation session 1 with four
    consecutive LOCAL misclassifications, why it happens, fix); §7 Mistakes Catalog
    scan updated to 1–31; §10 Observability updated with correction severity
...
- Q-MINUS-2 artifact present: goal type declared (REMEDIATION / GREENFIELD / HYBRID), signal phrase quoted, reading order consequence stated
- All three perspective artifact blocks present as separate named sections with layer-tagged citations (IMPLEMENTATION ARTIFACT, PRODUCT INTENT ARTIFACT, PRINCIPLES ARTIFACT), or session declared no structural hypothesis requiring three-perspective examination
- If any correction arrived this session: §4.6 classification artifact present in Step 3 of RESPONSE-CONSTRUCTION-PROTOCOL for each correction (severity class, detection signal, response shape)
...
- Mistakes Catalog scan ran (Mistakes 1–31); no hits that didn't get revised
