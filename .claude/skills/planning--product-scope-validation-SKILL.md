...
## STEP 4b — ≥2 SUB-OUTPUTS + CONTRASTIVE INPUTS (G02 universal addition from llm_mvp_core)

Steps 2–4 above can pass on a single happy-path call. The core
`product-scope-validation` standard adds two hard requirements that turn this from
single-output smoke into real product validation:

**(1) The scenario MUST exercise at least two sub-outputs.** A scenario that
touches only one service output is a smoke test, not product validation. The
end-to-end scenario must chain ≥2 distinct downstream outcomes (e.g. *member
record ACTIVE* AND *API key issued* AND *OnboardingCompleted consumed by a Wave-2
flow*). If your scenario asserts only one terminal artifact, widen it until ≥2
sub-outputs are checked, or record why only one exists.

**(2) Validate business outcomes with CONTRASTIVE (negative) inputs.** A correct
output for one input is not proof of business logic — a hardcoded return produces
it too. For each business outcome, run at least one contrasting input that MUST
produce a different result, and assert the difference:

```
Positive input  → expected outcome A   (e.g. valid email → ACTIVE member + key)
Contrastive input → expected outcome B (e.g. duplicate email → idempotent no-op, NO second key)
Contrastive input → expected outcome C (e.g. malformed preferences → typed Result failure, NO member)
```

If the positive and contrastive inputs produce the SAME observable result, the
outcome is not actually being computed — flag it BROKEN_DEPENDENCY even if the
happy path "passes". TS-adaptation: assert these with `npx jest` integration tests
and/or Playwright e2e against the real NestJS endpoint and React flow; the typed
`DataProcessResult<T>` / `Result<T>` discriminator is the contrast you assert on,
not the HTTP status. The Product Validation Report's SCENARIO RESULTS table gains
a `Contrastive input → different result?` column, and the verdict cannot be
PRODUCT-READY unless every business outcome shows a passing contrastive row.
...
❌ Scenario that exercises only ONE sub-output (smoke, not product validation)
❌ Asserting only the happy path with no contrastive/negative input (a hardcoded return passes too)
```
