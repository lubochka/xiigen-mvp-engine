...
### Guard tests assert the SPECIFIC domain error code (universal)

A guard test must prove the **correct guard** fired, not just that *some* failure
happened. HTTP status and record counts are necessary but not sufficient — assert
the specific `DataProcessResult.errorCode` (behavioral-assertion-gate; mirrors
`test-integrity` Rule 6 and `bug-to-tests` AP-5):

```typescript
// strengthened R-02: assert the exact domain error code, not only isSuccess / HTTP 409
const result = await registrationService.register({ email: 'existing@example.com' });
expect(result.isSuccess).toBe(false);
expect(result.errorCode).toBe('EMAIL_ALREADY_EXISTS');  // ← the guard identity, the load-bearing assertion
// (HTTP 409 and "exactly one member record" remain, but errorCode is what proves
//  THIS guard — a different bug could also yield 409 with a different code.)
```

Rules:
- `expect(result.isSuccess).toBe(false)` ALONE is a behavioral-assertion failure for
  a guard scenario — any unrelated failure satisfies it.
- Each UML guard `[X]` maps to one concrete `errorCode`; the test asserts that code.
- mvp `DataProcessResult<T>` idiom: success via `result.isSuccess`, payload via
  `result.data`, guard identity via `result.errorCode`
  (`server/src/kernel/data-process-result.ts`).

### Scenarios ARE the Gate-B acceptance conditions (universal)

The happy/guard/edge scenarios written here are not documentation — they are the
flow's **Gate-B acceptance conditions**. A flow does not pass Gate B until every
scenario in its `SCENARIO-MANIFEST` is PASS (or an explicitly justified N/A):

```
Gate-B for FLOW-XX = every SCENARIO-MANIFEST row has status PASS
  acceptance command per row: `npx jest --testPathPattern=<scenario>` (or the .sh stub)
  a single guard scenario at FAIL ⇒ Gate B is NOT met (no "happy path passes, ship it")
```

`SCENARIO-MANIFEST.md` tracks per-scenario **status** so Gate B is mechanically
checkable:

```markdown
| Scenario | Type  | Source UML fragment        | Acceptance command                         | Status |
|----------|-------|----------------------------|--------------------------------------------|--------|
| R-01     | happy | main flow                  | bash acceptance-tests/FLOW-01/R-01.sh      | PASS   |
| R-02     | guard | [Email Already Exists]     | jest --testPathPattern=R-02-duplicate      | PASS   |
| R-03     | guard | [Token Expired]            | jest --testPathPattern=R-03-token-expired  | FAIL   |  ← Gate B blocked
| R-05     | alt   | [SSO Path]                 | jest --testPathPattern=R-05-sso            | N/A: SSO not enabled for this tenant (reason required) |
```

A scenario authored BEFORE implementation legitimately starts at status FAIL (the
acceptance criterion the code must satisfy); it flips to PASS when the engine is
correct — never by weakening the scenario's assertion (Iron Rule).
