...
3. *(NEW v2.2.0, extended v2.5.0)* **Every synthesis claim that asserts something "works", "exists as implemented", or "is running" cites at least one excerpt tagged IMPLEMENTATION or TEST.** A claim backed only by DESIGN_DOC excerpts may assert intent, not existence. If the only evidence for a "works" claim is DESIGN_DOC layer, the session must choose one of three paths:

   **Path A — Reword (always available):** reword the claim to assert intent, not existence.
   "The AdapterDeploymentBridge pushes..." → "The AdapterDeploymentBridge was designed to push..."
   Use when the implementation reality is not needed to answer the session's question.

   **Path B — Read the file (Claude Code only):** run the grep or view command, record an
   IMPLEMENTATION excerpt. The claim is promoted to PASS status.
   Use in Claude Code sessions where bash is available.

   **Path C — HANDOFF block (web sessions, or when claim is worth verifying):** *(NEW v2.5.0)*
   When the session cannot produce IMPLEMENTATION evidence itself (web session, no bash access)
   and the claim matters enough to verify rather than reword:

   ```
   ## IMPLEMENTATION EVIDENCE NEEDED — Gate 3 Handoff

   Claim pending verification:
     "[exact synthesis claim as written]"

   Why it matters:
     [one sentence — what downstream decision depends on whether this is true]

   Command to run in Claude Code:
     [exact bash command — grep, cat, or find — that will confirm or deny]
     Example: grep -n "pushToGitHub\|deployToRepo" server/src/engine/flows/FLOW-XX/**.service.ts

   Expected CONFIRM signal:
     [what output means the claim is TRUE]
     Example: "At least one service file contains pushToGitHub or equivalent call"

   Expected DENY signal:
     [what output means the claim is FALSE]
     Example: "Zero hits — the push behavior does not exist in implementation"

   Claim status: PENDING — not in synthesis until Claude Code session returns result.
   ```

   The ⛔ STOP fires with the HANDOFF block visible. The claim is excluded from synthesis
   or marked as PENDING. The Claude Code session that consumes the HANDOFF runs the command,
   returns CONFIRMED or DISCONFIRMED, and the web session resumes with IMPLEMENTATION evidence.

   **Gate 3 does not close the synthesis. It routes claims that need verification to the layer
   that can verify them.** A Gate 3 failure is not a dead end — it is a handoff signal.

   Corpus origin: module-separation examination sessions ran indefinitely (H8 — going in circles)
   because Gate 3 rejected "works" claims but provided no path forward for web sessions that
   could not run bash. The session rewrote claims as "was designed to" (Path A) and kept
   examining more DESIGN_DOC evidence, accumulating plan versions v2.0 through v4.9 without
   any claim being verified against code. Path C closes this loop.
...
### Example D — FAIL with correct routing (evidence-layer violation) *(v2.2.0, extended v2.5.0)*

- **Session type:** ARCHITECT (web session — no bash access) — **Threshold met:** YES (20/8/12)
...
**v2.2.0 behavior (REJECTED — dead end for web sessions):**
- Required fix: Reword to "was designed to push..." OR read the service file.
- Problem: web session has no bash access. "Read the service file" is unavailable.
  Session rewrites claim as "was designed to push" and continues examining DESIGN_DOC evidence.
  Plan grows from v2.0 to v4.9 with no claim ever verified against code.

**v2.5.0 behavior (HANDOFF — routes to Claude Code):**
Session produces:
```
## IMPLEMENTATION EVIDENCE NEEDED — Gate 3 Handoff

Claim pending verification:
  "The AdapterDeploymentBridge pushes generated code to external repos via GitHub API."

Why it matters:
  If false, the module portability design rests on a service that doesn't exist —
  the entire adaptation pipeline design is based on a DESIGN_DOC-only premise.

Command to run in Claude Code:
  grep -rn "pushToGitHub\|deployToRepo\|createRepository\|git.push\|octokit" \
    server/src/engine/flows/*/adapter*.service.ts \
    server/src/engine/flows/*/deployment*.service.ts

Expected CONFIRM signal:
  At least one service file contains a GitHub API push call or equivalent

Expected DENY signal:
  Zero hits — the push behavior does not exist in current implementation
```
- **Synthesis status:** claim is PENDING, excluded from this session's synthesis
- **What happens next:** Claude Code session runs the command, returns CONFIRMED or DISCONFIRMED
- **Verdict:** STOP with HANDOFF block. Not REJECTED. The claim is routed, not abandoned.
...
- **Gate 3 routing to Claude Code (NEW v2.5.0):** when Gate 3 fails in a web session
  (no bash access), Path C produces a HANDOFF block specifying the exact command a Claude
  Code session must run to produce IMPLEMENTATION evidence. The web session closes with a
  HANDOFF document. Claude Code consumes it, runs the command, returns CONFIRMED or
  DISCONFIRMED. This closes the loop that previously caused web sessions to rewrite claims
  as "was designed to" indefinitely rather than verifying them. See gate 3 Path C for the
...
| Gate 3 failure with no path forward — web session circles indefinitely *(v2.5.0)* | Gate 3 Path C HANDOFF block routes unverifiable claims to Claude Code instead of forcing reword-and-continue; synthesis paused pending verification |
...
| Prior session behavioral corrections not loaded *(v2.5.0)* | Tier-0 T0-4.5 reads BEHAVIORAL-CORRECTIONS-REGISTRY.md at session start — BC-001..BC-010 as indexed rules, not buried in transcripts |
...
| T0-4.5 | `docs/sessions/BEHAVIORAL-CORRECTIONS-REGISTRY.md` | Full file | 1 file read | Repeating behavioral failures from prior sessions (BC-001..BC-010) |
...
**Total Tier-0 contribution:** 9 file reads (45% of ARCHITECT threshold) + 2 grep counts (25% of ARCHITECT threshold).
...
- **v2.5.0** — Gate 3 routing for web sessions (2026-04-23): gate 3 extended from two-option fix (reword OR read file) to three-path protocol — Path A (reword, always available), Path B (read file, Claude Code only), Path C (HANDOFF block, web sessions or when claim warrants verification not rewording); HANDOFF block template added with claim, rationale, exact command, confirm/deny signals, PENDING status; Example D updated to show Path C as the correct web-session response; §8 integration note added (Gate 3 routing to Claude Code); §9 two new failure-mode rows (Gate 3 dead-end, behavioral corrections not loaded); §10.1 T0-4.5 added — BEHAVIORAL-CORRECTIONS-REGISTRY.md read immediately after DECISIONS-LOCKED; Tier-0 total updated from 8 to 9 file reads (45% of ARCHITECT threshold). Corpus origin: H8 (examination sessions went in circles without a path from DESIGN_DOC evidence to IMPLEMENTATION verification) confirmed in April 2026 session corpus.
