...
description: >
  ...
  Transforms server-only plans (v4 and earlier) into complete Mode C plans
  that include event contracts, client state maps, E2E test matrices, and
  observability. Use for any user-facing flow (FLOW-01 through FLOW-24)
  before producing session files.
author: luba
updated: "2026-03-20"
priority: HIGH
triggers:
...
Pass 7 — GENESIS PROMPT + ARBITER UPDATES
  Input: existing genesis prompts (task-delta format)
  Output: Mode C section added, Phase A/integration gate steps updated
  Mode C section to add to each genesis prompt:
    CONSUMES: [event, schema ref, timing]
    EMITS: [event, schema ref, SLA]
    LANGUAGE TARGET: python | typescript | go
    IMPLEMENTATION REQUIREMENTS: queue-only, no HTTP coupling
    INTEGRATION BOUNDARY: INJECTABLE | PLATFORM-ONLY per factory
  Files updated: flow reference plan genesis prompts
  New Phase A steps: NEW-A1 through NEW-A6
...
## Validation — 15-Item Checklist (run SK-418 after all passes)
...
V8   Genesis prompts have Mode C section (CONSUMES/EMITS/BOUNDARY)
