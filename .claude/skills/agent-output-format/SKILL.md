...
description: >
  Enforces that deliverables produced during web sessions (claude.ai with Opus or
  Sonnet) are formatted for their STATED CONSUMER — typically Claude Code (Opus).
...
metadata:
  ...
  created: "March 2026"
  updated: "March 2026"
  environment: "Web sessions (claude.ai) — Opus and Sonnet models"
  failure_it_prevents: >
    Analysis drift — when deep investigation causes the stated output requirement
    ("plans for Claude Code") to be replaced by human-readable analysis documents
    that Claude Code treats as deliverables to commit, not instructions to execute.
---

# Agent Output Format Skill v2.0
## For: Web Sessions (claude.ai, Opus/Sonnet) producing work for Claude Code

---

## The Problem This Skill Solves

In web sessions, when the user says "make a plan for Claude Code to execute,"
...
reference materials, but they are NOT executable by Claude Code.

Claude Code receiving a 479-line analysis document will:
- Commit it as a file ✓
...
from the stated consumer (Claude Code) toward the actual audience (the human in
the web chat).
...
- The user says "make a plan for Claude Code" or similar
...
- The user mentions "Claude Code", "Opus", "autonomous", "self-implementing"
...
If the user said "make plans for Claude Code" → every plan file must be
formatted so Claude Code can execute it step by step.
...
Claude Code reads this for understanding but does NOT execute it.
...
Machine-readable JSON. Tells Claude Code which session is current,
what's pending, what's complete. Claude Code loads this FIRST.
...
**Claude Code should NEVER receive a single document that mixes analysis
...
- **Gate A:** Automated FC checks (12 failure classes — see plan-review-skill)
...
## Execution File Format (What Claude Code Can Actually Follow)
...
Claude Code loads this file → reads `current_session` → opens that session's
...
Before creating any file that Claude Code will consume, verify:

- [ ] **Consumer identified.** Who executes this — Claude Code or human?
- [ ] **Format matches consumer.** If Claude Code: numbered steps, exact paths,
...
- [ ] **One session per file.** Claude Code should open ONE file and execute it.
      Not scan a multi-session plan to find its current session.
- [ ] **State file exists.** Claude Code needs to know which session is current
...
Claude Code's reaction: git add → git commit → done
```
...
Claude Code's reaction: "Where do I start? I'll commit this."
```
...
Claude Code's reaction: Reads it, doesn't know what to DO
```
...
STATE.json                       ← STATE (Claude Code loads first)
SESSION-0-PLAN-REVIEW.md         ← VALIDATION (12 FC checks + 2 model reviews + approval)
SESSION-1-GOVERNANCE.md          ← EXECUTION (Claude Code follows step by step)
SESSION-2-PLANNING.md            ← EXECUTION
SESSION-3-OUTPUT-FORMAT.md       ← EXECUTION
...

Claude Code's reaction:
...
Claude Code natively works in steps — it runs a command, reads output, runs
the next command. It doesn't need a "stop here" marker because its execution
model is already sequential.

But in web sessions (claude.ai), the model produces ALL output at once —
...
how web chat works), but Claude Code needs INCREMENTAL instructions (because
that's how agent execution works).

This skill bridges that gap by forcing the web session to produce output in
Claude Code's native format — even though the web session could produce
everything at once.
...
```
Web session producing work for Claude Code:

  1. agent-output-format-skill      ← THIS SKILL (who is the consumer? format check)
  2. xiigen-core-principles-skill   ← Gate 0: 8 principles BEFORE discovery
  3. infrastructure-discovery-skill ← Ground plan in real code (verify paths, counts)
  4. planning-skill (8 gates)       ← Validate plan correctness (architecture, tests)
  5. plan-review-skill (12 FCs)     ← Validate plan consistency (counts, paths, principles)
     └── SESSION-0: Gate A (FC checks) + Gate B (2 AI reviews) + Gate C (Luba approval)
  6. SESSION-1 through SESSION-N    ← Only after plan-review-skill passes all 3 gates

  Before EVERY file creation:
     Re-check: Is this file for Claude Code or for the human?
     If Claude Code: use execution file format
