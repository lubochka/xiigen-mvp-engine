...
## Date: 2026-04-24
## v6 amendment: C33 (Phase 7 FC-18) + C34 (Phase G portability) — Phase 7 (FC-18 UI/UX Compliance audit) mandatory
...
[Phase G — PORTABILITY MOBILITY GATE — all distributable flows]  (1 session)
[Phase H — AUTH DECORATION — flows with HTTP controllers]         (1 session)
[Phase I — TENANT CERTIFICATION STATUS — distributed flows]       (1 session)
...
# SK-554 type coverage (NEW v6.3 — C35): scope_isolation must be present; no empty arbiterType
SCOPE_COUNT=$(jq '[.[] | select(.arbiterType=="scope_isolation")] | length'   fixtures/arbiters/{slug}-arbiters.bulk.ndjson 2>/dev/null || echo 0)
echo "scope_isolation records: $SCOPE_COUNT"
# Expected: >= 1 (BLOCK if 0)
[ "$SCOPE_COUNT" -lt 1 ] && { echo "FAIL SK-554: no scope_isolation arbiter records"; exit 1; }

NO_TYPE=$(jq '[.[] | select(.arbiterType == null or .arbiterType == "")] | length'   fixtures/arbiters/{slug}-arbiters.bulk.ndjson 2>/dev/null || echo 0)
echo "Records without arbiterType: $NO_TYPE"
# Expected: 0 (WARN if > 0, backfill arbiterType on each affected record)
[ "$NO_TYPE" -gt 0 ] && echo "WARN SK-554: $NO_TYPE records without arbiterType — backfill before Phase B"
...
Auth declaration (v6.3 — C35 — for services with @Controller):
```bash
# A-1: Verify @UseGuards present on controller (or AUTH_DEFERRED if auth.module.ts absent)
AUTH_MODULE=$(ls server/src/auth/auth.module.ts 2>/dev/null | wc -l)
if [ "$AUTH_MODULE" -eq 1 ]; then
  GUARDS=$(grep -c "@UseGuards" server/src/engine/flows/{slug}/{controller}.controller.ts || echo 0)
  CTRLS=$(grep -c "@Controller(" server/src/engine/flows/{slug}/{controller}.controller.ts || echo 0)
  [ "$GUARDS" -lt "$CTRLS" ] && echo "FAIL A-1: controller without @UseGuards" || echo "A-1 PASS"
  # A-2: Verify @Roles or @Public() on each route
  ROUTES=$(grep -cE "@Get\(|@Post\(|@Put\(|@Delete\(|@Patch\(" ... || echo 0)
  AUTH_DECL=$(grep -cE "@Roles\(|@Public\(\)" ... || echo 0)
  [ "$AUTH_DECL" -lt "$ROUTES" ] && echo "FAIL A-2: route without @Roles/@Public()" || echo "A-2 PASS"
else
  echo "AUTH_DEFERRED: auth.module.ts absent — decorate controllers in Phase H"
fi
# D-HIST-001: No direct SDK imports in service files
SDK=$(grep -cE "from '@elastic|from '@anthropic|from 'pg'|from 'ioredis'"   server/src/engine/flows/{slug}/*.service.ts 2>/dev/null   | awk -F: '{sum+=$2} END {print sum+0}')
echo "D-HIST-001 SDK imports: $SDK"   # Expected: 0
[ "$SDK" -gt 0 ] && { echo "FAIL D-HIST-001: replace SDK import with fabric interface"; exit 1; }
```
```
...
### G.6 — D-HIST-001 No direct SDK imports (NEW v6.3 — C35)
```bash
SDK=$(grep -rn "^import.*from '@elastic\|from '@anthropic\|from 'pg'\|from 'ioredis'" \
  server/src/engine/flows/{slug}/ --include="*.ts" \
  | grep -v "fabrics/implementations\|.spec." | wc -l)
echo "D-HIST-001 Direct SDK imports: $SDK"
# Expected: 0
# FAIL → replace with IDatabaseService, IAIService, IQueueService fabric interfaces
# See retroactive-development v1.2.0 D-HIST-001 fix row
```
...
# ⛔ STOP — portability status recorded; proceed to Phase H (auth) or Phase 7
```
```

### Step 12a — [C35 NEW v6.3] Write Phase H (Auth Decoration)

**Phase H is mandatory for every implementation plan that produces HTTP controllers.**
If no controllers: declare "No HTTP controllers — Phase H N/A."
If AUTH_DEFERRED (auth.module.ts absent): declare AUTH_DEFERRED with Phase H named as the
fix session after AUTH-ROLES-GROUPS-PLAN-v3.0 Phases 1-4 deploy.

```markdown
## PHASE H — AUTH DECORATION ★ MANDATORY for flows with HTTP controllers ★

Source: dna-compliance-guard v1.2.0 A-1..A-3; flow-implementation-guide v1.3.0 V10.
If AUTH_DEFERRED: record and skip to Phase I.

### H.1 — Auth infrastructure check
```bash
AUTH_MODULE=$(ls server/src/auth/auth.module.ts 2>/dev/null | wc -l)
...
echo "Auth infrastructure: $AUTH_SCORE/4"
# 4/4 → proceed with H.2
# < 4 → AUTH_DEFERRED: record authStatus=AUTH_DEFERRED in STATE.json; skip to Phase I
```

### H.2 — Verify @UseGuards on all controllers (A-1)
```bash
CONTROLLERS=$(find server/src/engine/flows/{slug} -name "*.controller.ts" | wc -l)
GUARDED=$(grep -rl "@UseGuards" server/src/engine/flows/{slug}   --include="*.controller.ts" 2>/dev/null | wc -l)
echo "A-1 Controllers: $CONTROLLERS | Guarded: $GUARDED"
# Expected: GUARDED == CONTROLLERS
# FAIL → add @UseGuards(JwtAuthGuard, RolesGuard) before @Controller(...)
```

### H.3 — Verify @Roles/@Public() on all routes (A-2)
```bash
for CTRL in $(find server/src/engine/flows/{slug} -name "*.controller.ts"); do
  ROUTES=$(grep -cE "@Get\(|@Post\(|@Put\(|@Delete\(|@Patch\(" $CTRL 2>/dev/null || echo 0)
  AUTH_DECL=$(grep -cE "@Roles\(|@Public\(\)" $CTRL 2>/dev/null || echo 0)
  echo "A-2 $CTRL — routes=$ROUTES auth_decl=$AUTH_DECL"
  # Expected: AUTH_DECL >= ROUTES
done
# FAIL → add @Roles(ROLE.ROLE_STRING) or @Public() above each undecorated route
```

### H.4 — Verify @Public() routes in bypass-paths.registry.ts (A-3)
```bash
PUBLIC_COUNT=$(grep -rcn "@Public()"   server/src/engine/flows/{slug}/ --include="*.controller.ts" 2>/dev/null | wc -l)
echo "A-3 @Public() routes: $PUBLIC_COUNT"
# For each @Public() route: verify route path is in server/src/auth/bypass-paths.registry.ts
# Manual cross-check — list routes and confirm BYPASS_PATHS entry exists
```

### H.5 — Rule 7 auth tests (test-integrity v2.2.0)
```bash
# 401 test: protected route returns 401 with no JWT
TESTS_401=$(grep -rcE "\.expect\(401\)|HttpStatus\.UNAUTHORIZED"   server/src/engine/flows/{slug}/ --include="*.spec.ts" 2>/dev/null   | awk -F: '{sum+=$2} END {print sum+0}')
echo "Rule 7a — 401 tests: $TESTS_401"   # Expected: >= 1

# 403 test: protected route returns 403 with wrong role
TESTS_403=$(grep -rcE "\.expect\(403\)|HttpStatus\.FORBIDDEN"   server/src/engine/flows/{slug}/ --include="*.spec.ts" 2>/dev/null   | awk -F: '{sum+=$2} END {print sum+0}')
echo "Rule 7b — 403 tests: $TESTS_403"   # Expected: >= 1
```

### Phase H gate
```bash
# V10 auth gate
if [ "$GUARDED" -lt "$CONTROLLERS" ] || [ "$TESTS_401" -eq 0 ] || [ "$TESTS_403" -eq 0 ]; then
  echo "❌ AUTH GATE INCOMPLETE — fix A-1/A-2 gaps and add 401/403 tests"
  echo "   (AUTH_DEFERRED acceptable — record in STATE.json with Phase H named as fix)"
else
  echo "✅ AUTH GATE PASSED — flow is AUTH_READY"
  node -e "
    const fs = require('fs');
    const s = JSON.parse(fs.readFileSync('docs/sessions/{slug}/IMPL-STATE.json'));
    s.authStatus = 'AUTH_READY'; s.authGaps = [];
    fs.writeFileSync('docs/sessions/{slug}/IMPL-STATE.json', JSON.stringify(s, null, 2));
  "
fi

[absolute gate block]
# ⛔ STOP — authStatus recorded; proceed to Phase I
```
```

---

### Step 12b — [C35 NEW v6.3] Write Phase I (Tenant Certification Status)

**Phase I is mandatory for every distributable flow (any flow targeting TIER-A or above).**
If not distributable: declare "Not targeting distribution — Phase I N/A."

```markdown
## PHASE I — TENANT CERTIFICATION STATUS ★ MANDATORY for distributed flows ★

Source: flow-implementation-guide v1.3.0 V11; SK-553 v1.1.0 TIER model.
Records the flow's TIER-A/B/C/D certification tier.

### I.1 — V11 tenant certification gate
```bash
PORTABILITY=$(jq -r '.portabilityStatus // "TBD"' docs/sessions/{slug}/IMPL-STATE.json 2>/dev/null || echo "TBD")
AUTH=$(jq -r '.authStatus // "TBD"' docs/sessions/{slug}/IMPL-STATE.json 2>/dev/null || echo "TBD")
SK553_L1=$(jq -r '.portabilityTest.layer1.status // "NOT_RUN"'   docs/sessions/{slug}/IMPL-STATE.json 2>/dev/null || echo "NOT_RUN")
echo "Portability: $PORTABILITY | Auth: $AUTH | SK-553 Layer 1: $SK553_L1"
```

Tier determination (stop at first match):
```
TIER-D: TIER-C + SK-549 full per-role coverage complete
TIER-C: TIER-B + AI Adaptation Phases 1-5 + per-role visual (N roles × 3 cells)
         + R6 cross-tenant JWT PASS + Guard 14 (AUTH-PLAN v3 Phases 1-4 deployed)
...
### I.2 — TIER-C pre-certification checklist (PER-FLOW-FIX-MAP-v2.9 §Guard 14)
Only run if targeting TIER-C. If targeting TIER-A or TIER-B, skip to Phase I gate.

```
□ Guard 14: AUTH-PLAN v3 Phases 1-4 deployed?
    YES → run SK-553 v1.1.0 Phase 0 Auth Pre-flight
    NO  → AUTH_DEFERRED; maximum tier = TIER-B
...
□ Repo naming: {tenantId}--{moduleName} applied in ForkFlowHandlerService?

□ Repo evidence: GitHub listing PNG at each cascade point?
    ls docs/e2e-snapshots/{slug}/repo-evidence/*.png | wc -l  # Expected: >= 1

□ Per-role visual: all N roles × 3 cells (en/he-RTL/mobile) via SK-549?

□ R6 cross-tenant JWT rejection tested?
    grep -rcE "cross.tenant|tenantB.*token" server/src/engine/flows/{slug}/ --include="*.spec.ts"
```

### Phase I gate
```bash
# Record current tier in STATE.json
node -e "
  const fs = require('fs');
  const s = JSON.parse(fs.readFileSync('docs/sessions/{slug}/IMPL-STATE.json'));
  s.tenantCertTier = 'NONE | TIER_A | TIER_B | TIER_C | TIER_D'; // set actual value
  fs.writeFileSync('docs/sessions/{slug}/IMPL-STATE.json', JSON.stringify(s, null, 2));
"
echo "tenantCertTier recorded in IMPL-STATE.json"

[absolute gate block]
# ⛔ STOP — tenantCertTier recorded; proceed to Phase 7 or session close
```
```
...
✅ docs/sessions/FLOW-XX/FLOW-XX-IMPL-STATE.json            (phase_status: ACTIVE)
                                                             portabilityStatus: MOBILE|PARTIAL_GAP
                                                             authStatus: AUTH_READY|AUTH_DEFERRED
                                                             tenantCertTier: TIER_A|TIER_B|...|NONE
[if Phase 7 ran:]
...
- [ ] **[C35 v6.3] Phase H present if any HTTP controllers produced; "N/A" or "AUTH_DEFERRED" if not**
- [ ] **[C35 v6.3] Phase I present if flow is distributable; "N/A" if not targeting distribution**
- [ ] **[C35 v6.3] Phase A gate includes SK-554 scope_isolation count check (≥1 required)**
- [ ] **[C35 v6.3] Phase B auth section present (A-1/A-2/D-HIST-001 checks or AUTH_DEFERRED)**
- [ ] **[C35 v6.3] IMPL-STATE.json template includes portabilityStatus + authStatus + tenantCertTier**
...
*v6.3 amendment: C35 — Phase H (auth decoration, A-1..A-3, Rule 7 tests) mandatory for*
*                       flows with HTTP controllers; Phase I (tenant certification,*
*                       TIER-A/B/C/D, Guard 14, TIER-C checklist) mandatory for*
*                       distributable flows; Phase A gate extended with SK-554 scope_isolation*
*                       count check; Phase B template extended with auth declaration +*
*                       D-HIST-001; Phase G extended with G.6 D-HIST-001; IMPL-STATE.json*
*                       updated with authStatus + tenantCertTier fields.*
*Closes AUTH-ARBITER-SKILLS-REMEDIATION-PLAN-v3.0 Phase 13.*
*Round: 27 of 72*
