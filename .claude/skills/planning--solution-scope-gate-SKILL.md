...
## RECONCILE — core `solution-scope-gate` parity (G02 refresh from llm_mvp_core)

This SK-434 ladder is the flow-domain rename of the core ladder; it is NOT replaced.
The two facts to reconcile (so a downstream reader does not assume parts are missing):

**(A) Ladder name mapping (keep mvp flow naming; know the core equivalent):**

```
core CONVENTION      ≡ mvp CONVENTION      (rule/skill/doc, no code)
core ADAPTATION      ≡ mvp ADAPTATION      (wrap an existing tool/MCP/npm SDK — Anthropic/OpenAI/Google — thin only)
core EXTENSION       ≡ mvp EXTENSION       (new step/param/check on an existing handler/skill/template)
core NEW COMPONENT   ≡ mvp NEW FLOW        (net-new topology; in mvp this means a registered flow)
core NEW INTERFACE   ≡ mvp NEW INFRA       (new fabric interface / index / service / external dependency = last resort)
```
TS-adaptation of "NEW INTERFACE": a new TypeScript `interface` + concrete impl +
NestJS `@Injectable()` provider registered in a module (NestJS DI is the fabric).
Level 2 ADAPTATION = a thin wrapper around an npm utility/SDK behind a capability
interface (named for the capability, not the vendor — see level-correction-response).

**(B) The three REFRAME TESTS above are the core's three tests under mvp names —
confirm all three are run, none is optional:**

```
mvp Test 1 "Next Instance"        ≡ core Next-Instance test
mvp Test 2 "Provider vs Capability" ≡ core Provider-vs-Capability test
mvp Test 3 "Old Model Survival"   ≡ core Backward-Compat / old-model-survives test
```

**(C) Scope Gate Record (core requires it explicitly — emit it before drafting any
session/plan file):**

```text
Scope Gate Record
problem_one_sentence:
convention_sufficient:        # yes/no + why-not if higher
adaptation_sufficient:        # yes/no + why-not if higher
extension_sufficient:         # yes/no + why-not if higher
new_flow_q1_q4_pass:          # only if claiming NEW FLOW
new_infra_truly_required:     # only if claiming NEW INFRA (last resort)
chosen_level:                 # CONVENTION | ADAPTATION | EXTENSION | NEW FLOW | NEW INFRA
reframe_test_1_next_instance: clean | failed→fix
reframe_test_2_provider_vs_capability: clean | failed→fix
reframe_test_3_old_model_survival: clean | failed→fix
lower_levels_eliminated_because:
```

Do not draft session/plan files until `chosen_level` is justified against every lower
level AND all three reframe tests return clean. NB (R5/G12): a value that should learn
is NOT "NEW INFRA you build here" — mvp consumes shared models from `llm_mvp_core` via
manifests/locators and trains only the adaptive leg; classify it (four-tier) before
reaching for a new component.
