...
### For mvp product code (NestJS / React / FastAPI — universal, stack-adapted):

```
1. [Typed Result] Does every public method on this NestJS provider/controller
   return the typed Result/DataProcessResult, never a bare throw for business
   logic and never null?

2. [Fail-path test] Is there a Jest spec that exercises the FAILURE path, not
   only the happy path? If not, why is the failure path unverified?

3. [Thresholds → config] Are all numeric thresholds, URLs, and limits read from
   env/IConfiguration, not hardcoded? (Q4 of the universal set.)

4. [React a11y/states] Does this React component handle loading/empty/error
   states and expose accessible names (aria-*), or does it only render the happy
   path with no a11y?

5. [RAG sidecar boundary] Does the FastAPI endpoint validate its input and return
   a typed error shape on bad input, with a numeric eval where quality is claimed?

6. [Core boundary R6] Is any core algorithm logic leaking into this
   agent/extension/controller layer that should live in core? (Keep the host
   layer to API-call-in / answer-out.)
```
