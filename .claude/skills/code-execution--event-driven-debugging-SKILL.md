...
## TRANSPORT NOTE — this repo emits CloudEvents (DNA-9), not raw emitters

In this engine, domain events travel as CloudEvents envelopes, not as bare
`EventEmitter2`/`queue.enqueue()` payloads. The correlation key used in every
search below is the envelope's `correlationId` (a raw `EventEmitter2.emit()` or
direct `queue.enqueue()` bypasses the envelope and is itself a DNA-9 / CF-448
violation — flag it). Events are emitted through the CloudEvents transport
(`createCloudEvent(...)` + `ICloudEventsEnvelopeService` / `cloudEvents.emit`),
and downstream delivery goes store-before-dispatch (DNA-8): the event is
persisted to the outbox before it is dispatched. So when an event "never
arrived", also check the outbox store, not only the live stream. Throughout this
skill, read `traceId` as "the envelope `correlationId` for this user action".
This protocol is the bisection method; it is distinct from `mental-debug` Rule 5
`boundary-message-map` (schema mapping) — do not merge them.
