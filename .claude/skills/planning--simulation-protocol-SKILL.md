...
**Step 1: Identify the emission point** — find the task type in FLOW-A that emits the event.

**Step 2: Find all registered consumers** — query xiigen-rag-patterns for CROSS_FLOW_DEPENDENCY patterns. Also grep engine-contracts for consumers.

**Step 3: Trace each consumer's handler** — produce a cross-boundary step table showing field transformations at each transit point.

**Step 4: Check each transformation point** — at every point where the field passes through a handler not in the flow topology, ask: is the field transformed before it reaches its destination? Check queue consumer handlers, middleware, adapter layers, and serialization.

**Step 5: Check combined invariants at the boundary** — for each invariant involving the shared field in both FLOW-A and FLOW-B, verify they hold when combined.

### Cross-flow trace verdicts

Use the same verdict vocabulary as single-flow simulation:
- WORKS: combined invariants hold at the boundary
- BREAKS: boundary invariant violated, visible error
- SILENT_FAILURE: boundary invariant violated, no visible error, wrong behavior shipped
- UNKNOWN: intermediate handler behavior not verified — this is a gap, not a verdict

### When cross-flow simulation is mandatory

Run CROSS_FLOW_TRACE before any flow that: consumes events from an existing ACTIVE flow, introduces a new BFA CF rule, modifies the payload schema of an event consumed by another flow, or uses a field value set in a different flow.
...
| Class | Layer | Description | Typical fix scope |
|-------|-------|-------------|-------------------|
| A | Intake | Cannot read/understand external artifacts | NEW FLOW or EXTENSION |
| B | Representation | No verified NODE for the capability | EXTENSION + NEW FLOW |
| C | Retrieval | Cannot retrieve patterns by meaning, only by field filter | EXTENSION |
| D | Generation | Cannot generate for a target stack | EXTENSION |
| E | Learning | Captures wrong or no training data | EXTENSION |
| F | Evaluation | No domain-specialized arbiter judgment | EXTENSION |
| G | Cross-flow | Invariant violated at boundary between flows | CONVENTION + EXTENSION |
| H | Visibility | Produces work but doesn't report progress | EXTENSION |
| I | Output types | Can generate code but not other output types | NEW FLOW |
