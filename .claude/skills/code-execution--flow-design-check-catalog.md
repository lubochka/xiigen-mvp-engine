...
```json
{
  ...
  "description": "Service files must not import ClsService or nestjs-cls. tenantId MUST come from EngineContract input payload, not AsyncLocalStorage context. In concurrent Promise.all() execution, AsyncLocalStorage context leaks across tenant boundaries. Fix: see retroactive-development SK-419 v1.1.0 portability fix table P-1 row."
}
```
...
```json
{
  ...
  "severity": "PORTABILITY_BLOCK",
  "description": "All FREEDOM config keys must use flow-scoped naming: flow{NN}_{semantic_name}. Unscoped key collides across flow installations. Fix: rename key to flow48_translation_enabled format; update STEP-1-INVARIANTS FREEDOM table."
}
```
...
```json
{
  ...
  "severity": "PORTABILITY_BLOCK",
  "description": "Flow code must not define local copies of canonical fabric interfaces. Fix: delete local interface; import from fabrics/interfaces/ or @xiigen/engine-infra-interfaces."
}
```
...
```json
{
  ...
  "severity": "PORTABILITY_BLOCK",
  "description": "Every cross-flow ES index read must be declared in package.json xiigen.requiredCoInstalls. Fix: add to package.json: 'xiigen': { 'requiredCoInstalls': ['FLOW-N'] }."
}
```

**Severity level: PORTABILITY_BLOCK** — Flow is ACTIVE but not MOBILE. Does NOT block production deployment within the monorepo. DOES block packaging and distribution to a second tenant. Must appear in portabilityGaps[] in STATE.json and V9 report.
