...
## RECONCILE — core `freedom-machine` parity (G02 refresh from llm_mvp_core)

SK-451 is the base that SK-510 (four-tier) builds on. The one test stays: **"If this
value changed, would the system's behavioral guarantees break?" YES ⇒ MACHINE
(named constant + presence test, never config). NO ⇒ FREEDOM (config/env + safe
default).** Two core rules to confirm so a MACHINE invariant does not silently become
FREEDOM:

**(A) The borderline case — the RULE is MACHINE, its PARAMETER may be FREEDOM.** A
threshold that enforces an invariant is MACHINE; a threshold that only tunes a
recall/precision tradeoff is FREEDOM, even though both are "a number".

```
MACHINE: dedup "same item" threshold — lowering it makes distinct items collide → guarantee breaks → hardcode + test
FREEDOM: BM25/search relevance cutoff — moving it shifts recall vs precision → ConfigService key
         (the rule "filter low-scoring results" is MACHINE; the specific cutoff is FREEDOM)
```
TS fabric corollary: the interface/contract (a NestJS provider token, the typed
`DataProcessResult<T>` shape, the tenant-scope invariant) is MACHINE; the concrete
adapter chosen by DI is FREEDOM.

**(B) MACHINE constants require an annotation AND a presence test.** Annotate the
classification in code and add a test that asserts the constant's value, so a later
edit that quietly moves it to config or changes it fails CI:

```ts
// MACHINE: cosine dedup threshold — changing it breaks deduplication correctness
export const COSINE_DEDUP_THRESHOLD = 0.999;

// FREEDOM: search relevance minimum — read from config, corpora differ
const minScore = this.config.get<number>('retrieval.minScore', 0.5);
```
```ts
// jest presence test for the MACHINE invariant
it('COSINE_DEDUP_THRESHOLD is the locked MACHINE value', () => {
  expect(COSINE_DEDUP_THRESHOLD).toBe(0.999);
});
```
Reading a MACHINE constant from env/config is the anti-pattern: a constant that
config can change is not a constant. (No-Secrets: a secret is "not in repo" =
MACHINE invariant, but its VALUE comes from Vault/config, never hardcoded.)
