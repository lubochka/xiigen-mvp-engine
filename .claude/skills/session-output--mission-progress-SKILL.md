...
## PRODUCT-AXIS MISSION PROGRESS (mvp default — supersedes the ML/DPO variant above)

Per R5/R6 and the G12 boundary, common-model TRAINING (DPO triples, shadow-gap,
graduation, IncrementalTrainer) lives in `llm_mvp_core`, NOT in mvp. So the five
ENGINE-PROGRESS questions above are the legacy engine variant. For an mvp product
session, use the SAME honesty discipline (FIRST section, every question answered,
PENDING-with-reason instead of omit/estimate, every number from a command) but on
the PRODUCT axis:

### Q1 — Did a product capability advance this session?
Name the concrete capability delta (flow / RAG retrieval / API / UI), not files.
```bash
npx jest --testPathPatterns "<spec>" 2>&1 | tail -3   # capability behavior
```
Report: "FLOW-XX moved 0→1 tested" / "retrieval moved vector-only → BM25+vector".

### Q2 — Did RAG retrieval QUALITY improve this session?
Numeric FastAPI eval run, never "looks better":
```bash
pytest tests/rag_eval.py 2>&1 | tail -5   # or the FastAPI eval endpoint
```
Report recall/precision before→after. PENDING (with reason) if no eval ran.

### Q3 — Which scenarios PASSED (and through which layer)?
```bash
npx jest 2>&1 | tail -3            # NestJS server unit/integration
npx playwright test 2>&1 | tail -3 # React UI e2e
```
Report `0 failed` per layer, or the exact failing count. `passed:true` in a JSON
file is NOT a scenario pass — name the layer and command.

### Q4 — What BLOCKED progress this session?
State each blocked dimension explicitly (e.g. "RAG eval fixture missing", "auth
not wired so Playwright route 401s"). Omitting this section is not allowed.

### Q5 — What does Luba DECIDE next?
One specific question with 2-3 options, or "No decision needed: [reason]".

### Product mission-progress table (FIRST section of mvp PHASE-COMPLETE)
```markdown
## PRODUCT PROGRESS — Phase [X]
*(Run the command — never estimate. State PENDING if no run.)*

| Metric | This Phase | Cumulative | Target | Gap |
|--------|-----------|------------|--------|-----|
| Capability delta | <flow/RAG/API/UI> | — | <goal> | — |
| RAG eval (recall@10) | N→M / PENDING | M | <goal> | — |
| Server tests passing | N (0 failed) | N | all green | — |
| UI e2e passing | N (0 failed) | N | all green | — |

### What advanced (required) — concrete observable delta, command-backed
### What is blocked (required — explicit, even if "None")
### Decision needed (question + options OR "No decision needed: [reason]")
```

Count/actor honesty applies: a generated table is not a cycle; N ledger rows ≠ N
reviews; parent self-review is not a sub-agent review.

## WHAT THIS SKILL PREVENTS

- Luba working blind after every session: "no idea if we are closer or farther away"
- PHASE-COMPLETE files that show "5,386 tests passing" with no progress metric
- Silent assumption that an unbuilt flow means progress cannot be reported
- A product/eval metric staying PENDING indefinitely without a named owner or action
- Decision thresholds existing nowhere — metrics visible but actions unclear
- Reporting an ML/DPO graduation metric for mvp when training lives in `llm_mvp_core`
