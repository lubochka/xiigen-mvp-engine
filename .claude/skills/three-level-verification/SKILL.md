...
### Anti-Pattern 5: Level 2 reuses the same mocks as Level 1 (universal)
```
✗ Level 2 spec injects the SAME mock for the fixed dependency that Level 1 used
   (e.g. both pass jest.fn().mockReturnValue(ok) for the path under repair)
✓ Level 2 wires the REAL class for the path being fixed via Test.createTestingModule
   ({ providers: [RealService, ...] }). Only dependencies NOT under test may stay
   mocked. If the fixed dependency is still mocked at Level 2, Level 2 proves nothing
   that Level 1 did not already prove — the mock insulates the real failure path.
```
Rule: the dependency under repair MUST be a real implementation at Level 2. Mocking
the fixed path at both levels is the classic "Level 2 with the same mocks" coverage
illusion — it produces a green Level 2 that cannot catch mock-masking.

---

## Baseline Test-Count Synchronization (universal)

Level 3 must compare the full-suite count against a recorded baseline, not just
"0 failures". A green run with FEWER tests than baseline means a test was deleted
or skipped — find it before declaring done.

```bash
# Record at session start (monorepo full server suite):
cd server && npx jest --listTests | wc -l          # or capture passed-count from the run summary
# Level 3 gate: Tests passed >= recorded baseline AND Failed = 0
```

- `Failed: 0` alone is NOT sufficient. `Passed: N` must satisfy `N >= baseline`.
- A drop in count = a removed/`.skip`ped test. Locate it, restore or justify it
  (Iron Rule: assertion/test removal needs Luba approval — see `bug-to-tests`).
- Keep the baseline current with the monorepo: re-record after a legitimately
  approved test addition/removal so the gate stays accurate.
