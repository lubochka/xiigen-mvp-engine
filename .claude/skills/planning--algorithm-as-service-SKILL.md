...
## UNIVERSAL IRON-RULE SET (binding, from core `algorithm-as-service`)

Independent of class, every algorithm-as-service task type must satisfy this
named iron-rule set. Each is a **score-0** check — a single failure blocks the
task type, it is not a "nice to have". (Per-class rules above add to, never
replace, this set.)

```
weights_from_config        — every weight/rate/threshold is config.get(key, default),
                             never a decimal literal in the formula (the T65 error class)
weights_sum_to_one         — Σ weights ≈ 1.0 at startup, tolerance ±0.05 (Class 1 / Class 4)
score_range_0..1           — composite/match output is clamped to [0.0, 1.0]
match_score_symmetric      — match(A,B) == match(B,A) within 1e-3 (Class 4)
points_idempotent          — same action + idempotency key = same points, never doubled (Class 5)
level_derived_not_stored   — level is computed from total points, never persisted on its own (Class 5)
```

**The one mandatory MACHINE test (config ≠ hardcoded).** For every task type that
computes a weighted score, write the negative test proving the result *moves* when
config moves — a hardcoded literal would keep the same output and fail this test:

```typescript
it('reads the weight from config, not a hardcoded literal', () => {
  const custom = 0.40;                       // deliberately != default 0.25
  mockConfig.set('scoring.<domain>.matchWeight', custom);
  const score = calculator.compute(signals); // ConfigService.get<number>() path
  expect(score).toBeCloseTo(custom * signals.match + /* ...other terms */ 0, 5);
  // config.get() implementation: score changes with config  → PASS
  // hardcoded 0.25 implementation: score unchanged           → FAIL (masquerade caught)
});
```

> R5 boundary (binding): Class 3 RECOMMENDATION has *adaptive* weights — the learned
> model and its training/DPO loop stay in `llm_mvp_core` and are consumed here via
> manifest/locator (IMLService). mvp/flows never train the common model locally; they
> only call it. WEIGHTED_SCORING/MATCHING/DECAY/GAMIFICATION weights are FREEDOM
> config and may live in the consuming app.
