...
> **Split note (binding):** this skill (SK-504) is the *design* half of the core
> `shared-service-design` skill. The *identification* half is
> `planning--cross-cutting-service-SKILL.md` (SK-495), which counts service
> appearances and applies the threshold: **≥3 consumers → fabric interface
> mandatory, built BEFORE the first consumer.** SK-504 takes every service that
> crossed that ≥3 line and designs its fabric interface, mock provider, key/TTL
> conventions, and pre-flow build sequence. Run the two as a pair: SK-495 decides
> *which* services are shared; SK-504 decides *how* they are built. A shared
> capability that reaches a consumer with no fabric interface in place is the
> exact drift defect both halves exist to prevent.
