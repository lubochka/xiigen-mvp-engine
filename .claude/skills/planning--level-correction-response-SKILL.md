...
## RECONCILE — core `level-correction-response` parity (G02 refresh from llm_mvp_core)

SK-439 already carries the core's single rule — **on a level challenge, abandon the
frame and rebuild one level up; do NOT add a layer, add a peer proposal, defend, or
run more checks**. Three reconcile confirmations so a reader does not assume drift:

**(A) The three proactive tests are the core's three, under mvp names — run all
three as a proactive scan over the WHOLE plan before drafting, not only reactively:**

```
mvp "Next Instance"          ≡ core Next-Instance test
mvp "Provider vs Capability" ≡ core Provider-vs-Capability test
mvp "Old Model Survival"     ≡ core Backward-Compat / old-model-survives test
```

**(B) Provider-vs-capability naming, TS examples** (name the capability, never the
vendor/provider; a provider-named symbol cannot be swapped without renaming callers):

```
FAILS (provider):                  PASSES (capability):
IOpenAIService / OpenAiClient   →  ISemanticCompiler / ITextGenerator
IRedisService                   →  ISessionStore (scoped, ephemeral)
PineconeService                 →  IVectorIndex
GitHubMCP (as an interface)     →  ICodeRepository
```
In NestJS, the DI provider token is named for the capability; the concrete adapter
(the npm SDK wrapper) is the swappable FREEDOM implementation behind it.

**(C) Without the abandon-frame rule the skill degrades into a naming checklist.**
The signal is: Luba says what you ARE ("you're thinking as a utility writer, not an
interface designer"), not what you got wrong — that is a level correction; go up.
Adding a wrapper "just in case" while keeping the rejected component IS defending the
frame (anti-pattern), not responsiveness.
