# XIIGEN FLOW DOCUMENT AUTHORING GUIDE — v1.17
## How to prepare Design Simulation, Teach-QA, and Implementation Plan files
## Date: 2026-04-23
## Supersedes: v1.16 (screen intent anchor) and all prior versions

---

## WHAT CHANGED IN v1.17 (skill prerequisites in flow documents)

One class of change. Additive. No existing rules removed.

**Rule 36 — Skill Prerequisites in Flow Documents** (new §19 below): any
**Rule 37 — Cross-Flow Event Contract Extension** (new §20 below): the H-29
flow-specific document that references a governance skill by name must include
a `Prerequisites` section immediately before that reference point. A skill
reference without a prerequisite load instruction is an inert citation — it
tells the executor what to do without ensuring the skill is in context to do
it. This rule closes the failure where sessions found "Run SK-549" in a
portability protocol, had no SK-549 in active context, and either misexecuted
the instruction or required multiple correction rounds to understand it.

**Completion Gate** updated: one new check item added (SKILL PREREQUISITES
PRESENT — General Rule 36) verifying all skill-name references in the
document have prerequisite load instructions.
...
☑ SKILL PREREQUISITES PRESENT (new v1.17 — General Rule 36):
  - Every skill-name reference in this document has a Prerequisites section
    immediately before the reference point
  - Prerequisites section names the skill file to load completely
  - If SK-549 referenced in Claude Code context: §For Claude Code section
    explicitly included in prerequisite instruction
  - If SK-410 or custom arbiter referenced: full FC battery loaded, not
    only custom subset
...
Rules 1–35 unchanged from v1.16. Rule 36 is new in v1.17. Rule 37 is new in v1.18.
...
## 19. Skill Prerequisites in Flow Documents (Rule 36 — new v1.17)

### What this rule says

Any flow-specific document that references a governance skill by name must
include a `Prerequisites` section immediately before that reference point.
The `Prerequisites` section lists every skill that must be loaded before
the instruction using that skill can be correctly executed.

**WRONG — inert citation:**
```
Step 3: Run SK-549 on every PNG.
```

**CORRECT — active prerequisite:**
```
Prerequisites before Step 3:
  - per-image-validation skill (SK-549) loaded and active
    Load: read `planning--per-image-validation-SKILL.md` completely
    In Claude Code: confirm §For Claude Code section read (computer_use
    discipline: navigate via computer_use, save to disk, never read PNG
    to chat)

Step 3: Run per-image-validation (SK-549) on every PNG per §For Claude Code.
```

### What "skill by name" means

Any of the following trigger the prerequisite requirement:
- `SK-NNN` numeric reference: "Run SK-549," "Apply SK-410"
- Human-readable skill name: "per-image-validation," "plan-review"
- Verb forms: "run the reconnaissance gate," "apply the FC battery"
- Imperative with skill as instrument: "validate using SK-549 axes"

The trigger is: the document is instructing an executor to use a skill.
Instructions to read or load a skill do not trigger the requirement —
only instructions to execute or apply one.

### Why this rule exists

The failure class that produced this rule occurred three times in the
April 2026 session corpus:

**Failure 1 (portability protocol):** FLOW-PORTABILITY-TEST-PROTOCOL v1.1
contained "Run SK-549 on every PNG" three times. SK-549 was not in active
context. The session could not determine what "running SK-549" meant in
Claude Code — it had no §For Claude Code section in memory. Three correction
rounds were required to establish: navigate via computer_use, save to disk,
never read PNG to chat.

**Failure 2 (plan review FC battery):** The module-separation fix plan went
through six custom arbiter passes (F1-F4, A-F passes). The fix plan did not
include "Prerequisite: plan-review-skill (SK-410) FC battery loaded." Sessions
developed flow-specific F1-F4 arbiters instead. FC-7 DNA compliance, FC-6
iron rules, and FC-4 pipeline contract became inaccessible because the general
FC battery had been displaced by the custom arbiter set. DNA violations
propagated to fifteen flows undetected.

**Failure 3 (session cascade):** the general examination protocol was available
as SK-552 but sessions built per-range custom examination structures because
flow examination plans cited SK-552 as reference without loading it as an
active prerequisite.

Root cause: flow-specific documents cite skills as labels, not as active
prerequisites. The label is present; the behavior is absent. The executor
reads "Run SK-549," treats it as an instruction, and has no skill in context
to interpret the instruction correctly. Every skill reference without a
prerequisite load instruction is an execution gap.

### The prerequisite format

```
Prerequisites before [section/step name]:
  - [skill human name] ([SK-NNN]) loaded and active
    Load: read `[skill filename]` completely before proceeding
    [Optional: specific section to read, e.g. "read §For Claude Code"]
    [Optional: confirmation check, e.g. "confirm §X section loaded"]
```
...
Prerequisites before Phase 3:
  - per-image-validation skill (SK-549) loaded and active
    Load: read `planning--per-image-validation-SKILL.md` §For Claude Code
  - coverage-matrix skill (SK-551) loaded and active
    Load: read `coverage-matrix-SKILL.md` §Governance section
```

### Scope

Rule 36 applies to all three document types produced by this guide:
Design Simulation, Teach-QA, and Implementation Plan.

Rule 36 also applies to any supplementary document a session produces
alongside these three: portability protocols, cascade test protocols,
visual examination protocols, and any flow-specific procedure document.

Rule 36 does not apply to documents that only discuss skills (reviews,
retrospectives, gap analyses). It applies when a document instructs
an executor to use a skill.

### Completion Gate check

The COMPLETION GATE gains one new item:

```
☑ SKILL PREREQUISITES PRESENT (General Rule 36):
  - Every skill name reference in this document has a Prerequisites section
    immediately before the reference point
  - Prerequisites section names the skill file to load completely
  - If SK-549 referenced in Claude Code context: §For Claude Code section
    explicitly included in prerequisite instruction
  - If SK-410 or custom FC arbiter referenced: full FC battery loaded,
    not only custom subset
```

If any skill reference lacks a Prerequisites section: add it before this
phase closes. A document with inert skill citations is a document that will
require correction cycles at execution time.

### Relationship to Rule 35

Rule 35 gates JSX creation: Q8 must be answered before the first React
page is written.

Rule 36 gates skill execution: prerequisites must be declared before any
skill-dependent instruction is executed.

Both rules address the same root failure class: documents that cite
authoritative sources (user intent, governance skills) by name without
ensuring those sources are in active context at the moment they are needed.

---

## 20. Cross-Flow Event Contract Extension (Rule 37 — new v1.18)

### The Problem

When FLOW-48 (i18n-translation) needs the `AccountCreated` event to carry `acceptLanguage`,
it cannot modify that event's contract in isolation. `AccountCreated` is owned by FLOW-01.
Modifying it without a documented extension protocol produces:
- Silent breakage of all existing consumers of `AccountCreated`
- No record of why the field was added
- No FREEDOM key gate that makes it configurable
- No validation that the optional field is properly formatted

This pattern — one flow extending another flow's event payload — is the H-29 pattern
from the module-separation corpus. It was undocumented for 48 flows.

### Rule 37 — Mandatory Cross-Flow Extension Protocol

**When a flow document instructs Claude Code to add a field to an event owned by another flow,
the document MUST include the H-29 extension protocol for that field. No exceptions.**

**Trigger:** Any instruction of the form:
- "Extend event `{EventName}` with field `{fieldName}`"
- "Add `{fieldName}` to the `{EventName}` payload"
- "The `{EventName}` event should carry `{fieldName}` from this flow forward"

**Required — four elements per cross-flow field extension:**

#### Element 1 — Optional field declaration

The new field must be optional on the source event:

```typescript
// In the source flow's event contract (e.g., FLOW-01)
interface AccountCreatedEvent {
  userId: string;
  email: string;
  tenantId: string;
  // H-29 extension: added by FLOW-48 (i18n-translation)
  acceptLanguage?: string;  // OPTIONAL — not all consumers will use this
}
```

A REQUIRED new field on an existing event is a BREAKING CHANGE to all existing consumers.
Rule 37 requires OPTIONAL fields only. If the field must be required, the event needs
a new version — document that in a Design Record instead.

#### Element 2 — FREEDOM key gate

The field value must be configurable, not hardcoded:

```typescript
// In the extending flow's service (e.g., FLOW-48)
const acceptLanguage = this.freedom.get('flow48_accept_language_default', 'en');
```

The FREEDOM key must:
- Use flow-scoped prefix: `flow{NN}_{semantic_name}` (e.g., `flow48_accept_language_default`)
- Have a safe default (the behavior before this extension existed)
- Be documented in the extending flow's STEP-1-INVARIANTS FREEDOM table

#### Element 3 — Validated format

The field value must be validated before the event is emitted:

```typescript
// Validation before emitting
const VALID_LOCALE_PATTERN = /^[a-z]{2}(-[A-Z]{2})?$/;  // MACHINE — format is invariant
if (acceptLanguage && !VALID_LOCALE_PATTERN.test(acceptLanguage)) {
  return DataProcessResult.failure('INVALID_ACCEPT_LANGUAGE_FORMAT');
}
```

#### Element 4 — Consumer update declaration

The document must declare which flows consume the event and whether they need updating:

```markdown
## Cross-flow consumers of AccountCreated

| Flow | Action required |
|------|----------------|
| FLOW-03 (Profile Setup) | No change — does not use acceptLanguage |
| FLOW-12 (Subscription Billing) | No change — does not use acceptLanguage |
| FLOW-48 (i18n-translation) | READS acceptLanguage — this is the adding flow |
```

Consumers that do not use the new field require no code change (the field is optional).
The declaration is documentation only — it proves the author read the consumer list.

### Completion Gate addition

```
☑ CROSS-FLOW EVENT EXTENSION (Rule 37 — new v1.18):
  - If any instruction extends an event owned by another flow:
    □ New field declared as OPTIONAL on source event contract
    □ FREEDOM key declared with flow-scoped prefix and safe default
    □ Validated format documented (MACHINE constant for the format rule)
    □ Consumer update declaration present (list of flows + action required)
```

### Reference implementation

FLOW-48 `acceptLanguage` extension of `AccountCreated` (from the module-separation corpus)
is the reference implementation for Rule 37. All four elements are present in
FLOW-48-DESIGN-R2.md.

### Relationship to Rule 36 and BFA

Rule 37 is downstream of BFA cross-flow validation (Phase F). A cross-flow extension
that violates Rule 37 will also fail the BFA validator — but the BFA validator fires at
implementation time. Rule 37 fires at plan review time (FC-13 documentation gate).
Catching it earlier is always better than catching it at Phase F.

---

*End of XIIGEN FLOW DOCUMENT AUTHORING GUIDE v1.18*
