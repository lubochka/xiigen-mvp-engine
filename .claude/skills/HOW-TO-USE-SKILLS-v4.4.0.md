# HOW TO USE XIIGEN SKILLS — v4.4.0
## Updated: 2026-04-20 | For: Claude.ai Project custom instructions
## Status: Current — supersedes v4.3.0
...
##   2 stale v1.5 SESSION-GUIDE references patched to v1.7
...
### LOAD ORDER 5.5 — UI/UX Compliance (SK-539 v1.0.0)

**Load `planning--ui-ux-compliance-SKILL.md` (SK-539 v1.0.0) before writing any React page.**

Fires when a MATERIALIZATION or GENERATION session produces `*.tsx` files in `client/src/pages/`. Answers the four role questions (Q1 ROLE_TIER, Q2 ROLE_GATE, Q3 ROUTE_GUARD, Q4 VISIBILITY) before the first line of JSX. Checks whether the page matches one of the 7 screen templates (T-1..T-7). Verifies the plan includes Phase 7 (UI/UX Compliance) as a declared step.
...
**SK-539 is mandatory for MATERIALIZATION and GENERATION sessions that produce React pages (load_order 5.5).**
...
| **MATERIALIZATION** | SK-529 v2.0.0 FIRST + SK-535 + SK-536 + SK-528 + SK-532 + SK-533 + SK-537 + SK-531 + SK-534 + SK-530 + SK-538 + **RESPONSE-CONSTRUCTION-PROTOCOL** + **SK-539** (if producing React pages) |
...
**SK-539 is mandatory for GENERATION and MATERIALIZATION sessions that produce React pages (load_order 5.5). Sessions producing only server-side code declare "No React pages — SK-539 N/A."**
...
→ If flow is named: run Q9 from DESIGN-ARCHITECT-SESSION-GUIDE v1.7 (4-step per-flow read).
...
→ **SK-539 mandatory if session produces React pages (load_order 5.5). Phase 7 must be declared as a plan step.**
...
10. GOAL REMINDER BLOCK (SK-536)
    ...
    □ Layer 2: quotes STATE.productGoal.statement (permanent product goal from SESSION-LOAD-PLAN v30)
...
| **Any session producing React pages (*.tsx in client/src/pages/)** | **SK-539 UI/UX Compliance v1.0.0 (load_order 5.5) — before first page written** |
...
Load order at every session start:
  ...
  SK-539 UI/UX Compliance v1.0.0       (load_order 5.5, if session produces React pages)
...
Layer 8 — Governance discipline (11 skills, SK-529..SK-539):         COMPLETE
          ...
          SK-539 v1.0.0 — UI/UX compliance, 29 checks, 52 roles, 7 templates
Layer 9 — Response construction (1 protocol, v1.0):                  COMPLETE
Reference — XIIGEN-CODEBASE-ORIENTATION-MAP v1.2:                    updated v4.4.0
          Not a skill (no SK number). Q-21..Q-23 question-class lookup.

Total: 114 skills + 1 protocol + 1 reference document
Next available: SK-540
Next available FC: FC-19
...
**Sessions started under v4.4.0:**
- SK-539 loads at 5.5 before any React page is written. Sessions that produce only server-side code skip it (declare "No React pages — SK-539 N/A").
- Check 15 fires at every STOP that produced React pages. Server-only sessions declare N/A.
- CODE-REVIEW-PROTOCOL v1.7 is the plan-review version. Plans reviewed under v1.6 remain approved; re-review under v1.7 may surface Gate 0m findings for React pages plans — expected behavior, not regression.
- DESIGN-REVIEW-PROTOCOL v1.4 adds Signal 12. Prior fleet reviews (v1.3) are not retroactively invalid; Signal 12 MISSING for flows that never produced React pages is acceptable.

**Plans approved under CODE-REVIEW-PROTOCOL v1.6:**
- Remain approved.
- Re-review under v1.7 may surface Gate 0m findings for plans that included React pages without role declarations or Phase 7 steps — expected behavior, not regression.
...
- Q1 ROLE_TIER declared for both pages: `DynamicFormsWorkflowsPage` → TENANT_OPS; submit page → PUBLIC
- Q3 ROUTE_GUARD: admin CRUD page stays at existing route; new `/forms/:schemaId` route created for public submitter
- SK-539 §6 missing-page check: `/forms/:schemaId` is in the registry — page is created in this session
- Phase 7 declared as a plan step after the React pages phase
- At STOP: Check 15 fires — FC-18 Audit Trail at `docs/ux-review/dynamic-forms-workflows/FC-18-AUDIT-TRAIL.md` produced; all BLOCK findings cleared; UX-25 noted as CONCERN (admin CRUD still present) in carry-forward

**If session produces the schema editor page AND the public `/forms/:schemaId` route AND FC-18 Audit Trail:** governance worked.

**If session produces only the admin CRUD page without the public submitter route:** Check 15 fires on UX-20 — STOP does not fire.
...
- Q9 from SESSION-GUIDE v1.7 fires: FLOW-32 named, 4-step read confirmed complete (steps 1-4 = 4 more reads, total 12/20)
...
## END OF HOW TO USE XIIGEN SKILLS v4.4.0
