...
description: >
  ...
  higher (over-engineering) or lower (treating symptoms).
triggers:
...
## REJECTION-RESPONSE — do not throw away a valid Level-2/3 fix as "over-engineered"

The anti-patterns above guard one direction: do not escalate a Level-1 symptom
into a Level-3 redesign. This section guards the OPPOSITE, equally common
failure: a genuinely-needed Level-2 (structural) or Level-3 (architectural) fix
gets dismissed as "over-engineering", "scope creep", "we don't need that now", or
"just patch the symptom" — and the root cause is left in place to recur.

```
When a Level-2/3 fix is challenged as over-engineered, do NOT silently drop it.
Run this test first:

  1. Has this problem (or its siblings) recurred before?         → recurrence proves Level-2+.
  2. Does the Level-1 patch leave the structure/design that
     produced the symptom unchanged?                             → yes = the root is still open.
  3. Will the same root produce a new symptom elsewhere?         → yes = Level-3, not over-engineering.

If any answer is "yes", the Level-2/3 fix is VALID. Keep it. A reviewer calling
it "over-engineered" must instead say which of the three tests fails — otherwise
the rejection is itself the bug (it re-installs the symptom-patch loop).
```

**Valid Level-2/3 ≠ over-engineering. Over-engineering is acting ABOVE the
demonstrated level** (e.g. proposing a new flow when a one-line gate addition
fixes it). Acting AT the demonstrated level — even when that is architectural —
is correct, not excess. Record the rejection and the reason it was overruled so
the decision is auditable; do not let "that's too much" quietly downgrade a fix
the recurrence evidence already justifies.
