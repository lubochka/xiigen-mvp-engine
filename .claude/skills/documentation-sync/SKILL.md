...
description: >
  ...
  or CLAUDE.md are left stale. Invoked at session end — not at session start.
---
...
|-------------|-------------|----------------|
| `server/src/engine-contracts/*.ts` | ENGINE_ARCHITECTURE_MERGED, CLAUDE.md | Update factory list, contract shape, artifact numbers |
...
| New task type (T-XXX) registered | TASK_TYPES_CATALOG_MERGED, CLAUDE.md | Add entry: T-XXX name, factory ID, flow(s) |
| New factory ID (F-XXXX) registered | ENGINE_ARCHITECTURE_MERGED, CLAUDE.md | Add entry with fabricType, taskTypes[], bfaRegistration |
| `CLAUDE.md` artifact number section | STATE-Pn.json reservedThisSession | Confirm numbers match what was committed |
| `.claude/skills/*/SKILL.md` added or changed | `.claude/skills/SKILL-INDEX.md` **AND** the `.agents/skills/` mirror **AND** `.agents` index | Both catalogs must reflect the same skill — see "Two-catalog sync" below |
...
mvp ships **two** skill catalogs — `.claude/skills/` (~358) and `.agents/skills/`
(~65). They are parallel projections of the same skill set. At session end, a
...
☐ For every SKILL.md added/changed this session:
    ☐ present & current in .claude/skills/  (+ row in .claude/skills/SKILL-INDEX.md)
    ☐ present & current in .agents/skills/   (+ .agents index/AGENTS.md), OR
...
Artifact number audit:
  ☐ If new F-XXXX or T-XXX was assigned: confirm CLAUDE.md shows updated next-available number
...
2. **"The code is self-documenting."** The canonical docs are machine-readable context for future Claude Code sessions. Stale docs cause stale plans, which cause stale artifact numbers, which cause collisions.

3. **"I updated CLAUDE.md but not ENGINE_ARCHITECTURE_MERGED."** Use the maintenance rules table — multiple docs often need the same update. One file change can touch 2–3 docs.
