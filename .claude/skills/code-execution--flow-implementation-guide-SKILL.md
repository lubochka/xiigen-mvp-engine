...
V12: NODE REPRESENTATION INTEGRITY
...
## PHASE F SELF-MODEL UPDATE (4 steps after ACTIVE registration)

After registering FLOW-XX as ACTIVE in flow-lifecycle, run these four steps:

**F-SM-1: SEED ARTIFACT RANGE TO RAG**

Seed the artifact range document so future sessions can query next-available numbers:
```json
{
  ...
  "flowId": "FLOW-XX",
  "status": "ACTIVE",
  "taskTypes": { "range": "T${start}-T${end}", "next": "T${end+1}" },
  "factories": { "range": "F${start}-F${end}", "next": "F${end+1}" },
  "families": { "assigned": "${N}", "next": "${N+1}" },
  "bfaRules": { "range": "CF-${start}-CF-${end}", "next": "CF-${end+1}" }
}
```
Verify: `curl -sf "localhost:9200/xiigen-rag-patterns/_search" -d '{"query":{"term":{"patternId.keyword":"artifact-range::FLOW-XX"}}}' | jq '.hits.total.value'` → Expected: 1

**F-SM-2: VERIFY ARCHITECTURE DECISIONS SEEDED**

At least one ARCHITECTURE_DECISION document must exist for this flow in RAG.
```bash
ARCH_COUNT=$(curl -sf "localhost:9200/xiigen-rag-patterns/_count" \
  -d '{"query":{"bool":{"must":[{"term":{"patternType.keyword":"ARCHITECTURE_DECISION"}},{"term":{"flowId.keyword":"FLOW-XX"}}]}}}' | jq .count)
echo "Architecture decisions for FLOW-XX: ${ARCH_COUNT}"
```
If 0: seed from FLOW-XX-ARCHITECTURE-DECISIONS.json now.

**F-SM-3: UPDATE CLAUDE.md FROM RAG (rendered view)**

Query all ARTIFACT_RANGE documents to compute true next-available numbers:
- `max(taskTypes.next)` across all ACTIVE flows = true next T
- `max(factories.next)` = true next F
- `max(families.next)` = true next Family

CLAUDE.md is a rendered view. The RAG is the source of truth. Never edit CLAUDE.md manually.

**F-SM-4: VERIFY NODE REPRESENTATIONS PROPAGATED**

Every NODE seeded in Phase A must have `patternType=NODE_REPRESENTATION` in xiigen-rag-patterns.
A NODE that is never retrieved may have content too abstract or too implementation-specific.
Fix: revise purpose and domainConcepts fields to be more discoverable.
