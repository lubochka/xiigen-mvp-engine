...
**Classification threshold (universal binding rule, from core `shared-service-design`):**
- Appears in ≥3 UMLs/flows → SHARED INFRASTRUCTURE: **a fabric interface is mandatory,
  and it must be built (or at least stubbed as the interface + DI registration)
  BEFORE the first consumer.** A shared capability with no fabric interface is the
  drift defect this skill exists to prevent — not an acceptable "build it later".
- Appears in 2 UMLs/flows → REVIEW: may be shared depending on usage type (Pattern below)
- Appears in 1 UML/flow → PER-FLOW: build inside that flow

> **Split note (binding):** this skill (SK-495) is the *identification* half of the
> core `shared-service-design` skill — it counts appearances and applies the 3/2/1
> threshold. The *design* half is `planning--shared-infrastructure-design-SKILL.md`
> (SK-504): once a service crosses the ≥3 line here, SK-504 designs its fabric
> interface, mock provider, and build-before-first-consumer sequence. Run them as a
> pair; neither alone closes the core `shared-service-design` requirement.
