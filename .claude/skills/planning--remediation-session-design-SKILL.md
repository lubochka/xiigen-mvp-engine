...
**Map the order onto SK-432's three ladder levels (Level-3 → Level-2 → Level-1):**

```
Level 3 (architectural — design artefact / locked decision / new contract) FIRST
  → everything downstream is wired against it; do it before structure and content.
Level 2 (structural — missing/updated skill, template, or process gate) SECOND
  → fixes the pattern that allowed the symptom, once the architecture exists.
Level 1 (immediate — the specific broken thing) LAST
  → the symptom fix lands on top of the corrected structure and architecture.

This is the SAME ordering as "INFRASTRUCTURE before EXTENSION before CONTENT"
and "registration before injection" — those are the fix-class projections of the
Level-3 → Level-2 → Level-1 ladder. A 5-line Level-3 change still goes first
because everything downstream requires it; order is by dependency level, not by
effort or line count.
```
...
Gate B:       [exact command] returns "[N] passed, 0 failed"
              [for INFRASTRUCTURE groups also: npm run build → exit 0, no TS errors]
...
**`Gate B:` is mandatory on EVERY group — no exceptions.** A group is the unit of
remediation; if it has no Gate B, "done" cannot be decided. "Works correctly",
"looks fixed", or "tests should pass" is NOT a Gate B. A Gate B is an exact
command and an exact expected result you can compare:

```
this repo (NestJS + React + FastAPI sidecar):
  server group:        npm test            (or  npx jest --testNamePattern "<t>")  → "N passed, 0 failed"
  client/UI group:     npx playwright test  (or  npx playwright test -g "<title>")  → "N passed"
  INFRASTRUCTURE group: ALSO  npm run build  → exit 0, no TypeScript errors
  RAG sidecar group:   pytest -k "<test>"  → "N passed"  (where applicable)
```

(C# `dotnet test` / `dotnet build` framing from the upstream `llm_mvp_core`
standard is replaced by the commands above for this repo.)
...
Gate B: npx jest --testNamePattern "learning loop|engine-bootstrapper|feedback.handler" → "N passed, 0 failed"
...
Gate B: npx jest --testNamePattern "score.handler|arbiter-context" → "N passed, 0 failed"
        AND npm run build → exit 0 (new file + module registration = INFRASTRUCTURE-touch)
...
Gate B: npx jest --testNamePattern "rag-retrieve|flow-rag-seed" → "N passed, 0 failed"
...
Gate B: npx jest --testNamePattern "patternType|design-reasoning seed" → "N passed, 0 failed"
```

Every group above carries an explicit `Gate B:` line — that is the rule, not a
decoration. Ordering note for this example: there is no Level-3 architectural
group here, so the plan runs Level-2/structural wiring (Groups A–B) before the
Level-1/content-adjacent additions (Groups C–D), and Group B's module
registration (infrastructure-touch) precedes any injection that depends on it.
...
❌ A group with no explicit Gate B line
   → "Works correctly" / "looks fixed" is not a Gate B. Without an exact command
     and an exact expected result, the group's "done" cannot be decided.
   → Every group needs `Gate B: <command> returns "N passed, 0 failed"`.

❌ Ordering groups by effort or line count instead of ladder level
   → Level-3 (architectural) goes first even when it is 5 lines, because every
     downstream group is wired against it. Order is Level-3 → Level-2 → Level-1
     (= INFRASTRUCTURE → EXTENSION → CONTENT; registration before injection).
```
...
Feeds into:    planning--assumption-registry-SKILL.md (SK-456) — pre-flight per group
               ...
               code-execution--diagnostic-escalation-SKILL.md (SK-501) — when a
                 group's Gate B goes red, diagnose the failure layer (TEST_DATA/
                 WIRING/STUB/ALGORITHM) and decide self-fix vs escalate
