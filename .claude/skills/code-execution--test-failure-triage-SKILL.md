...
> **IRON RULE (universal — does not bend for Class 3).** Changing what a test
> **asserts** (the `expect(...)` value or the asserted error code) requires **Luba's
> explicit approval**. Class 3 permits rewriting *how* the test reaches the
> behavior (replace `toHaveBeenCalledTimes`/exact-string/order-dependence with a
> behavioral assertion), but it does **not** authorize loosening or changing the
> asserted outcome to make a red test green. If the only way to make the test pass
> is to weaken its assertion (e.g. `expect(result.isSuccess).toBe(true)` → `...not.toThrow()`,
> or changing the expected `errorCode`), that is **not Class 3** — it is a real
> failure (Class 1/2/4) and "adapting the test to accept the output" is a session
> failure (see `bug-to-tests` Iron Rule). Record the approval reference before any
> assertion edit.
...
### Class 4 also covers TEST-ENVIRONMENT failure (fails BEFORE Act) — universal

The universal core meaning of Class 4 is broader than the AF-pipeline case: a
**test-environment failure** is any failure where the test never reaches its **Act**
step — the harness/setup blew up first. The decisive signal is the **line number**
of the failure relative to the Arrange / Act / Assert structure:

- **Fails BEFORE Act (Arrange/setup line)** → **Class 4 test-environment failure.**
  The system-under-test was never exercised, so the assertion proves nothing.
  Typical NestJS/Jest markers:
  ```
  Nest can't resolve dependencies of <Service> (?, ...)        # DI graph broken in fixture
  No provider for <Token>                                       # missing provide: in Test.createTestingModule
  Cannot find module / TypeError in beforeAll/beforeEach        # fixture/import error
  ```
  Response: fix the **fixture/DI wiring** (`Test.createTestingModule` providers,
  imports, env). Do NOT touch production code or the assertion — the Act never ran.
- **Fails ON the Assert line (after Act ran)** → **Class 1** (implementation/generation
  regression): the code executed and produced the wrong value.

```bash
# Read the failing line number from the jest stack and locate it in the spec:
grep -nE "beforeAll|beforeEach|// Act|// Assert|expect\(" test/<spec>.spec.ts
# Failure line sits in Arrange/beforeEach  → Class 4 (env). Failure line is an expect() → Class 1.
```

Misclassifying a Class 4 (pre-Act/DI) failure as Class 1 leads to "fixing" correct
production code to satisfy a test that never ran — the most common triage error.
