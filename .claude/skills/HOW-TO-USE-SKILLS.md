# HOW TO USE XIIGEN SKILLS — v5.0
## Updated: 2026-04-21 | For: Claude.ai Project custom instructions
## Status: Current — supersedes v4.9.0
...
□ SK-529 v2.1.0 reconnaissance threshold met (STATE.recon saved)
...
8. RECONNAISSANCE EVIDENCE (SK-529 v2.4.0)
   □ STATE.recon exists
   ...
   □ Every "works" or "exists as implemented" claim cites IMPLEMENTATION or TEST excerpt (Gate 3)
...
Layer 8 — Governance discipline (11 skills, SK-529..SK-539):         COMPLETE
          SK-529 now at v2.4.0 — evidence-layer tags (v2.2.0);
...
Layer 14 — Multi-entity Examination Protocol (1 skill, SK-552):      COMPLETE ← NEW v5.0
          SK-552 v1.0.0 — per-entity-examination-protocol: systematic N ≥ 5 instance
...
Total: 127 skills + 1 protocol + reference documents
Next available: SK-553
Next available FC: FC-19
```
...
This overlay supplements existing XIIGen skill routing for universal process guidance.
Resolve exact universal skill file paths through `.claude/skills/SKILL-INDEX.md`;
if the local index uses a different canonical name, follow the local index.

Universal skills guide agent behavior. They do not override `AGENTS.md`, `STATE.json`,
the current HOW-TO sections above, or active source files.

### Universal Load Rule

1. Read `AGENTS.md` and the read-first files listed there.
2. Identify the current task trigger in the table below.
3. Load the smallest matching set of universal skills.
4. Read each selected skill file completely before acting.
5. Keep XIIGen MVP source conventions intact while applying the skill.

### Universal Trigger Map

| Task trigger | Load universal skills | XIIGen MVP notes |
|---|---|---|
| Any coding or documentation task | `planning-session-startup`, `authority-requirement-binding`, `work-scope-inventory`, `reconnaissance-gate`, `claim-verification`, `dev-safety` | Start from local repo rules and active source, not memory or another branch. |
| Broad or risky change | `phase-planning`, `plan-review`, `plan-self-validation`, `multi-reviewer-design`, `work-scope-inventory` | Bound the work, record assumptions, and keep user decisions explicit. |
| Server service, fabric, provider, or factory work | `code-examination`, `implementation-integrity`, `test-integrity`, `verification-before-completion` | Preserve `DataProcessResult<T>`, NestJS providers, injection tokens, dynamic payloads, and local config defaults. |
| Search, persistence, tenant-scoped document work | `code-examination`, `implementation-integrity`, `test-integrity`, `claim-verification` | Use `buildSearchFilter` or `buildSearchFilterFlat`; skip empty criteria; keep documents dynamic with `Record<string, unknown>`. |
| AI, RAG, flow engine, AF station, or orchestration work | `code-examination`, `implementation-integrity`, `authority-requirement-binding`, `test-integrity` | Implement from XIIGen-owned interfaces and active source only. Do not import `llm_mvp_core` internals or source design patterns. |
| Client UI, hooks, pages, or theme work | `code-examination`, `implementation-integrity`, `test-integrity`, `verification-before-completion` | Follow `client/src` structure and verify user-facing flows when behavior changes. |
| Debugging or failing tests | `systematic-debugging`, `root-cause-tracing`, `code-examination`, `verification-before-completion` | Reproduce first, identify the failing layer, then change the smallest relevant code. |
| Adding or changing tests | `test-integrity`, `coverage-completeness-gate`, `verification-before-completion` | Tests must prove real behavior, not only implementation details. |
| Code review or generated-code review | `claim-verification`, `code-examination`, `implementation-integrity`, `multi-reviewer-design`, `output-readability-gate` | Findings need file-grounded evidence. The executor should not self-approve substantial changes. |
| Documentation or guidance updates | `documentation-sync`, `output-readability-gate`, `claim-verification`, `coverage-completeness-gate` | Keep docs aligned with active NestJS source and avoid inactive-stack guidance as active text. |
| Secrets, keys, environment, or tenant isolation | `dev-safety`, `authority-requirement-binding`, `work-scope-inventory`, `verification-before-completion` | Do not expose secrets. Preserve tenant scoping and existing secret-provider boundaries. |
| Final response or handoff | `goal-delivery-completeness`, `session-output-contract`, `verification-before-completion`, `output-readability-gate` | Summarize changed files, verification, and remaining risk clearly. |

### Universal Conflict Handling

- If a universal skill conflicts with `AGENTS.md` **or** `CLAUDE.md`, follow `AGENTS.md`/`CLAUDE.md`. Both agent files outrank every skill file in this library — no skill, FC gate, V-gate, or HOW-TO section overrides them.
- If guidance conflicts with active source files, follow the source files and update guidance only when the user allows it.
- If a mapped skill is absent from the local index, use the nearest indexed universal skill that matches the trigger. Do not copy skill content from another branch or from `llm_mvp_core`.

### Universal Single-Source Rule (entropy reduction)

This file is the single source of truth for skill load order in the MVP. Versioned siblings
(`HOW-TO-USE-SKILLS-vX.Y.Z.md`, `SKILL-INDEX-vX.Y.md`, `FLOW-DESIGN-SKILL-INDEX-vX.Y.md`)
and the parallel `XIIGEN-SESSION-LOAD-PLAN` / `XIIGEN-SESSION-START-PROMPT` documents are
historical/archived references, not competing indexes. When two indexes disagree, this file
wins; when this file and `AGENTS.md`/`CLAUDE.md` disagree, the agent files win. The same load
order applies whether the skill is read from `.claude/skills/` or `.agents/skills/` — keep
those two libraries reconciled to one canonical body per skill, never two diverging copies.

### Universal No User-Facing STOP Without an Explicit Gate B

A skill, FC gate, or V-gate may declare an internal ⛔ STOP, but it must not become a
user-facing stop, approval-wait, branch switch, or full-test-suite run unless the active
work order or `AGENTS.md`/`CLAUDE.md` states an explicit Gate B that authorizes it. In this
MVP (NestJS + React, TypeScript), an explicit Gate B is a concrete, measurable pass condition
expressed in the project's own tooling:

- build / type gate: `npm run build` (server and client) returns 0 errors;
- unit gate: `npx jest <path>` (or the package script) reports `0 failed` for the named suite;
- e2e / UI gate: `npx playwright test <spec>` passes the named scenario.

"Works correctly" is not a Gate B. A count, a comparison, or a named passing suite is.
Translate legacy skill instructions accordingly: legacy `STOP` is an internal check while a
known `next_action` exists; legacy `dotnet test` / "must be on main" map to the MVP's
`jest`/`playwright` Gate B and to "record/verify the current branch — never switch branch as
an implicit fix"; legacy `run the full test suite` runs only when the active Gate B or the
user's current instruction requires it.

### Universal Plan-Governance Orchestration Block (every multi-step plan)

Every multi-step plan produced in this MVP must carry its own plan-governance orchestration
block — anti-stop behavior, role transitions, skills, state tracking, and completion checks
are not left as scattered background rules. The block must define, at minimum:

- the Requirements Ledger extracted from the authority before any plan prose;
- mapping from every requirement to plan section + test/gate (Jest/Playwright) + artifact;
- a contradiction pass proving no plan sentence conflicts with a binding requirement;
- the role loop `PLAN_AUTHOR → PLAN_REVIEW → EXECUTOR → REVIEW → ARCHITECT_REPAIR → EXECUTOR_CONTINUE`;
- the branch rule (current branch only), allowed verification, and forbidden broad tests;
- state fields/update points, an anti-loophole register, and the completion coverage matrix;
- commit/push treated as a checkpoint, not a user-facing stop gate.

A plan missing this block is repaired (the block is added) before execution; adding the block
is not a pause and not permission to wait for approval when a safe `next_action` is derivable.
