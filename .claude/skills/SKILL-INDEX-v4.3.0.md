...
## Date: 2026-04-23 | v4.3.0 | 127 skills + 1 protocol | Single source of truth
...
| **test-integrity** | **2.1.0** | 6 rules: branch reachability, contract testing + **Rule 6 behavioral assertion gate (D2-F1)** — stub count = 0, domain-outcome assertions required |
| **self-verification** | **1.1.0** | 6 post-fix verification rules + **PORTABILITY_REMEDIATION change category** — V9 + concurrent isolation test, not L1 only |
...
| **dna-compliance-guard** | **1.1.0 MANDATORY** | 9 DNA patterns + **P-1..P-5 portability checks** (ClsService, @connectionType, FREEDOM key prefix, no local interfaces, requiredCoInstalls). Priority MANDATORY. |
...
| **flow-implementation-guide** | **1.2.0** | 7-step protocol + **Step 3.b** (@connectionType TS annotation) + **V9 portability gate** (MOBILE / PARTIAL_GAP / NOT_PORTABLE) |
| **flow-prerequisites** | 1.0 | Check prerequisites before starting |
| **retroactive-development** | **1.1.0** | Engine fix propagation + **portability fix propagation table** (P-1..P-5 rows: service fix + genesis prompt fix + 8-step protocol) |
...
| code-execution/code-execution--phase-preflight-SKILL.md | SK-457 | **v1.1.0** — Claude Code session START gate; default check #5: portability prerequisites (ClsService scan of existing services); Pattern 4 portability resumption |
...
| code-execution/code-execution--generated-code-review-SKILL.md | — | **v1.1.0** — After Phase B generates: 4-layer review; Layer 2 DNA-5 fixed (ClsService import, not scope_id); Layer 4 added: behavioral assertion gate (D2-F1) |
...
Layer 1  — Engine internals (47 skills, SK-426..SK-470):               COMPLETE
           ...
           retroactive-development **v1.1.0** — portability fix propagation table ← NEW v4.3.0
           phase-preflight **v1.1.0** — default check #5 portability prereqs ← NEW v4.3.0
           generated-code-review **v1.1.0** — DNA-5 fix + Layer 4 behavioral assertion ← NEW v4.3.0
           test-integrity **v2.1.0** — Rule 6 behavioral assertion gate (D2-F1) ← NEW v4.3.0
           self-verification **v1.1.0** — PORTABILITY_REMEDIATION change category ← NEW v4.3.0
...
Reference documents (no SK):
  ...
  FLOW-DOCUMENT-AUTHORING-GUIDE **v1.18** — updated v4.3.0 ← BUMPED
    Rule 37: H-29 cross-flow event contract extension protocol
  SK-553 flow-portability-test-protocol **v1.0.0** — NEW v4.3.0
    Loadable wrapper; MOBILE → TENANT-READY; three-layer certification
    ROUND CONTRACT; MANDATORY STOP FORMAT; Q4 FORMAT RULE;
    code skill version bumps (v4.3.0); portability activation triggers added
  ...
  GUIDE-B17 **v6.2** — updated v4.3.0 ← BUMPED
    Phase G Mobility Gate (C34): P-1..P-5 bash checks after Phase F
  GUIDE-B21 **v3.1** — updated v4.3.0 ← BUMPED
    Portability constraints in FLOW-SPECIFIC CONSTRAINTS template
  GUIDE-B04 **v3.1** — updated v4.3.0 ← BUMPED
    Q5 redefined: actual portability conditions, not scope_isolation arbiter
  GUIDE-B46 **v3.1** — updated v4.3.0 ← BUMPED
    Step 0d: AdminCrudPanel/PlatformOpsPage coupling check
  prompt-to-claude **v3.1** — updated v4.3.0 ← BUMPED
...
Reference documents (no SK):
  XIIGEN-DESIGN-ARCHITECT-SESSION-GUIDE **v2.2** — updated v4.1.0
    ...
    v2.2: correction severity classification (LOCAL/TRAJECTORY/SESSION-RESTART)
    Mistakes catalog: 1–31; Observability checklist updated
  HOW-TO-USE-SKILLS **v5.1** — updated v4.2.0
