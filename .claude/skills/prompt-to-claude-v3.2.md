...
## Version: v3.2
## v3.2 amendment: Rule 8 (Phase H auth decoration mandatory for HTTP controllers),
##   Rule 9 (Phase I tenant certification mandatory for distributed flows),
##   FP-7 (controller without auth declarations), FP-8 (no tenant cert tier for
##   distributable flow), Key Files section extended with auth + cert guidance files.
##   Closes AUTH-ARBITER-SKILLS-REMEDIATION-PLAN-v3.0 Phase 19.
...
8. AUTH DECORATION — Include Phase H (Auth Decoration) in the implementation plan
   (GUIDE-B17 v6.3). Phase H is mandatory for every flow that produces HTTP controllers.
   Phase H runs AFTER Phase G and performs three checks:
     A-1: @UseGuards on every controller
     A-2: @Roles or @Public() on every route
     A-3: @Public() routes listed in bypass-paths.registry.ts
   If auth.module.ts is absent: declare "AUTH_DEFERRED — decorate in Phase H after
   AUTH-ROLES-GROUPS-PLAN-v3.0 Phases 1-4 deploy." Do NOT skip Phase H silently.
   A plan with controllers but without Phase H ships unguarded endpoints.
9. TENANT CERTIFICATION — Include Phase I (Tenant Certification Status) in the
   implementation plan (GUIDE-B17 v6.3). Phase I is mandatory for every distributable
   flow (any flow targeting TIER-A or above per SK-553 v1.1.0).
   Phase I runs AFTER Phase H and records the tenantCertTier in IMPL-STATE.json:
     TIER-A: SK-553 Layer 1 PASS (or AUTH_DEFERRED documented)
     TIER-B: TIER-A + repo {tenantId}--{moduleName} + evidence PNG
     TIER-C: TIER-B + AI Adaptation + per-role visual + R6 + Guard 14
     TIER-D: full per-role coverage at all cascade points
   If the flow is not distributable: declare "Not targeting distribution — Phase I N/A."
   A plan without Phase I leaves tenantCertTier = NONE in IMPL-STATE.json forever.
...
| Implementation | Phase H (auth) + Phase I (cert) | Required before GOAL_REACHED on flows with controllers |

---

## The 8 Failure Patterns the Library Prevents
...
| FP-7 | HTTP controller shipped without @UseGuards/@Roles — unguarded endpoint | GUIDE-B17 Phase H mandatory — NEW v3.2 |
| FP-8 | Distributable flow with tenantCertTier = NONE — tenant marketplace blocked | GUIDE-B17 Phase I mandatory — NEW v3.2 |
...
Evidence for FP-7: dna-compliance-guard v1.2.0 A-1/A-2 checks found unguarded controllers
across the fleet. Root cause: implementation plans lacked a dedicated auth decoration phase.
Every flow with HTTP controllers requires @UseGuards + @Roles/@Public() declarations before
shipping; absent auth.module.ts → AUTH_DEFERRED (acceptable), absent decoration → FP-7.

Evidence for FP-8: IMPL-STATE.json across 49 flows: tenantCertTier field absent or "NONE"
in all cases. Root cause: no implementation plan phase ever wrote this field. A flow cannot
be listed on the tenant marketplace without a declared cert tier — even TIER_A (Layer 1 pass)
requires explicit certification to unlock distribution.
...
**Auth and certification (attach when producing implementation plans — NEW v3.2):**
12. `GUIDE-B17-IMPLEMENTATION-PLAN-v6.3.md` — Phase H (auth decoration A-1..A-3 + Rule 7)
    + Phase I (tenant cert tier TIER-A/B/C/D + Guard 14 + TIER-C checklist)
13. `code-execution--flow-implementation-guide-SKILL-v1.3.0.md` — V10 authStatus gate,
    V11 tenantCertTier gate, TIER-C pre-certification checklist
14. `code-execution--test-integrity-SKILL-v2.2.0-GR001.md` — Rule 7 (401/403 tests per
    controller), Rule 8 (R6 cross-tenant JWT isolation)
15. `code-execution--dna-compliance-guard-SKILL-v1.2.0.md` — A-1..A-3 auth checks,
    D-HIST-001 SDK import check, AUTH GATE + D-HIST-001 gate verdicts

**NDJSON type coverage (attach when producing TEACH-QA or Phase A):**
16. `code-execution--arbiter-ndjson-requirements-SKILL-SK-554.md` — minimum type matrix
    (scope_isolation mandatory, security for PII flows, domain for business-logic flows),
    6 detection commands, UC-7 trigger conditions
...
The relevant guidance file for whatever specific output you need
...
Prompt version: v3.2 | 9 rules | 8 failure patterns
