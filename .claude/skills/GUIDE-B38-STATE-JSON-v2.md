...
## Date: 2026-04-24
## v2 amendment: portabilityStatus, authStatus, tenantCertTier, portabilityTest,
##   protocolStatus fields added to IMPL-STATE.json schema (Variant 2). Phase G/H/I
##   labels added. Initialization script extended. Acceptance criteria + Key Rule 6.
##   Closes AUTH-ARBITER-SKILLS-REMEDIATION-PLAN-v3.0 Phase 22.
...
- Valid phase labels include G (`"Portability mobility gate V9"`), H (`"Auth decoration A-1..A-3"`),
  I (`"Tenant certification status TIER-A/B/C/D"`) — added in GUIDE-B02 v6.2
...
const state = {
  ...
  portabilityTest: {
    phase0Auth: { status: 'NOT_RUN', date: null, evidence: null },
    layer1: { status: 'NOT_RUN', date: null, evidence: {
      connectionAnnotations: null, clsServiceHits: null,
      dHist001SdkImports: null, stubTests: null,
      behavioralAssertions: null, tenantIsolation: null
    }},
    layer2: { status: 'NOT_RUN' },
    layer3: { status: 'NOT_RUN' },
    r6AuthIsolation: { status: 'NOT_RUN' },
  ...
  protocolStatus: {
    sk553Layer1: 'NOT_RUN', sk553Phase0: 'NOT_RUN',
    ndjsonScopeIsolation: 0, ndjsonNoTypeRecords: 0, dHist001SdkImports: 0
  }
};
...
- [ ] **[v2] `portabilityStatus` is not `"TBD"` on PHASE_F+ sessions (Phase G ran)**
- [ ] **[v2] `authStatus` is not `"TBD"` on PHASE_F+ sessions (Phase H ran)**
- [ ] **[v2] `tenantCertTier` is set (non-NONE) for distributable flows (Phase I ran)**
- [ ] **[v2] `protocolStatus.ndjsonScopeIsolation` ≥ 1 (SK-554 scope_isolation record)**
- [ ] **[v2] `protocolStatus.dHist001SdkImports` = 0 (no SDK imports in service files)**
...
**6. portabilityStatus and authStatus TBD blocks GOAL_REACHED. (NEW v2)**
IMPL-STATE.json is the canonical source for the flow's portability and auth posture.
When the PHASE-COMPLETE.md and QA session read `portabilityStatus = TBD` or
`authStatus = TBD`, that is the signal that V9 (Phase G) or V10 (Phase H) never ran.

A flow with TBD portability/auth status is not implementation-complete regardless of
how many phases show COMPLETE. The correct sequence is:
  Phase F COMPLETE → Phase G (V9 gate) → Phase H (V10 gate) → Phase I (V11 gate)
  → all three write their status → GOAL_REACHED verdict available

TBD values in portabilityStatus or authStatus make GOAL_REACHED unavailable.
They must be resolved — not documented as "acceptable TBD" — before the verdict
can be set.

**7. protocolStatus is the aggregate summary for the QA gate. (NEW v2)**
The `protocolStatus` object aggregates the most important protocol check results
into one place so the QA session (SK-481 v2.0.0 Step 5b) can read them without
parsing nested portabilityTest evidence objects. Write `protocolStatus` atomically
at the close of each protocol gate phase — not incrementally by field.

---

*End of GUIDE-B38 — STATE.json (with ZIP-15 §1 role registry integration) v2*
...
*Round: 48 of 72*
*v2 amendment: portabilityStatus, authStatus, tenantCertTier, portabilityTest,*
*protocolStatus added to IMPL-STATE.json schema and initialization script.*
*Phase G/H/I labels noted. Acceptance criteria extended. Key Rules 6 + 7.*
*Closes AUTH-ARBITER-SKILLS-REMEDIATION-PLAN-v3.0 Phase 22.*
