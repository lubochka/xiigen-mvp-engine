...
## STEP 0 — EXAMINATION RECORD + DESIGN CONTEXT + COUPLING CHECK (updated v3.1)

Before applying the domain-screen classification gate, run all four sub-steps.
Steps 0a–0c existed. Step 0d is NEW — it catches the AdminCrudPanel coupling
that causes 16 flows to be non-portable even after CRUD tables are fixed.
...
# Step 0d — AdminCrudPanel / PlatformOpsPage coupling check (NEW v3.1 — GAP-07/GAP-08)
grep -rl "AdminCrudPanel\|PlatformOpsPage" client/src/pages/ 2>/dev/null
# If any files listed: determine sub-type before writing any screen spec:
#
#   ACP-SPLIT: The page has a clean public/tenant branch that can be extracted.
#              A new purpose-built page wraps only that branch.
#              The AdminCrudPanel import is removed from the new page.
#
#   ACP-FUSED: All role paths go through AdminCrudPanel with no separable branch.
#              A complete purpose-built replacement is required.
#              Do not author client screen spec until replacement is confirmed in plan.
#
# WHY THIS MATTERS: AdminCrudPanel and PlatformOpsPage are engine-internal components.
# A React page that imports them cannot be packaged as a standalone component
# for a second tenant. It produces a distributable that hard-depends on engine internals.
# Finding this BEFORE writing the spec saves a full rewrite later.
```
