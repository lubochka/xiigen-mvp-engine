...
## Every Delta Needs a Numeric Source (universal, from core)

A delta is valid ONLY if it has a numeric source from a real command run — never
an estimate, never "about +N", never a recalled count:

| Metric chain | Numeric source of each delta |
|--------------|------------------------------|
| Server test count | `npx jest 2>&1 \| tail -3` (passing total before→after) |
| UI e2e count | `npx playwright test 2>&1 \| tail -3` |
| RAG eval (recall/precision) | a numeric FastAPI eval run, before→after |
| Type errors | `npx tsc --noEmit` count |

If a delta has no command-backed source, the chain is unverified — do not accept
the final total.

## Mismatch → Recompute, Not Nitpick (universal, from core)

When `claimed_final ≠ baseline + Σ(deltas)`, the audit's job is to REQUIRE a
recompute and name the exact transition where the math breaks — not to quibble
about wording. Output the failing edge with both values, then recompute the
correct final from the command-backed deltas. The corrected number replaces the
claimed one before any completion claim.
