...
| GOVERNANCE | Affects agent/CI behavior | `.claude/` files, skill files, `SESSION-*.md` | Separate commit BEFORE code files |
| PRODUCTION-CI | Affects build/deploy pipeline | `ci.yml`, `docker-compose.yml` | YAML validation + rollback command + scope guard |
| SOURCE | Affects runtime code | `server/src/**`, `client/src/**` | `npx tsc --noEmit` per file |
...
5. **Never mix** TEST-ONLY and PRODUCTION-CI in the same commit
