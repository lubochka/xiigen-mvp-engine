...
### Step 6: Populate Q5 — Cross-Tenant Install

Q5 checks whether the flow's cross-tenant install path works end-to-end.

```bash
# Check for scope portability test
find client/e2e -name "*portab*" -o -name "*scope*" 2>/dev/null | grep -i "{slug}"
# Check provisioning controller
ls server/src/engine/flows/{slug}/*provision*.ts 2>/dev/null
ls server/src/engine/flows/{slug}/*controller*.ts 2>/dev/null
# Check scope isolation arbiter
grep -c "scope_isolation" fixtures/arbiters/{slug}-arbiters.bulk.ndjson 2>/dev/null
```

```json
"Q5_cross_tenant_install": {
  "verdict": "PASS | PARTIAL_GAP | NOT_APPLICABLE | TBD",
  "applicability": "DIRECTLY_APPLICABLE | APPLICABLE | NOT_APPLICABLE",
  "applicabilityReason": "{why applicable or not}",
  "scopeIsolationArbiterPresent": true,
  "evidence": [
    "{controller or service that handles cross-tenant operations}",
    "fixtures/arbiters/{slug}-arbiters.bulk.ndjson — scope_isolation arbiter confirmed"
  ],
  "criticalGaps": [
    "{gap if provisioning logic uses stub instead of real implementation}",
    "{gap if cross-tenant install not validated against live ES}"
  ]
}
```

**Q5 verdict rules:**
- `PASS`: applicable AND scope_isolation arbiter present AND provisioning logic real (not stub) AND e2e test passes
- `PARTIAL_GAP`: applicable but stub logic, or e2e not verified against live ES
- `NOT_APPLICABLE`: flow doesn't expose multi-tenant install surface
