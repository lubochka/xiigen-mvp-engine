...
- Multi-model critique: Claude + GPT-4 + Gemini evaluate candidates independently
...
**Negative example (violation):** Every call routes to `claude-opus` regardless of task complexity. Low-complexity code scaffolding costs the same as high-stakes architectural generation. No training data captured.
