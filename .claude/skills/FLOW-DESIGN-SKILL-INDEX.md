...
After v3.2.0: SK-520 (superseded — see NEXT AVAILABLE SK at end of file)

---

## WHAT CHANGED IN v3.3.0 (Layers 6, 6x, 7 — AI topology + enforcement)

Source: SKILL-INDEX-v4.0.0 Layer 6/6x/7 registration 2026-03-26

| Change | What it adds |
|--------|-------------|
| NEW SK-520 | intent-to-plan: FLOW-PLAN session type, step 1 |
| NEW SK-521 | depth-decision: before any plan depth commitment |
| NEW SK-522 | ai-context-package-authoring: FLOW-PLAN session type, step 2 |
| NEW SK-523 | principles-arbiter: any flow with principles requirements |
| NEW SK-524 | cycle-visibility: render cycle state for tenant visibility |
| NEW SK-525 | meta-arbiter: arbitration of arbiters |
| NEW SK-526 | scope-isolation-arbiter: FC-32 enforcement; every arbiter panel |
| NEW SK-528 | pipeline-position-check: Q0 enforcement; GENERATION/PLANNING/MATERIALIZATION |

Note: SK-527 (module-isolation-arbiter, FC-33) is pending — tracked separately.

---

## NEW SKILL TABLE — v3.3.0

### Layer 6 — AI-Driven Topology Planning (SK-520..SK-525)
*FLOW-PLAN session type and arbiter panel design*

| File | SK | Category | Load | When |
|------|----|----------|------|------|
| `planning--intent-to-plan-SKILL.md` | SK-520 | planning | 1 | FLOW-PLAN session type, step 1 |
| `planning--depth-decision-SKILL.md` | SK-521 | planning | 0 | Before any plan depth commitment |
| `planning--ai-context-package-authoring-SKILL.md` | SK-522 | planning | 0 | FLOW-PLAN session type, step 2 — SK-522 FIRST, 10-step process |
| `planning--principles-arbiter-SKILL.md` | SK-523 | planning | 1 | Any flow with principles requirements |
| `planning--cycle-visibility-design-SKILL.md` | SK-524 | planning | 1 | Flows requiring tenant-visible cycle state |
| `planning--meta-arbiter-SKILL.md` | SK-525 | planning | 2 | When arbiters disagree; upper judge function |

### Layer 6x — Scope Isolation Enforcement (SK-526)
*Mandatory for every arbiter panel*

| File | SK | Category | Load | When |
|------|----|----------|------|------|
| `planning--scope-isolation-arbiter-SKILL.md` | SK-526 | planning | 8 | **Every arbiter panel** — FC-32 enforcement; absent = BUILD_FAILURE |

### Layer 7 — Pipeline Position Enforcement (SK-528)
*Unconditional for GENERATION, PLANNING, MATERIALIZATION*

| File | SK | Category | Load | When |
|------|----|----------|------|------|
| `planning--pipeline-position-check-SKILL.md` | SK-528 | planning | 5 | **SK-528 loads unconditionally** before any other skill in GENERATION, PLANNING, MATERIALIZATION sessions |

---

## SKILL ACTIVATION ADDITIONS — v3.3.0

### FLOW-PLAN sessions

```
FLOW-PLAN session (preparing AI-driven topology context packages):
  SK-522 ai-context-package-authoring  ← FIRST; 10-step process; ⛔ STOP after every step
  SK-520 intent-to-plan                ← step 1
  planning--session-file-authoring     ← after SK-522 + SK-520

Before any plan depth commitment:
  SK-521 depth-decision                ← classify shallow / deep before committing

Any flow with principles requirements:
  SK-523 principles-arbiter            ← isolation rule; growth rule; self-reinforcement

Flows requiring tenant-visible cycle state:
  SK-524 cycle-visibility              ← render cycle state design

When arbiters disagree / upper judge needed:
  SK-525 meta-arbiter                  ← arbitration of arbiters
```

### Enforcement — every session with arbiters or pipeline position

```
Every arbiter panel (FC-32):
  SK-526 scope-isolation-arbiter       ← MANDATORY in every node
                                          Absent = BUILD_FAILURE
                                          Also: every archetype includes SK-534 (Goal Delivery) + SK-526 together

GENERATION / PLANNING / MATERIALIZATION sessions:
  SK-528 pipeline-position-check       ← LOAD FIRST, unconditionally
                                          Answers Q0a (receives) / Q0b (produces) / Q0c (consumer) / Q0d (consumer requirement)
                                          Q0d unknown → emit CONTEXT_INSUFFICIENT, halt until answered
```

---

## LAYER MAP (complete — 7 layers + 6x)
...
Layer 5 — DYNAMIC DECISION ARCHITECTURE (SK-510..SK-519, 10 skills)
  STATUS: Complete (v3.2.0).

Layer 6 — AI-DRIVEN TOPOLOGY PLANNING (SK-520..SK-525, 6 skills)  ← NEW v3.3.0
  FLOW-PLAN session type, arbiter panel design, depth decision, meta-arbitration.
  STATUS: Complete.

Layer 6x — SCOPE ISOLATION ENFORCEMENT (SK-526, 1 skill)          ← NEW v3.3.0
  FC-32: scope_isolation arbiter mandatory in every arbiter panel.
  SK-527 pending (module-isolation-arbiter, FC-33).
  STATUS: Complete except SK-527.

Layer 7 — PIPELINE POSITION ENFORCEMENT (SK-528, 1 skill)         ← NEW v3.3.0
  Q0 four-part detection unconditional for GENERATION/PLANNING/MATERIALIZATION.
  STATUS: Complete.
```
...
## WHAT CHANGED IN v4.3.0 (Layer 11 — UI/UX Fleet Discipline)

| Change | What it adds |
|--------|-------------|
| NEW SK-543 | planning--work-scope-inventory-SKILL.md — denominator before any UI/UX work; STATE.scope six-count protocol; prevents fleet claims without established scope |
| NEW SK-544 | planning--improvement-measurement-protocol-SKILL.md — Layer 2 observable PNG delta required before any improvement claim; prevents "grep score improved" standing as UI quality evidence |
| NEW SK-545 | planning--ui-fleet-completion-criteria-SKILL.md — three-criterion minimum per flow; N_examined/(48-N_blocked) as only valid fleet progress metric |
| NEW SK-546 | planning--coverage-completeness-gate-SKILL.md — claim scope must match evidence scope; Step 6 gate at every ⛔ STOP |
| NEW SK-547 | planning--output-skepticism-SKILL.md — three questions (refutation / scope / proxy) before any STOP; planning equivalent of SK-429 |

## WHAT CHANGED IN v4.4.0 (Layer 12 — Plan Integrity)

| Change | What it adds |
|--------|-------------|
| NEW SK-548 | planning--plan-self-validation-SKILL.md — Phase 0 mandatory in every plan; SK-410 focused battery before Phase 1; catches FC-3 phantom skills, FC-5 missing registrations, SK-440 blast radius, execution order inversions |

## WHAT CHANGED IN v4.5.0 (Layer 13 — Visual Examination)

Source: screen-examination.zip analysis 2026-04-21

| Change | What it adds |
|--------|-------------|
| NEW SK-549 | planning--per-image-validation-SKILL.md — 7-axis validation per PNG: Axis A (shell correctness), Axis B (role branching), Axis C (language/RTL), Axis D (business-logic phase + state — mandatory for NEEDS_PURPOSE_BUILT_UI flows), Axis E (human-friendliness), Axis F (5-layer UX audit), Axis G (follow-ups); closes V-R7 convergence gap |
| NEW SK-550 | planning--visual-examination-round-SKILL.md — multi-round fleet improvement protocol; five phases (baseline → systemic → per-cell → rescore → convergence); dual convergence criterion: score-delta < 1% AND primary cells examined = 100% AND NEEDS_PURPOSE_BUILT_UI Axis D verified |
| NEW SK-551 | planning--coverage-matrix-SKILL.md — governs ROUND-2-COVERAGE-MATRIX.json at cell granularity (flow × screen × role × language × state); Python queries; three convergence conditions consumed by SK-550 |

## LOAD ORDER additions (v4.5.0)

| Load order | Skill | Session type | When |
|-----------|-------|-------------|------|
| 5.6 | per-image-validation (SK-549) | Any | Before opening any PNG for examination |
| 5.7 | visual-examination-round (SK-550) | Claude Code | Before any fleet-level visual improvement round |
| 5.8 | coverage-matrix (SK-551) | Any | Any ⛔ STOP claiming visual improvement or convergence |
