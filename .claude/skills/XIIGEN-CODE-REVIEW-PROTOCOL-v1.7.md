...
## Status: Current — supersedes v1.6
...
**SK-539 sub-check (NEW v1.7):** For any plan step that produces a React page, the plan must also declare Phase 7 (UI/UX Compliance) as a step following the React pages phase. A plan that adds `*.tsx` files without a corresponding Phase 7 step fails this sub-check. This is a lighter check than Gate 0m (which verifies the UX compliance itself) — Gate 0g only verifies that Phase 7 is *planned*; Gate 0m verifies that it was *executed correctly*.

Verdict: PASS | FAIL. On FAIL, specify which tenant-facing step lacks visual proof or which React pages phase lacks a corresponding Phase 7 step.
...
### Gate 0m — UI/UX compliance for React page plans — NEW v1.7
...
**This gate catches the five structural FC-18 failure modes (FM-1..FM-5 from `fc-18-ui-ux-compliance-gate.md`) before execution, not after.**
...
### FC-18 — UI/UX Compliance (SK-539) — NEW v1.7

Tier 2 cross-check of Gate 0m from Tier 1. While Gate 0m verifies the plan's *structural intent* is UX-correct, FC-18 at Tier 2 verifies the plan author ran the SK-539 role question check and BLOCK matrix at authoring time — i.e., the plan document contains evidence that the four role questions were answered and the BLOCK check matrix was consulted.

**FC-18 Tier 2 check procedure:**

1. Verify the plan document contains role declarations for each React page (Q1–Q4 per SK-539 §1). If Gate 0m passed but no role declaration appears in the plan text, FC-18 flags CONCERN — Gate 0m may have caught it structurally but the plan provides no audit trail.

2. Verify the plan references FC-18 or SK-539 in its Phase 7 specification, or includes an explicit statement that Phase 7 was considered. A plan that passed Gate 0m structurally but contains no mention of FC-18 compliance work → CONCERN.

3. If the plan implements any of the four missing-page flows (FLOW-20/21/28/48), verify the missing page creation step appears explicitly in the plan with its correct route path.

4. If the plan uses a screen template (T-1..T-7), verify the template name is stated in the plan alongside the page specification.

Cross-reference: FC-18 at Tier 2 overlaps with Gate 0m at Tier 1. Gate 0m catches structural violations in the plan; FC-18 at Tier 2 catches the absence of UX compliance authoring-time evidence in the session record that produced the plan.
...
| 0m | PASS | Role tier declared for all 2 pages; no BLOCK checks triggered; Phase 7 step present |
...
- **SK-539** — UI/UX compliance (29 checks, 52-role taxonomy, 7 screen templates, 4 missing pages), enforced by Gate 0m and FC-18
- **FC-18** — UI/UX compliance gate definition (`fc-18-ui-ux-compliance-gate.md`) — canonical source for Gate 0m BLOCK matrix, Audit Trail format, and exemption rules
- **RESPONSE-CONSTRUCTION-PROTOCOL v1.0** — enforced by FC-17 at Tier 2 and Gates 0j/0k/0l at Tier 1
- **HOW-TO-USE-SKILLS v4.4.0** — Rule 33 mandates protocol; FC-17 and FC-18 are in that rule's FC assignment; Mandatory Check 15 enforces FC-18 at ⛔ STOP
- **DESIGN-REVIEW-PROTOCOL v1.4** — fleet-level aggregation of per-plan findings; Signal 12 corresponds to FC-18
- **FLOW-DOCUMENT-AUTHORING-GUIDE v1.15** — General Rule 30 applies the three-step loop at authoring time (mirroring Gate 0i); General Rule 34 mandates Phase 7 at authoring time (mirroring Gate 0m and FC-18)
