...
## METHOD vs CLASS BOUNDARY (universal binding rule, from core)

The four tests decide where a *service/task-type* boundary falls. A second,
finer question decides whether a unit of logic deserves its **own class behind an
interface** at all, or whether it is just a method on an existing class. This is
the core "class vs method" threshold, TS-adapted for NestJS.

```
Promote logic to its OWN class (behind an `interface`/DI token) when ANY holds:
  □ it owns its own STATE (fields/cache it alone reads and mutates), OR
  □ it has its own LIFECYCLE (independent init/dispose, retry, or TTL), OR
  □ it has 3+ CONSUMERS (3+ NestJS providers/feature modules call it).

Keep it as a METHOD on the existing class when ALL hold:
  □ it is stateless, AND
  □ it shares the caller's lifecycle, AND
  □ it has fewer than 3 consumers.
```

| Owns state? | Own lifecycle? | 3+ consumers? | Decision |
|------------|----------------|---------------|----------|
| YES | any | any | CLASS behind an interface (`@Injectable()` provider + DI token) |
| any | YES | any | CLASS behind an interface |
| any | any | YES | CLASS behind an interface (shared → fabric interface, see cross-cutting/shared-infrastructure) |
| NO | NO | NO | METHOD on the existing class — do not over-extract |

**Why this is binding:** over-extraction (a class for every helper) and
under-extraction (a god-service holding unrelated state) are the two failures the
four boundary tests do not catch by themselves. The 3-consumer line is the same
threshold that promotes a service to SHARED INFRASTRUCTURE in
`planning--cross-cutting-service-SKILL.md` (SK-495) — a class that crosses it must
get a real fabric interface, built before its first consumer.
