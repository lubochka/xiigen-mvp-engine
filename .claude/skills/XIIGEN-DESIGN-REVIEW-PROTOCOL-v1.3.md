...
## 4. The 11 fleet signals — what each column measures
...
The fleet review produces a table with one row per flow and 11 signal columns. An additional 12th column holds the per-flow overall verdict.

Table structure:

| Flow | S1 Topo | S2 Bundle | S3 UI | S4 PNGs | S5 IMPL | S6 Habits | S7 Absorb | S8 Shape | S9 PNG-ev | S10 MD | S11 Order | Verdict |
|------|---------|-----------|-------|---------|---------|-----------|-----------|----------|-----------|--------|-----------|---------|
| FLOW-01 user-registration | PASS | PASS | PARTIAL | MISSING | PASS | PASS | PASS | PASS | CONCERN | PASS | PASS | PARTIAL |
| FLOW-02 profile-enrichment | PASS | PASS | PARTIAL | MISSING | PARTIAL | PASS | PASS | PASS | CONCERN | PASS | PASS | PARTIAL |
| ... | | | | | | | | | | | | |
| FLOW-47 module-lifecycle | PASS | PASS | N/A | N/A | PASS | PASS | PASS | PASS | N/A | PASS | PASS | PASS |
...
Each flow's row of 11 signals aggregates to a single verdict:
...
For each of the 11 signals, count flows in each verdict bucket:
...
- Every per-flow row has verdicts in all 11 signal columns (PASS, PARTIAL, MISSING, or N/A)
...
- **XIIGEN-CODE-REVIEW-PROTOCOL v1.6** — Gates 0i, 0j, 0k, 0l produce the per-flow inputs for Signals 6, 11, correction-thread-audit, source-layer-tags
- **HOW-TO-USE-SKILLS v4.2.0** — Rule 33 mandates response construction; fleet review is where fleet-wide compliance is verified
- **FLOW-DOCUMENT-AUTHORING-GUIDE v1.14** — General Rule 30 (three-step loop at authoring), Rules 31 (JSON+MD companions), 32 (Playwright PNG per feature), 33 (topology disambiguation) — enforced fleet-wide by Signals 10, 9, 1
- **XIIGEN-DESIGN-ARCHITECT-SESSION-GUIDE v1.4** — defines the per-session architect discipline whose fleet-wide drift shows up in Signal 6 and fleet-reviewer self-habits
